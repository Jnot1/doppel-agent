"""ACP auth helpers — detect and advertise Doppel authentication methods."""
...
TERMINAL_SETUP_AUTH_METHOD_ID = "doppel-setup"


def detect_provider() -> Optional[str]:
    """Resolve the active Doppel runtime provider, or None if unavailable.
...
def has_provider() -> bool:
    """Return True if Doppel can resolve any runtime provider credentials."""
    return detect_provider() is not None


def build_auth_methods() -> list[Any]:
    """Return registry-compatible ACP auth methods for Doppel.
    ...
    not have Doppel provider credentials configured yet, so Doppel always
    ...
    if provider:
        methods.append(
            AuthMethodAgent(
                ...
                description=(
                    "Authenticate Doppel using the currently configured "
    ...
    methods.append(
        TerminalAuthMethod(
            ...
            name="Configure Doppel provider",
            description=(
                "Open Doppel's interactive model/provider setup in a terminal. "
                "Use this when Doppel has not been configured on this machine yet."
            ),
            type="terminal",
            args=["--setup"],
        )
