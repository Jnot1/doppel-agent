"""ACP session manager — maps ACP sessions to Doppel AIAgent instances.

Sessions are persisted to the shared SessionDB (``~/.doppel/state.db``) so they
...
def _translate_acp_cwd(cwd: str) -> str:
    """Translate Windows ACP cwd values when Doppel itself is running in WSL.

    Windows ACP clients can launch ``doppel acp`` inside WSL while still sending
...
def _register_task_cwd(task_id: str, cwd: str) -> None:
    ...
    Zed can launch Doppel from a Windows workspace while the ACP process runs
...
def _expand_acp_enabled_toolsets(
    ...
    expanded: List[str] = []
    for name in list(toolsets or ["doppel-acp"]):
...
@dataclass
class SessionState:
    """Tracks per-session state for an ACP-managed Doppel agent."""
...
class SessionManager:
    """Thread-safe manager for ACP sessions backed by Doppel AIAgent instances.
    ...
    def __init__(self, agent_factory=None, db=None):
        ...
        Args:
            agent_factory: Optional callable that creates an AIAgent-like object.
                           ...
                           using the current Doppel runtime provider configuration.
            db:            Optional SessionDB instance. When omitted, the default
                           SessionDB (``~/.doppel/state.db``) is lazily created.
        """
