...
def init_agent(
    ...
    Args:
        ...
        load_soul_identity (bool): If True, still use ~/.doppel/SOUL.md as the primary
    ...
    # both live under ~/.doppel/logs/.  Idempotent, so gateway mode
        ...
        if api_key and base_url:
            ...
            if _routed_client is not None:
                ...
                if not getattr(agent, "_fallback_activated", False):
                    ...
                    raise RuntimeError(
                        "No LLM provider configured. Run `doppel model` to "
                        "select a provider, or run `doppel setup` for first-time "
                        "configuration."
                    )
    ...
    # Session logs go into ~/.doppel/sessions/ alongside gateway sessions
    hermes_home = get_hermes_home()
    agent.logs_dir = hermes_home / "sessions"
    agent.logs_dir.mkdir(parents=True, exist_ok=True)
    # Per-session JSON snapshot writer (~/.doppel/sessions/session_{sid}.json)
    ...
    if _ctx and _ctx < MINIMUM_CONTEXT_LENGTH:
        raise ValueError(
            ...
            f"by Doppel Agent.  Choose a model with at least "
