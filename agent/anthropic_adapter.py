"""Anthropic Messages API adapter for Doppel Agent.
...
# ── Doppel-native PKCE OAuth flow ───────────────────────────────────────
# Mirrors the flow used by Claude Code, pi-ai, and OpenCode.
# Stores credentials in ~/.doppel/.anthropic_oauth.json (our own file).
...
def run_hermes_oauth_login_pure() -> Optional[Dict[str, Any]]:
    """Run Doppel-native OAuth PKCE flow and return credential state."""
    import secrets
    import time
    import webbrowser
    ...
    print()
    print("Authorize Doppel with your Claude Pro/Max subscription.")
    print()
...
def read_hermes_oauth_credentials() -> Optional[Dict[str, Any]]:
    """Read Doppel-managed OAuth credentials from ~/.doppel/.anthropic_oauth.json."""
    if _HERMES_OAUTH_FILE.exists():
        try:
            ...
            logger.debug("Failed to read Doppel OAuth credentials: %s", e)
    return None
    ...
    if is_oauth:
        ...
        for block in system:
            if isinstance(block, dict) and block.get("type") == "text":
                text = block.get("text", "")
                text = text.replace("Doppel Agent", "Claude Code")
                text = text.replace("Doppel agent", "Claude Code")
                text = text.replace("doppel-agent", "claude-code")
