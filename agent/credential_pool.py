...
class CredentialPool:
    ...
    def _sync_codex_entry_from_auth_store(self, entry: PooledCredential) -> PooledCredential:
        ...
        Meanwhile the user may run ``doppel model`` / ``doppel auth`` which
    ...
    def _sync_xai_oauth_entry_from_auth_store(self, entry: PooledCredential) -> PooledCredential:
        ...
        xAI OAuth refresh tokens are single-use.  When another Doppel Agent process
    ...
    def _available_entries(self, *, clear_expired: bool = False, refresh: bool = False) -> List[PooledCredential]:
        ...
        for entry in self._entries:
            ...
            # re-authed via `doppel model` / `doppel auth` after a 429/401,
            ...
            # tokens that another process (or a fresh `doppel model` ->
            ...
            if entry.last_status == STATUS_DEAD:
                ...
                # user can always re-add via ``doppel auth add``.  Singleton-
                ...
                if _is_manual_source(entry.source):
                    ...
                    if dead_at and now - dead_at > DEAD_MANUAL_PRUNE_TTL_SECONDS:
                        ...
                        logger.warning(
                            ...
                            "(reason=%s, age=%.1fh) — re-add via `doppel auth add %s`",
                            _label,
...
def _seed_from_singletons(provider: str, entries: List[PooledCredential]) -> Tuple[bool, Set[str]]:
    ...
    # `doppel auth remove <provider> <N>` is stable across all source types.
    try:
    ...
    if provider == "anthropic":
        # Only auto-discover external credentials (Claude Code, the legacy hermes_pkce source)
        ...
        # API-key vs OAuth is a user-visible choice at `doppel setup` ("Claude
        ...
        # that masquerade.  Prefer ~/.doppel/.env over os.environ for the
        # same reason `_seed_from_env` does — that's the authoritative file
        # that `doppel setup` writes.
        _env_file = load_env()
        ...
        # MiniMax OAuth tokens live in ~/.doppel/auth.json providers.minimax-oauth.
        # Seed the pool so `/auth list` reflects the logged-in state and the
        # standard `doppel auth remove minimax-oauth <N>` flow works.
        ...
        # When the user logs in via ``doppel model`` -> xAI Grok OAuth,
        # tokens are written to the auth.json singleton
        # (``providers["xai-oauth"]``).  Surface them in the pool too so
        # ``doppel auth list`` reflects the logged-in state and so the pool
...
def _seed_from_env(provider: str, entries: List[PooledCredential]) -> Tuple[bool, Set[str]]:
    ...
    # Prefer ~/.doppel/.env over os.environ — the user's config file is the
    # authoritative source for Doppel Agent credentials. Stale env vars from parent
    ...
    # Honour user suppression — `doppel auth remove <provider> <N>` for an
    # env-seeded credential marks the env:<VAR> source as suppressed so it
    # won't be re-seeded from the user's shell environment or ~/.doppel/.env.
    ...
    if provider == "openrouter":
        # Prefer ~/.doppel/.env over os.environ
