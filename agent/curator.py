...
def _load_config() -> Dict[str, Any]:
    """Read curator.* config from ~/.doppel/config.yaml. Tolerates missing file."""
    try:
...
def should_run_now(now: Optional[datetime] = None) -> bool:
    ...
    ``doppel update``. On first observation we seed ``last_run_at`` to "now"
    and defer the first real pass by one full interval. Users who want to
    run it sooner can always invoke ``doppel curator run`` (with or without
...
CURATOR_REVIEW_PROMPT = (
    "You are running as Doppel's background skill CURATOR. This is an "
    ...
    "to find: doppel-config-*, doppel-dashboard-*, gateway-*, codex-*, "
...
def _parse_structured_summary(
    ...
    # Prefer PyYAML when available — every doppel install already has it
...
def _render_report_markdown(p: Dict[str, Any]) -> str:
    ...
    # on disk still lives under ~/.doppel/skills/.archive/ (every removal is
    ...
    if pruned:
        ...
        lines.append(
            ...
            "Restore any via `doppel curator restore <name>`._\n"
        )
        SHOW = 50
    ...
    lines.append("- Restore an archived skill: `doppel curator restore <name>`")
    lines.append("- All archives live under `~/.doppel/skills/.archive/` and are recoverable by `mv`")
...
def run_curator_review(
    ...
    # real pass out. We still record a summary so `doppel curator status`
    ...
    def _llm_pass():
        ...
        # recorded in state so `doppel curator status` can point at it.
        try:
            after_report = skill_usage.agent_created_report()
        except Exception:
            after_report = []
...
def _resolve_review_model(cfg: Dict[str, Any]) -> tuple[str, str]:
    ...
    — so it participates in the canonical aux-model plumbing (``doppel model`` →
...
def _run_llm_review(prompt: str) -> Dict[str, Any]:
    ...
    # (canonical aux-task slot, wired through `doppel model` → auxiliary
