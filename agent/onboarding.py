...
def openclaw_residue_hint_cli() -> str:
    """Banner shown the first time Doppel starts and finds ``~/.openclaw/``.

    Points users at ``doppel claw migrate`` (non-destructive port of config,
    memory, and skills) first. ``doppel claw cleanup`` is mentioned as the
    ...
    return (
        ...
        "To port your config, memory, and skills over to Doppel, run "
        "`doppel claw migrate`.\n"
        "If you've already migrated and want to archive the old directory, "
        "run `doppel claw cleanup` (renames it to ~/.openclaw.pre-migration — "
