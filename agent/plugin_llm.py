...
Plugins built on Doppel Agent often need to make their own LLM calls
...
Today the only stable plugin surfaces extend an existing Doppel Agent
...
embedded slugs, no shorthands. This mirrors Doppel Agent's main config
...
handles every provider, fallback chain, and per-task override Doppel Agent
supports.
"""
