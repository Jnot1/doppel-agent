...
# cli.py) or `HERMES_REDACT_SECRETS=false` in ~/.doppel/.env. An opt-out
...
def mask_secret(
    ...
    *,
    ...
    Canonical helper for display-time redaction across Doppel Agent — used by
    ``doppel config``, ``doppel status``, ``doppel dump``, and anywhere
