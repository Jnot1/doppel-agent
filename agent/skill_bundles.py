...
Storage
-------
Bundles live in ``~/.doppel/skill-bundles/*.yaml`` (and the equivalent
profile-aware directory under ``DOPPEL_HOME``). Each file looks like::
...
- :func:`list_bundles` — return rich info for display (``doppel bundles``)
...
def _bundles_dir() -> Path:
    """Return the canonical bundles directory under DOPPEL_HOME.

    Honors ``HERMES_BUNDLES_DIR`` for tests; falls back to
    ``<DOPPEL_HOME>/skill-bundles``.
    """
...
# ---------------------------------------------------------------------------
# File-level CRUD helpers — used by `doppel bundles` CLI subcommand.
# ---------------------------------------------------------------------------
