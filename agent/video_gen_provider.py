...
as ``kind: backend``) or ``~/.doppel/plugins/video_gen/<name>/`` (user, opt-in
via ``plugins.enabled``).
...
class VideoGenProvider(abc.ABC):
    ...
    def capabilities(self) -> Dict[str, Any]:
        ...
        Used by the tool layer for soft validation and by ``doppel tools``
...
def _videos_cache_dir() -> Path:
    """Return ``$DOPPEL_HOME/cache/videos/``, creating parents as needed."""
...
def save_b64_video(
    ...
    *,
    ...
    """Decode base64 video data and write under ``$DOPPEL_HOME/cache/videos/``.
