"""doppel claw — OpenClaw migration commands.

Usage:
    doppel claw migrate              # Preview then migrate (always shows preview first)
    doppel claw migrate --dry-run    # Preview only, no changes
    doppel claw migrate --yes        # Skip confirmation prompt
    doppel claw migrate --preset full --overwrite --migrate-secrets  # Full run w/ secrets
    doppel claw migrate --no-backup  # Skip pre-migration snapshot
    doppel claw cleanup              # Archive leftover OpenClaw directories
    doppel claw cleanup --dry-run    # Preview what would be archived
"""
...
def _warn_if_gateway_running(auto_yes: bool) -> None:
    ...
    print_info("Recommendation: stop the gateway first with 'doppel stop'.")
    print()
...
def claw_command(args):
    ...
    if action == "migrate":
        ...
        print("Usage: doppel claw <command> [options]")
        print()
        print("Commands:")
        print("  migrate          Migrate settings from OpenClaw to Doppel")
        print("  cleanup          Archive leftover OpenClaw directories after migration")
        print()
        print("Run 'doppel claw <command> --help' for options.")


def _cmd_migrate(args):
    """Run the OpenClaw → Doppel migration."""
    ...
    if not source_dir.is_dir():
        ...
        print_info("You can specify a custom path: doppel claw migrate --source /path/to/.openclaw")
        return
    ...
    if not auto_yes:
        if not sys.stdin.isatty():
            ...
            print_info("To execute, re-run with: doppel claw migrate --yes")
            return
        if not prompt_yes_no("Proceed with migration?", default=True):
            print_info("Migration cancelled.")
            return

    # ── Phase 2b: Pre-apply backup of the Doppel home ─────────
    ...
    # restorable with `doppel import`.  Mirrors OpenClaw's
    ...
    if not no_backup:
        try:
            ...
            if backup_archive:
                ...
                print_info(f"Restore with: doppel import {backup_archive.name}")
        except Exception as e:
            print()
            print_error(f"Could not create pre-migration backup: {e}")
            print_info(
                "Re-run with --no-backup to skip, or free up disk space under the Doppel home."
            )
...
def _cmd_cleanup(args):
    ...
    if running:
        ...
        if not auto_yes:
            ...
            if not prompt_yes_no("Proceed anyway?", default=False):
                print_info("Aborted. Stop OpenClaw first, then re-run: doppel claw cleanup")
                return
    ...
    for source_dir in dirs_to_check:
        ...
        if dry_run:
            ...
            print_info("To execute, re-run with: doppel claw cleanup --yes")
...
def _print_migration_report(report: dict, dry_run: bool):
    ...
    if dry_run:
        ...
        print_info(f"  doppel claw migrate --preset {report.get('preset', 'full')}")
    elif migrated:
        print()
        ...
        if skipped_keys:
            ...
            print_info("  doppel claw migrate --migrate-secrets")
            print()
            print_info("Or add your key manually:")
            print_info("  doppel config set OPENROUTER_API_KEY sk-or-v1-...")
