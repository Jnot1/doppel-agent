...
def _cmd_run(args) -> int:
    ...
    if not synchronous:
        print("llm pass running in background — check `doppel curator status` later")
    if dry:
        if synchronous:
            print(
                "dry-run: no changes applied. Read the report with "
                "`doppel curator status` and run `doppel curator run` (no flag) to apply."
            )
        else:
            print(
...
def _cmd_archive(args) -> int:
    ...
    if skill_usage.get_record(args.skill).get("pinned"):
        print(
            ...
            f"`doppel curator unpin {args.skill}`"
        )
        return 1
    ...
    if target_path is None:
        ...
        if not rows:
            print(
                ...
                "`doppel curator backup` or wait for the next curator run."
            )
        else:
            print(
...
def cli_main(argv=None) -> int:
    ...
    parser = argparse.ArgumentParser(prog="doppel curator")
    register_cli(parser)
