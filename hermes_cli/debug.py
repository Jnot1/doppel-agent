...
Currently supports:
    doppel debug share    Upload debug report (system info + logs) to a
...
_REDACTION_BANNER = (
    "[doppel debug share: log content redacted at upload time. "
...
def _pending_file() -> Path:
    ...
    ``_sweep_expired_pastes`` once per hour.  ``doppel debug share`` also
...
def _save_pending(entries: list[dict]) -> None:
    ...
    try:
        ...
        # Non-fatal — worst case the user has to run ``doppel debug delete``
        # manually.
        pass
...
def _sweep_expired_pastes(now: Optional[float] = None) -> tuple[int, int]:
    ...
    intended to be called from every ``doppel debug`` invocation with
    minimal noise.
    """
    entries = _load_pending()
    if not entries:
...
⚠️  This will upload the following to a public paste service:
  • System info (OS, Python version, Doppel version, provider, which API keys
...
def delete_paste(url: str) -> bool:
    ...
    req = urllib.request.Request(
        ...
        headers={"User-Agent": "doppel-agent/debug-share"},
    )
...
def _schedule_auto_delete(urls: list[str], delay_seconds: int = _AUTO_DELETE_SECONDS):
    ...
    every ``doppel debug share`` invocation added ~20 MB of resident Python
    interpreters that never exited until the sleep completed.

    The replacement is stateless: we append to ``~/.doppel/pastes/pending.json``
    and the gateway's cron ticker sweeps expired entries once per hour.
    ``doppel debug share`` also runs an opportunistic sweep as a fallback
...
def _upload_paste_rs(content: str) -> str:
    ...
    req = urllib.request.Request(
        ...
        headers={
            ...
            "User-Agent": "doppel-agent/debug-share",
        },
    )
    with urllib.request.urlopen(req, timeout=30) as resp:
...
def _upload_dpaste_com(content: str, expiry_days: int = 7) -> str:
    ...
    """
    boundary = "----DoppelDebugBoundary9f3c"
...
def _capture_dump() -> str:
    """Run ``doppel dump`` and return its stdout as a string."""
...
def collect_debug_report(
    *,
    ...
    dump_text
        Pre-captured dump output.  If empty, ``doppel dump`` is run
        internally.
...
def run_debug_share(args):
    ...
    if redact:
        logger.info(
            "doppel debug share: applied force-mode redaction to log snapshots before upload"
        )
    ...
    print(f"To delete now:  doppel debug delete <url>")

    print(f"\nShare these links with the Doppel team for support.")
...
def run_debug_delete(args):
    ...
    if not urls:
        print("Usage: doppel debug delete <url> [<url> ...]")
        print("  Deletes paste.rs pastes uploaded by 'doppel debug share'.")
        return
...
def run_debug(args):
    ...
    # Opportunistic sweep of expired pastes on every ``doppel debug`` call.
    ...
    # best-effort — any failure is swallowed so ``doppel debug`` stays
    ...
    if subcmd == "share":
        ...
        # Default: show help
        print("Usage: doppel debug <command>")
        print()
        print("Commands:")
