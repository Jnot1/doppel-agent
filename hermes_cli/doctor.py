...
Diagnoses issues with Doppel Agent setup.
"""
...
def run_doctor(args):
    ...
    print(color("│                 🩺 Doppel Doctor                        │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
        ...
        if fresh_hits:
            for hit in fresh_hits:
                ...
                manual_issues.append(
                    ...
                    f"`doppel doctor --ack {hit.advisory.id}`."
                )
    ...
    if env_path.exists():
        ...
        if _has_provider_env_config(content):
            ...
            issues.append("Run 'doppel setup' to configure API keys")
    else:
        ...
        if fallback_env.exists():
            ...
            if should_fix:
                ...
                check_info("Run 'doppel setup' to configure API keys")
                fixed_count += 1
            else:
                check_info("Run 'doppel setup' to create one")
                issues.append("Run 'doppel setup' to create .env")
                ...
                if catalog_provider is None or (
                    ...
                    _fail_and_issue(
                        ...
                        (
                            ...
                            f"Fix: run 'doppel config set model.provider <valid_provider>'"
                        ),
                        issues,
                    )
                ...
                try:
                    ...
                    if not configured:
                        _fail_and_issue(
                            ...
                            f"(check {_DHH}/.env or run 'doppel setup')",
                            (
                                f"No credentials found for provider '{runtime_provider}'. "
                                f"Run 'doppel setup' or set the provider's API key in {_DHH}/.env, "
                                f"or switch providers with 'doppel config set model.provider <name>'"
                            ),
                            issues,
                        )
                except Exception:
            ...
            if current_ver < latest_ver:
                ...
                if should_fix:
                    try:
                        ...
                        issues.append("Run 'doppel setup' to migrate config")
                else:
                    issues.append("Run 'doppel doctor --fix' or 'doppel setup' to migrate config")
            else:
        ...
        try:
            ...
            if stale_root_keys:
                ...
                if should_fix:
                    ...
                    issues.append("Stale root-level provider/base_url in config.yaml — run 'doppel doctor --fix'")
        except Exception:
            pass
    ...
    if soul_path.exists():
        ...
        check_warn(f"{_DHH}/SOUL.md not found", "(create it to give Doppel a custom personality)")
        if should_fix:
            soul_path.parent.mkdir(parents=True, exist_ok=True)
            soul_path.write_text(
                "# Doppel Agent Persona\n\n"
                "<!-- Edit this file to customize how Doppel communicates. -->\n\n"
                "You are Doppel, a helpful AI assistant.\n",
                encoding="utf-8",
            )
        ...
        try:
            ...
            if wal_size > 50 * 1024 * 1024:  # 50 MB
                ...
                if should_fix:
                    ...
                    issues.append("Large WAL file — run 'doppel doctor --fix' to checkpoint")
        ...
        if _venv_bin is None:
            ...
            if _cmd_link.is_symlink():
                ...
                if _target == _expected:
                    ...
                    check_warn(
                        f"{_cmd_link_display}/doppel points to wrong target",
                        f"(→ {_target}, expected → {_expected})"
                    )
                    if should_fix:
                        _cmd_link.unlink()
                        _cmd_link.symlink_to(_venv_bin)
                        check_ok(f"Fixed symlink: {_cmd_link_display}/doppel → {_venv_bin}")
                        fixed_count += 1
                    else:
                        issues.append(f"Broken symlink at {_cmd_link_display}/doppel — run 'doppel doctor --fix'")
            elif _cmd_link.exists():
                # It's a regular file, not a symlink — possibly a wrapper script
                check_ok(f"{_cmd_link_display}/doppel exists (non-symlink)")
            else:
                check_fail(
                ...
                if should_fix:
                    ...
                    _cmd_link.symlink_to(_venv_bin)
                    check_ok(f"Created symlink: {_cmd_link_display}/doppel → {_venv_bin}")
                    fixed_count += 1
                    ...
                    issues.append(f"Missing {_cmd_link_display}/doppel symlink — run 'doppel doctor --fix'")
    ...
    try:
        ...
        if api_disabled:
            issues.append("Run 'doppel setup' to configure missing API keys for full tool access")
    except Exception as e:
    ...
    if should_fix and fixed_count > 0:
        ...
        print()
        if not should_fix:
            print(color("  Tip: run 'doppel doctor --fix' to auto-fix what's possible.", Colors.DIM))
    else:
        print(color("─" * 60, Colors.GREEN))
