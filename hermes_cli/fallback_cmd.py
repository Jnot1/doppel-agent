"""
doppel fallback — manage the fallback provider chain.

Fallback providers are tried in order when the primary model fails with
rate-limit, overload, or connection errors. See:
the fallback providers guide in Doppel documentation.

Subcommands:
  doppel fallback [list]   Show the current fallback chain (default when no subcommand)
  doppel fallback add      Pick provider + model via the same picker as `doppel model`,
                           then append the selection to the chain
  doppel fallback remove   Pick an entry to delete from the chain
  doppel fallback clear    Remove all fallback entries

Storage: ``fallback_providers`` in the active agent home config (fresh installs
default to ``~/.doppel/config.yaml``). The legacy single-dict ``fallback_model``
format is migrated to the new list format on first add.
"""
...
def _restore_auth_active_provider(value: Any) -> None:
    ...
    try:
        ...
        # `doppel model` to fix it.  Don't fail the fallback add.
        pass
...
def cmd_fallback_list(args) -> None:  # noqa: ARG001
    ...
    if not chain:
        ...
        print()
        print("  Add one with:  doppel fallback add")
        print()
        return
    ...
    print(f"  Docs: {FALLBACK_DOCS_URL}")
    print()
...
def cmd_fallback_add(args) -> None:
    """Launch the same picker as `doppel model`, then append the selection to the chain."""
    ...
    print("  `doppel model` — select the provider + model you want as a fallback.")
    print()
    ...
    print()
    print("  Run `doppel fallback list` to view, or `doppel fallback remove` to delete.")
...
def cmd_fallback(args) -> None:
    """Top-level dispatcher for ``doppel fallback [subcommand]``."""
