...
def hooks_command(args) -> None:
    ...
    if not sub:
        print("Usage: doppel hooks {list|test|revoke|doctor}")
        print("Run 'doppel hooks --help' for details.")
        return
...
def _cmd_list(_args) -> None:
    ...
    if not specs:
        print("No shell hooks configured in ~/.doppel/config.yaml.")
        print("See `doppel hooks --help` or")
                ...
                if entry and entry.get("approved_at"):
                    ...
                    if mtime_now and mtime_at and mtime_now > mtime_at:
                        print(
                            ...
                            f"run `doppel hooks doctor` to re-validate"
                        )
        print()
...
def _print_run_result(result: Dict[str, Any]) -> None:
    ...
    if parsed:
        print(f"      parsed (Doppel wire shape): {json.dumps(parsed)}")
    else:
    ...
    if entry and entry.get("script_mtime_at_approval"):
        ...
        if mtime_now and mtime_at and mtime_now > mtime_at:
            ...
            print(f"      ⚠ script modified since approval "
                  ...
                  f"then `doppel hooks revoke` + re-approve to refresh")
