"""``doppel logs`` — view and filter Doppel log files.

Supports tailing, following, session filtering, level filtering,
component filtering, and relative time ranges. All log files live under the
active agent home (fresh installs default to ``~/.doppel/logs/``).

Usage examples::

    doppel logs                    # last 50 lines of agent.log
    doppel logs -f                 # follow agent.log in real time
    doppel logs errors             # last 50 lines of errors.log
    doppel logs gateway -n 100     # last 100 lines of gateway.log
    doppel logs --level WARNING    # only WARNING+ lines
    doppel logs --session abc123   # filter by session ID substring
    doppel logs --component tools  # only tool-related lines
    doppel logs --since 1h         # lines from the last hour
    doppel logs --since 30m -f     # follow, starting 30 min ago
"""
...
def tail_log(
    ...
    *,
    ...
    if not log_path.exists():
        ...
        print(f"(Logs are created when Doppel runs — try 'doppel chat' first)")
        sys.exit(1)
...
def list_logs() -> None:
    ...
    if not found:
        print("  (no log files yet — run 'doppel chat' to generate logs)")
