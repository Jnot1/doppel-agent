...
and install them with ``doppel mcp install <name>`` (or by toggling in the
picker, which flows them through any required env/OAuth setup).

Catalog policy:
- Entries are added only by merging a PR into doppel-agent. Presence in the
...
- Secrets prompted at install time go to ``~/.doppel/.env`` (the
...
def _parse_manifest(path: Path) -> CatalogEntry:
    ...
    if mv != _MANIFEST_VERSION:
        raise CatalogError(
            ...
            f"(this Doppel Agent understands version {_MANIFEST_VERSION})"
        )
...
def list_catalog() -> List[CatalogEntry]:
    ...
    UIs can tell the user their Doppel Agent install is out of date.
    """
    root = _catalog_root()
    if not root.exists():
        return []
...
def catalog_diagnostics() -> List[tuple]:
    ...
    is one of:
      - ``future_manifest`` — manifest_version is newer than this Doppel Agent
        understands. Update Doppel Agent to install this entry.
...
def _apply_tool_selection(
    ...
    if probed is None:
        ...
        if manifest_default:
            ...
            print(color(
                ...
                f"Run `doppel mcp configure {entry.name}` after the server "
            ...
            print(color(
                ...
                f"Run `doppel mcp configure {entry.name}` after first "
                "connect to prune.",
                Colors.YELLOW,
            ))
        return
    ...
    if not chosen_indices:
        ...
        print(color(
            f"  No tools selected. Run `doppel mcp configure {entry.name}` "
            "to change.",
            Colors.YELLOW,
        ))
        return
...
def install_entry(entry: CatalogEntry, *, enable: bool = True) -> None:
    ...
    if entry.auth.type == "api_key":
        ...
        if entry.auth.provider:
            ...
            print(color(
                ...
                f"`doppel auth {entry.auth.provider}` if you have not "
    ...
    print(color(
        ...
        f"Start a new Doppel Agent session to load its tools.",
        Colors.GREEN,
    ))
