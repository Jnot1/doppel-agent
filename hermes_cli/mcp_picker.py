"""MCP picker — interactive `doppel mcp picker` (also the default `doppel mcp`).

Lists every catalog entry plus any custom MCP servers the user has added via
``doppel mcp add``, lets them pick one, and routes to install / enable /
disable / uninstall / configure-tools flows.

Mirrors the `doppel plugin` picker UX: arrow keys to navigate, ENTER on a row
...
def _enable_disable(name: str, *, enable: bool) -> None:
    ...
    print(color(
        ...
        "Start a new Doppel session for changes to take effect.",
        Colors.GREEN,
    ))
...
def _print_rows_text(rows: List[_Row]) -> None:
    ...
    as the default output of `doppel mcp catalog`."""
    if not rows:
        print()
    ...
    print()
    print(color(
        "  Install: doppel mcp install <name>    Picker: doppel mcp",
        Colors.DIM,
    ))

    # Surface manifest-version warnings so users know when their Doppel is
    ...
    if future:
        ...
        for name, _, msg in future:
            print(color(
                f"  ⚠ '{name}' requires a newer Doppel — run `doppel update` "
...
def show_catalog() -> None:
    """`doppel mcp catalog` — print the curated list + custom servers, no interaction."""
    _print_rows_text(_build_rows())


def run_picker() -> None:
    """`doppel mcp picker` (and default `doppel mcp`) — interactive selector.
...
def install_by_name(identifier: str) -> int:
    """`doppel mcp install <name>` — non-interactive entry-point.
    ...
    if entry is None:
        print(color(
            ...
            "Run `doppel mcp catalog` to see available entries.",
            Colors.RED,
        ))
        return 1
    try:
