...
def _xai_curated_models() -> list[str]:
    ...
    when the cache is empty or unreadable. Doppel Agent refreshes the cache from
...
def union_with_portal_free_recommendations(
    ...
    *,
    ...
    older Doppel Agent that doesn't ship that model in its hardcoded curated
...
def union_with_portal_paid_recommendations(
    ...
    *,
    ...
    picker even if the user is running an older Doppel Agent that doesn't ship
...
class ProviderEntry(NamedTuple):
    slug: str
    label: str
    tui_desc: str   # detailed description for `doppel model` TUI
...
#
# Some vendors expose several provider slugs (one per endpoint /
...
def model_supports_fast_mode(model_id: Optional[str]) -> bool:
    """Return whether Doppel Agent should expose the /fast toggle for this model."""
...
def provider_model_ids(provider: Optional[str], *, force_refresh: bool = False) -> list[str]:
    ...
    on the platform appear in ``/model`` without a Doppel Agent release.
    """
    normalized = normalize_provider(provider)
    if normalized == "openrouter":
    ...
    if normalized == "openai-codex":
        ...
        # a Doppel Agent release). Falls back to the hardcoded catalog if no token
    ...
    if normalized == "nous":
        ...
        # added Portal models still surface without a Doppel Agent release.
    ...
    *,
    ...
    if normalized == "custom" or normalized.startswith("custom:"):
        ...
        message = (
            ...
            f"Doppel Agent will still save `{requested}`, but the endpoint should expose `/models` for verification."
        )
        if api_mode == "anthropic_messages":
            message += (
