...
def format_nous_portal_entitlement_message(
    ...
    *,
    ...
    if account_info is None:
        return (
            f"Doppel Agent could not verify your Nous Portal entitlement, so {capability} "
            f"is unavailable. Run `doppel model` to refresh your login, or check "
    ...
    if not account_info.logged_in:
        if account_info.inference_credential_present:
            return (
                f"Nous inference credentials are configured, but Doppel Agent cannot verify "
                f"your Nous Portal paid access for {capability}. Log in with "
                f"`doppel model` to enable Portal-managed features. Billing and "
                f"credits are managed at {billing_url}."
            )
        return (
            f"Log in to Nous Portal to use {capability}: run `doppel model`. "
            f"Billing and credits are managed at {billing_url}."
        )

    if account_info.paid_service_access is None:
        detail = (
            f"Doppel Agent could not verify your Nous Portal paid access, so {capability} "
            f"is unavailable."
        )
        if account_info.error:
            detail += f" Account lookup failed: {account_info.error}."
        if include_refresh_hint:
            detail += " Run `doppel model` to refresh your session."
    ...
    if reason == "account_missing":
        return (
            f"Doppel Agent could not find a Nous Portal account or organisation for this "
            f"login, so {capability} is unavailable. Run `doppel model` to "
    ...
    if reason == "no_usable_credits" or account_info.paid_service_access is False:
        ...
        if include_refresh_hint and not account_info.fresh:
            message += " If you recently bought credits, run `doppel model` to refresh Doppel Agent."
        return message
