...
A distribution is a Doppel profile published as a git repository (or
...
def check_hermes_requires(spec: str, current_version: str) -> None:
    ...
    if not ok:
        raise DistributionError(
            f"This distribution requires Doppel {op}{target}, "
...
def plan_install(
    ...
    if manifest is None:
        raise DistributionError(
            ...
            "this source is not a Doppel distribution."
        )
    ...
    if canon == "default":
        raise DistributionError(
            ...
            "root profile (~/.doppel).  Pass --name <name> to install under a "
            "new profile."
        )
    manifest.name = canon
