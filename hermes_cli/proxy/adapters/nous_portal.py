...
Reads the user's Nous OAuth state from the active agent home auth.json file
through the
    ...
    def _get_credential(
        ...
        *,
        ...
        with self._lock:
            ...
            if state is None:
                raise RuntimeError(
                    "Not logged into Nous Portal. Run `doppel auth add nous` first."
                )
            ...
            if not runtime_key:
                raise RuntimeError(
                    ...
                    "Try `doppel auth add nous` to re-authenticate."
                )
