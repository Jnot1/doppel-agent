...
class XAIGrokAdapter(UpstreamAdapter):
    """Proxy upstream for xAI Grok via Doppel-managed OAuth credentials."""

    auth_hint = "doppel auth add xai-oauth --type oauth"
        ...
        with self._lock:
            ...
            if pool is None or not pool.has_credentials():
                raise RuntimeError(
                    ...
                    "`doppel auth add xai-oauth --type oauth` first."
                )
            ...
            if entry is None:
                raise RuntimeError(
                    ...
                    "`doppel auth reset xai-oauth` or re-authenticate with "
    ...
    def _credential_from_entry(self, entry: PooledCredential) -> UpstreamCredential:
        ...
        if not bearer:
            raise RuntimeError(
                ...
                "Re-authenticate with `doppel auth add xai-oauth --type oauth`."
            )
