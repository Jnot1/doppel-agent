"""CLI handlers for the ``doppel proxy`` subcommand."""
...
def _print_aiohttp_missing() -> None:
    print(
        "doppel proxy requires aiohttp. Install one of:\n"
...
def cmd_proxy_start(args: Any) -> int:
    ...
    if not adapter.is_authenticated():
        auth_hint = getattr(adapter, "auth_hint", f"doppel auth add {adapter.name}")
        print(
    ...
    print(
        f"Starting Doppel proxy for {adapter.display_name}\n"
...
def cmd_proxy_status(args: Any) -> int:
    ...
    print("Doppel proxy upstream adapters\n")
    ...
    print(
        "\nStart the proxy with: doppel proxy start [--provider <name>]"
    )
    return 0
...
def cmd_proxy(args: Any) -> int:
    """Dispatch ``doppel proxy <subcommand>``."""
    ...
    print(
        "doppel proxy — local OpenAI-compatible proxy that attaches your\n"
        "OAuth-authenticated provider credentials to outbound requests.\n"
        "\n"
        "Subcommands:\n"
        "  doppel proxy start [--provider nous|xai] [--host 127.0.0.1] [--port 8645]\n"
