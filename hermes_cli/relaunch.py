"""
Unified self-relaunch for Doppel CLI.

Preserves critical flags (--tui, --dev, --profile, --model, etc.) across
process replacement so that ``doppel sessions browse`` or post-setup relaunch
doesn't silently drop the user's UI mode or other preferences.

Also works when ``doppel`` is not on PATH (e.g. ``nix run`` or ``python -m``).
"""
...
def _extract_inherited_flags(argv: Sequence[str]) -> list[str]:
    """Pull out flags that should carry over into a self-relaunched doppel."""
    flags: list[str] = []
    i = 0
    while i < len(argv):
        arg = argv[i]
...
def resolve_hermes_bin() -> Optional[str]:
    """Find the preferred CLI entry point.

    Priority:
      1. ``sys.argv[0]`` if it resolves to a real executable.
      2. ``shutil.which("doppel")`` on PATH.
      3. ``shutil.which("hermes")`` on PATH as a legacy fallback.
    ...
    ``doppel.exe`` / ``hermes.exe`` on PATH or the
...
def build_relaunch_argv(
    ...
    *,
    ...
    """Construct an argv list for replacing the current process with doppel.
...
def relaunch(
    ...
    *,
    ...
    """Replace the current process with a fresh doppel invocation.
    ...
    relaunch contract wants: "run doppel again as if the user had typed
    the new argv".
    ...
    *emulates* exec by spawning the child and exiting the parent, but
    ...
    is usually ``doppel.exe`` (a Python console-script shim that wraps
    ...
    That's functionally equivalent — the user sees "doppel exited, then
    new doppel started" — just with two PIDs in play instead of one.
    """
    ...
    if sys.platform == "win32":
        ...
        try:
            ...
            # cryptic.  Common causes: ``doppel`` not on PATH yet (install
            # hasn't propagated User PATH into this shell) or a stale shim.
            print(
                f"\nDoppel relaunch failed: {exc}\n"
                f"Command: {' '.join(new_argv)}\n"
                f"Fix: open a new terminal so PATH picks up, then re-run doppel.",
                file=sys.stderr,
            )
            sys.exit(1)
    else:
