"""
Security advisory checker for Doppel Agent.
...
- **Acknowledgeable.** Once the user has read and acted on an advisory they
  can dismiss it via ``doppel doctor --ack <id>``; the ack is persisted to
...
1. ``doppel doctor`` (and ``doppel doctor --ack <id>``)
...
This module is intentionally dependency-free beyond the stdlib so it can run
in environments where the rest of Doppel failed to import.
"""
...
ADVISORIES: tuple[Advisory, ...] = (
    Advisory(
        ...
        summary=(
            ...
            "imported mistralai 2.4.6 — including doppel when configured "
        ...
        remediation=(
            ...
            "Rotate API keys in ~/.doppel/.env (OpenRouter, Anthropic, OpenAI, "
            ...
            "After cleanup: doppel doctor --ack shai-hulud-2026-05  to dismiss "
            "this warning.",
        ),
...
def _installed_version(pkg_name: str) -> Optional[str]:
    ...
    except ImportError:  # py<3.8 — Doppel requires 3.10+ but defensive.
        return None
    try:
...
def short_banner_lines(hits: list[AdvisoryHit]) -> list[str]:
    ...
    lines = [
        ...
        "  Run 'doppel doctor' for remediation steps.",
    ]
    if len(hits) > 1:
...
# ``<agent-home>/cache/advisory_banner_seen`` (a single line per advisory ID:
...
def render_doctor_section(hits: list[AdvisoryHit]) -> tuple[bool, list[str]]:
    """Render the security-advisory section for ``doppel doctor``.
...
def gateway_log_message(hits: list[AdvisoryHit]) -> Optional[str]:
    ...
    return (f"{len(fresh)} security advisories active "
            ...
            f"Run `doppel doctor` on the gateway host for details.")
