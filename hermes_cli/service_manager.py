...
# active (set via the ``doppel -p <profile>`` flag before the call). This
...
class GatewayNotRegisteredError(S6Error):
    ...
    Most commonly: ``doppel -p typo gateway start`` when no profile
    ...
    def __init__(self, profile: str) -> None:
        ...
        super().__init__(
            ...
            f"`doppel profile create {profile}` first, or pass "
    ...
    def _render_log_run(profile: str) -> str:
        ...
        per line, in order:
          ...
          2. ``T <log_dir>`` — also write a timestamped copy to the
             ...
             files). This is what ``doppel logs`` reads and what
