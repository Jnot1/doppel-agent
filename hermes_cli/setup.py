"""
Interactive setup wizard for Doppel Agent.
...
def print_noninteractive_setup_guidance(reason: str | None = None) -> None:
    ...
    print()
    print(color("⚕ Doppel Setup — Non-interactive mode", Colors.CYAN, Colors.BOLD))
    print()
    if reason:
        print_info(reason)
    print_info("The interactive wizard cannot be used here.")
    print()
    print_info("Configure Doppel using environment variables or config commands:")
    print_info("  doppel config set model.provider custom")
    print_info("  doppel config set model.base_url http://localhost:8080/v1")
    print_info("  doppel config set model.default your-model-name")
    print()
    print_info("Or set OPENROUTER_API_KEY / OPENAI_API_KEY in your environment.")
    print_info("Run 'doppel setup' in an interactive terminal to use the full wizard.")
    print()
...
def _prompt_api_key(var: dict):
    ...
    if value:
        ...
        print_warning("  Skipped (configure later with 'doppel setup')")
...
def _print_setup_summary(config: dict, hermes_home):
    ...
    if _vision_backends:
        ...
        tool_status.append(("Vision (image analysis)", False, "run 'doppel setup' to configure"))
    ...
    if subscription_features.tts.managed_by_nous:
        ...
        if neutts_ok:
            ...
            tool_status.append(("Text-to-Speech (NeuTTS — not installed)", False, "run 'doppel setup tts'"))
        ...
        if kittentts_ok:
            ...
            tool_status.append(("Text-to-Speech (KittenTTS — not installed)", False, "run 'doppel setup tts'"))
    else:
    ...
    if subscription_features.modal.managed_by_nous:
        ...
        if subscription_features.modal.direct_override:
            ...
            tool_status.append(("Modal Execution", False, "run 'doppel setup terminal'"))
    ...
    if disabled_tools:
        print_warning(
            "Some tools are disabled. Run 'doppel setup tools' to configure them,"
        )
    ...
    print()
    print(f"   {color('doppel setup', Colors.GREEN)}          Re-run the full wizard")
    print(f"   {color('doppel setup model', Colors.GREEN)}    Change model/provider")
    print(f"   {color('doppel setup terminal', Colors.GREEN)} Change terminal backend")
    print(f"   {color('doppel setup gateway', Colors.GREEN)}  Configure messaging")
    print(f"   {color('doppel setup tools', Colors.GREEN)}    Configure tool providers")
    print()
    print(f"   {color('doppel config', Colors.GREEN)}         View current settings")
    print(
        f"   {color('doppel config edit', Colors.GREEN)}    Open config in your editor"
    )
    print(f"   {color('doppel config set <key> <value>', Colors.GREEN)}")
    ...
    print()
    print(f"   {color('doppel', Colors.GREEN)}              Start chatting")
    print(f"   {color('doppel gateway', Colors.GREEN)}      Start messaging gateway")
    print(f"   {color('doppel doctor', Colors.GREEN)}       Check for issues")
    print()
...
def setup_model_provider(config: dict, *, quick: bool = False):
    ...
    try:
        ...
        print_info("You can try again later with: doppel model")
    ...
    if not quick and _supports_same_provider_pool_setup(selected_provider):
        try:
            ...
            print_info(
                "Doppel can keep multiple credentials for one provider and rotate between"
            )
            print_info(
    ...
    if _vision_needs_setup:
        ...
        print_info("doesn't currently provide one Doppel can auto-use for vision,")
        ...
        if _vision_idx == 0:  # OpenRouter
            ...
            print_info("Skipped — add later with 'doppel setup' or configure AUXILIARY_VISION_* settings")
            ...
            if choice_idx == 0:
                ...
                if api_key:
                    ...
                    print_warning(
                        ...
                        f"via doppel setup model or {_dhh()}/.env to use xAI TTS. "
                        "Falling back to Edge TTS."
                    )
                    selected = "edge"
            else:
...
def setup_tts(config: dict):
    """Standalone TTS setup (for 'doppel setup tts')."""
    _setup_tts_provider(config)
...
def _apply_default_agent_settings(config: dict):
    ...
    print_info("  Run `doppel setup agent` later to customize.")
...
def _setup_telegram():
    ...
    print()
    print_info("📬 Home Channel: where Doppel delivers cron job results,")
    ...
    if existing:
        ...
        if not prompt_yes_no("Reconfigure Slack?", False):
            ...
            if prompt_yes_no(
                ...
                "list? (recommended after `doppel update`)",
                True,
            ):
...
def _write_slack_manifest_and_instruct():
    ...
    try:
        ...
        manifest = _build_full_manifest(
            bot_name="Doppel",
            bot_description="Your Doppel agent on Slack",
        )
        ...
        print_info(
            "   Re-run `doppel slack manifest --write` anytime to refresh after "
            "Doppel adds new commands."
        )
...
def _setup_matrix():
    ...
    if token or get_env_value("MATRIX_PASSWORD"):
        ...
        print()
        print_info("📬 Home Room: where Doppel delivers cron job results and notifications.")
...
def _setup_bluebubbles():
    ...
    print_info("Connects Doppel to iMessage via BlueBubbles — a free, open-source")
...
def setup_gateway(config: dict):
    ...
    print_info("Connect to messaging platforms to chat with Doppel from anywhere.")
    ...
    if not selected:
        print_info("No platforms selected. Run 'doppel setup gateway' later to configure.")
        return
    ...
    if any_messaging:
        ...
        if missing_home:
            ...
            for plat in missing_home:
                print_info(
                    f"     doppel config set {plat.upper()}_HOME_CHANNEL <channel_id>"
                )
        ...
        if service_running:
            ...
            if prompt_yes_no(
                ...
                print_info("  Or run in foreground:  doppel gateway")
        else:
            from hermes_constants import is_container
            if is_container():
                print_info("Start the gateway to bring your bots online:")
                print_info("   doppel gateway run         # Run as container main process")
                print_info("")
                ...
                print_info("Start the gateway to bring your bots online:")
                print_info("   doppel gateway             # Run in foreground")
...
# Config values may have different semantics between OpenClaw and Doppel.
...
def _print_migration_preview(report: dict):
    ...
    if conflict_items:
        print(color("  Would overwrite (conflicts with existing Doppel config):", Colors.YELLOW))
    ...
    if warnings_shown:
        ...
        print()
        print(color("  Note: OpenClaw config values may have different semantics in Doppel.", Colors.YELLOW))
        print(color("  For example, OpenClaw's tool_call_execution: \"auto\" ≠ Doppel's yolo mode.", Colors.YELLOW))
...
def _offer_openclaw_migration(hermes_home: Path) -> bool:
    ...
    print_info("Doppel can preview what would be imported before making any changes.")
    print()

    if not prompt_yes_no("Would you like to see what can be imported?", default=True):
        print_info(
            "Skipping migration. You can run it later with: doppel claw migrate --dry-run"
        )
        return False
    ...
    if not prompt_yes_no("Proceed with migration?", default=False):
        print_info(
            "Migration cancelled. You can run it later with: doppel claw migrate"
        )
        print_info(
    ...
    # Execute the migration — overwrite=False so existing Doppel configs are
    ...
    try:
        migrator = mod.Migrator(
            ...
            execute=True,
            workspace_target=None,
            overwrite=False,  # preserve existing Doppel config
            migrate_secrets=True,
            output_dir=None,
            selected_options=selected,
            preset_name="full",
    ...
    if conflicts:
        print_info(f"Skipped {conflicts} item(s) that already exist in Doppel (use doppel claw migrate --overwrite to force).")
    if skipped:
...
def _run_portal_one_shot(config: dict) -> None:
    ...
    print(color("│     ⚕ Doppel Setup — Nous Portal (one-shot)             │", Colors.MAGENTA))
    print(
        color(
            "└─────────────────────────────────────────────────────────┘",
            Colors.MAGENTA,
    ...
    # runs `doppel setup --portal` after a successful first run).
    ...
    if already_logged_in:
        ...
        try:
            ...
            print_info("  You can retry later with `doppel auth add nous --type oauth`.")
            return
        except (KeyboardInterrupt, EOFError):
            print()
    ...
    print_info("  Run `doppel portal status` to inspect routing.")
    print_info("  Run `doppel` to start chatting.")
...
def run_setup_wizard(args):
    ...
    Supports full, quick, and section-specific setup:
      doppel setup           — full or quick (auto-detected)
      doppel setup model     — just model/provider
      doppel setup tts       — just text-to-speech
      doppel setup terminal  — just terminal backend
      doppel setup gateway   — just messaging platforms
      doppel setup tools     — just tool configuration
      doppel setup agent     — just agent settings
    """
    ...
    if section:
        for key, label, func in SETUP_SECTIONS:
            if key == section:
                ...
                print(color(f"│     ⚕ Doppel Setup — {label:<34s} │", Colors.MAGENTA))
                print(
                    color(
                        "└─────────────────────────────────────────────────────────┘",
                        Colors.MAGENTA,
    ...
    print(
        color(
            "│             ⚕ Doppel Agent Setup Wizard                │", Colors.MAGENTA
        )
    )
    print(
        color(
    ...
    print(
        color(
            "│  Let's configure your Doppel Agent installation.       │", Colors.MAGENTA
        )
    )
    print(
        color(
    ...
    if is_existing:
        ...
        print_success("You already have Doppel configured.")
        print_info("Running the full wizard — each prompt shows your current value.")
        print_info("Press Enter to keep it, or type a new value to change it.")
        print_info("")
        print_info("Tip: jump straight to a section with 'doppel setup model|terminal|")
        ...
        setup_mode = prompt_choice("How would you like to set up Doppel?", [
    ...
    print()
    print_info("You can edit these files directly or use 'doppel config edit'")
...
def _run_first_time_quick_setup(config: dict, hermes_home, is_existing: bool):
    ...
    gateway_choice = prompt_choice(
        ...
        [
            ...
            "Skip — set up later with 'doppel setup gateway'",
        ],
        0,
    )
    ...
    print()
    print_info("  Configure all settings:    doppel setup")
    if gateway_choice != 0:
        print_info("  Connect Telegram/Discord:  doppel setup gateway")
    print()
...
def _run_quick_setup(config: dict, hermes_home):
    ...
    if not has_anything_missing:
        ...
        print()
        print_info("Run 'doppel setup' and choose 'Full Setup' to reconfigure,")
    ...
    if missing_messaging:
        print()
        print_header("Messaging Platforms")
        print_info("Connect Doppel to messaging apps to chat from anywhere.")
        print_info("You can configure these later with 'doppel setup gateway'.")
