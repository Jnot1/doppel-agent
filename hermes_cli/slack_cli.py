"""``doppel slack ...`` CLI subcommands.

Today only ``doppel slack manifest`` is implemented — it generates the
...
Typical workflow::
    ...
    $ doppel slack manifest > slack-manifest.json
    # or:
...
def _build_full_manifest(bot_name: str, bot_description: str) -> dict:
    ...
    it stays in sync with the rest of Doppel. Other manifest sections
    (display info, OAuth scopes, socket mode) are set to sensible defaults
    for a Doppel deployment — users can tweak them in the Slack UI after
    pasting.
    """
...
def slack_manifest_command(args) -> int:
    ...
    Flags (all parsed in ``hermes_cli/main.py``):
      --write [PATH]  Write to file instead of stdout (default path:
                      ``<active home>/slack-manifest.json``)
      --name NAME     Override the bot display name (default: "Doppel")
      --description DESC  Override the bot description
      --slashes-only  Emit only the ``features.slash_commands`` array (for
                      merging into an existing manifest manually)
    """
    name = getattr(args, "name", None) or "Doppel"
    description = getattr(args, "description", None) or "Your Doppel agent on Slack"
    ...
    if write_target is not None:
        ...
        print(
            "\nNext steps:\n"
            "  1. Open https://api.slack.com/apps and pick your Doppel app\n"
