...
on Japanese locales, etc.).  Doppel's banners, tool output feed, and slash
...
def _flip_console_code_page_to_utf8() -> None:
    ...
    is silent — if there's no attached console (e.g. Doppel is running
...
def configure_windows_stdio() -> bool:
    ...
    Set ``DOPPEL_DISABLE_WINDOWS_UTF8=1`` in the environment to opt out
    ...
    disable_flag = (
        os.environ.get("DOPPEL_DISABLE_WINDOWS_UTF8")
        or os.environ.get("HERMES_DISABLE_WINDOWS_UTF8")
    )
    if disable_flag in {"1", "true", "True", "yes"}:
        _CONFIGURED = True
        return False
    ...
    # Augment PATH with the Doppel-managed Git install directories so
    ...
    # already-running shells don't see the change — which means doppel
    ...
    # install without Doppel-managed PortableGit).  Legacy Hermes-managed
    # Windows layouts are still included as hidden fallbacks.
    ...
    # stdin is re-configured for completeness; Doppel's interactive
...
def _default_windows_editor() -> str:
    ...
    The prompt_toolkit buffer's ``open_in_editor`` and Doppel's
    ``doppel config edit`` both honour ``$EDITOR``.  Users who prefer a
    ...
    Set this before launching Doppel (User env var in Windows Settings, or
    export in a PowerShell profile) and Doppel picks it up automatically.
    """
    import shutil
...
def _augment_path_with_known_tools() -> None:
    """Prepend well-known Doppel-managed tool directories to os.environ['PATH'].
    ...
    ``%LOCALAPPDATA%\\doppel\\git\\bin`` to the User PATH via
    ...
    *spawned* processes only — already-running shells (including the one the
    user invokes ``doppel`` from right after install) retain their old PATH.

    Any subprocess Doppel spawns — bash, ``rg``, ``grep``, ``npm`` — inherits
    ...
    Patch-up strategy: add the known Doppel-managed tool directories to our
    PATH at startup so subprocess calls resolve correctly.  No-op on POSIX
    and when the directories don't exist.  Legacy Hermes-managed Windows
    layouts are still recognized as fallbacks.  The User PATH broadcast
    still happens in the background for future shells; this just smooths
    over the first-launch gap.
    """
    if not is_windows():
        return
    ...
    # see on next launch.  Prefer current Doppel paths first, then fall
    # back to hidden legacy Hermes layouts so pre-rebrand installs still
    # work without surfacing the old brand to users.
    candidate_dirs = [
        os.path.join(local_appdata, "doppel", "git", "cmd"),
        os.path.join(local_appdata, "doppel", "git", "bin"),
        os.path.join(local_appdata, "doppel", "git", "usr", "bin"),
        os.path.join(local_appdata, "doppel", "doppel-agent", "venv", "Scripts"),
        os.path.join(local_appdata, "doppel", "hermes-agent", "venv", "Scripts"),
        ...
        os.path.join(local_appdata, "hermes", "doppel-agent", "venv", "Scripts"),
