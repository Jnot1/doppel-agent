...
TIPS = [
    ...
    "/title \"my project\" names your session — resume it later with /resume or doppel -c.",
    ...
    "doppel -w creates an isolated git worktree — perfect for parallel agent workflows.",
    "doppel -w -q \"Fix issue #42\" combines worktree isolation with a one-shot query.",
    "doppel chat -t web,terminal enables only specific toolsets for a focused session.",
    "doppel chat -s github-pr-workflow preloads a skill at launch.",
    "doppel chat -q \"query\" runs a single non-interactive query and exits.",
    "doppel chat --max-turns 200 overrides the default 90-iteration limit per turn.",
    "doppel chat --checkpoints enables filesystem snapshots before every destructive file change.",
    "doppel --yolo bypasses all dangerous command approval prompts for the entire session.",
    "doppel chat --source telegram tags the session for filtering in doppel sessions list.",
    "doppel -p work chat runs under a specific profile without changing your default.",

    # --- CLI Subcommands ---
    "doppel doctor --fix diagnoses and auto-repairs config and dependency issues.",
    "doppel dump outputs a compact setup summary — great for bug reports.",
    "doppel config set KEY VALUE auto-routes secrets to .env and everything else to config.yaml.",
    "doppel config edit opens config.yaml in your default editor.",
    "doppel config check scans for missing or stale configuration options.",
    "doppel sessions browse opens an interactive session picker with search.",
    "doppel sessions stats shows session counts by platform and database size.",
    "doppel sessions prune --older-than 30 cleans up old sessions.",
    "doppel skills search react --source skills-sh searches the skills.sh public directory.",
    "doppel skills check scans installed hub skills for upstream updates.",
    "doppel skills tap add myorg/skills-repo adds a custom GitHub skill source.",
    "doppel skills snapshot export setup.json exports your skill configuration for backup or sharing.",
    "doppel mcp add github --command npx adds MCP servers from the command line.",
    "doppel mcp serve runs Doppel itself as an MCP server for other agents.",
    "doppel auth add lets you add multiple API keys for credential pool rotation.",
    "doppel completion bash >> ~/.bashrc enables tab completion for all commands and profiles.",
    "doppel logs -f follows agent.log in real time. --level WARNING --since 1h filters output.",
    ...
    "Doppel routes vision, compression, and aux tasks to cheaper models — cuts background token cost 85%+ without downgrading your main chat model.",
    ...
    "Set worktree: true in config.yaml to always create a git worktree (same as doppel -w).",
    ...
    "DOPPEL_TIMEZONE overrides the server timezone with any IANA timezone string.",
    ...
    # --- Tools & Capabilities ---
    "execute_code runs Python scripts that call Doppel tools programmatically — results stay out of context.",
    ...
    "Profile names become shell commands — 'doppel profile create coder' creates the 'coder' command.",
    "doppel profile export coder -o backup.tar.gz creates a portable profile archive.",
    ...
    "When exiting, Doppel prints a resume command with session ID and stats.",
    "doppel sessions export backup.jsonl exports all sessions for backup or analysis.",
    "doppel -r SESSION_ID resumes any specific past session by its ID.",
    ...
    "doppel skills install official/security/1password installs optional skills from the repo.",
    ...
    "BlueBubbles brings iMessage to Doppel via a local macOS server.",
    ...
    "SOUL.md at ~/.doppel/SOUL.md is the agent's primary identity — customize it to shape behavior.",
    ...
    # --- MCP ---
    "doppel mcp opens an interactive picker of Nous-approved MCPs you can install in one keystroke.",
    "doppel mcp catalog lists Nous-approved MCP servers shipped with the repo.",
    "doppel mcp install <name> installs a catalog entry, prompts for credentials, and lets you pick which of its tools to enable.",
    ...
    "MCP servers auto-generate toolsets at runtime — doppel tools can toggle them per platform.",
    ...
    "Checkpoints use shadow repos in ~/.doppel/checkpoints/ — your project's .git is never touched.",

    # --- Batch & Data ---
    "batch_runner.py processes hundreds of prompts in parallel for training data generation.",
    "doppel chat -Q enables quiet mode for programmatic use — suppresses banner and spinner.",
    ...
    "doppel plugins install owner/repo installs plugins directly from GitHub.",
    ...
    "doppel acp runs Doppel as an ACP server for VS Code, Zed, and JetBrains integration.",
    ...
    "doppel auth add nous or doppel auth add openai-codex sets up OAuth-based providers.",
    ...
    "doppel status --deep runs deeper diagnostic checks across all components.",
    ...
    "Cron scripts live in ~/.doppel/scripts/ and run before the agent — perfect for data collection pipelines.",
    "prefill_messages_file in config.yaml injects few-shot examples into every API call, never saved to history.",
    "SOUL.md completely replaces the agent's default identity — rewrite it to make Doppel your own.",
    "SOUL.md is auto-seeded with a default personality on first run. Edit ~/.doppel/SOUL.md to customize.",
    ...
    "Before a gateway session reset, Doppel auto-flushes important facts to memory in the background.",
    ...
    "The skills audit log at ~/.doppel/skills/.hub/audit.log tracks every install and removal operation.",
    "Stale git worktrees are auto-cleaned: 24-72h old with no unpushed commits get pruned on startup.",
    "Each profile gets its own subprocess HOME at DOPPEL_HOME/home/ — isolated git, ssh, npm, gh configs.",
    "HERMES_HOME_MODE env var (octal, e.g. 0701) sets custom directory permissions for web server traversal.",
    "Container mode: place .container-mode in DOPPEL_HOME and the host CLI auto-execs into the container.",
    "Ctrl+C has 5 priority tiers: cancel recording → cancel prompts → cancel picker → interrupt agent → exit.",
    "Every interrupt during an agent run is logged to ~/.doppel/interrupt_debug.log with timestamps.",
    ...
    "Profile names cannot collide with existing PATH binaries — 'doppel profile create ls' would be rejected.",
    "doppel profile create backup --clone-all copies everything (config, keys, SOUL.md, memories, skills, sessions).",
    ...
    "Each MCP server gets its own toolset (mcp-servername) that can be toggled independently via doppel tools.",
    "MCP ${ENV_VAR} placeholders in config are resolved at server spawn — including vars from ~/.doppel/.env.",
    "Skills from trusted repos (NousResearch) get a 'trusted' security level; community skills get extra scanning.",
    "The skills quarantine at ~/.doppel/skills/.hub/quarantine/ holds skills pending security review.",

    # --- Advanced Slash Commands ---
    '/steer <prompt> injects a note after the next tool call — nudge direction mid-task without interrupting.',
    '/goal <text> sets a standing Ralph-loop objective — Doppel auto-continues turn after turn until a judge says done.',
    '/snapshot create [label] saves a full state snapshot of Doppel config; /snapshot restore <id> reverts later.',
    ...
    '/busy queue|steer|interrupt controls what pressing Enter does while Doppel is working.',
    ...
    '/reload reloads ~/.doppel/.env into the running session — pick up new API keys without restarting.',
    ...
    # --- Gateway Hooks ---
    'Gateway hooks live under ~/.doppel/hooks/<name>/ with HOOK.yaml + handler.py — handler must be named `handle`.',
    'Hook events include gateway:startup, session:start, agent:step, and command:* wildcard subscriptions.',
    'Drop a ~/.doppel/BOOT.md checklist and a gateway:startup hook runs it as a one-shot agent every boot.',

    # --- Curator ---
    'doppel curator run --dry-run previews what the curator would archive or consolidate without mutating anything.',
    "doppel curator pin <skill> hard-fences a skill against both auto-archival and the agent's skill_manage tool.",
    'doppel curator rollback restores skills from a pre-run snapshot — backups live under skills/.curator_backups/.',

    # --- Credential Pools & Routing ---
    'doppel auth reset <provider> clears all cooldowns and exhaustion flags on a credential pool.',
    ...
    'doppel dashboard launches a local web UI at 127.0.0.1:9119 — zero data leaves localhost.',
    'doppel dashboard --tui embeds the full Doppel TUI in your browser via xterm.js and a WebSocket PTY.',
    'Drop a YAML in ~/.doppel/dashboard-themes/ with two palette colors to reskin the entire dashboard.',
    'Dashboard plugins are drop-in: manifest.json + JS bundle in ~/.doppel/dashboard-plugins/ — no npm build required.',
    ...
    # --- TTS ---
    'tts.provider: piper runs 44-language local TTS on CPU — voices auto-download to ~/.doppel/cache/piper-voices/.',
    ...
    '/reload-skills re-scans ~/.doppel/skills/ so drop-in skills appear without restarting the session.',
    ...
    'doppel -z "<prompt>" is the purest one-shot: final answer on stdout, nothing else — ideal for piping in scripts.',
    'doppel chat --pass-session-id injects the session ID into the system prompt so the agent can self-reference it.',
    'doppel chat --image path/to/pic.png attaches a local image to a single -q query without a separate upload step.',
    'doppel chat --ignore-user-config skips ~/.doppel/config.yaml — reproducible bug reports and CI runs.',
    "doppel chat --source tool tags programmatic chats so they don't clutter doppel sessions list.",
    'doppel dump --show-keys includes redacted API key fingerprints for deeper support debugging.',
    'doppel sessions rename <ID> "new title" renames any past session; doppel sessions delete <ID> removes one.',
    'doppel import restores a session export or profile archive produced by sessions export or profile export.',
    'doppel fallback manages the fallback_model chain interactively — no hand-editing config.yaml.',
    'doppel pairing rotates the DM pairing token — the first messager after rotation claims access to the bot.',
    ...
    'DOPPEL_AGENT_TIMEOUT=0 disables the gateway inactivity kill for a running agent — use for long research runs.',
    'DOPPEL_ENABLE_PROJECT_PLUGINS=1 auto-loads repo-local plugins from ./.doppel/plugins/ — trust-gated by design.',
    ...
    'DOPPEL_RESTART_DRAIN_TIMEOUT (default 900s) caps how long /restart waits for in-flight runs before forcing.',
    'DOPPEL_CHECKPOINT_TIMEOUT (default 30s) caps filesystem checkpoint creation — raise it on huge monorepos.',
    ...
    # --- Security ---
    'security.tirith_fail_open: false makes Doppel block commands when the tirith scanner itself errors out.',
    ...
    '--source tool chats are excluded from doppel sessions list by default — set --source explicitly to see them.',
    ...
    'Dashboard plugins are served from /dashboard-plugins/<name>/ — drop files into ~/.doppel/dashboard-plugins/.',
]
