"""doppel webhook — manage dynamic webhook subscriptions from the CLI.
...
Subscriptions persist to your agent home directory and are hot-reloaded by the
webhook adapter without a gateway restart.
"""
...
def _setup_hint() -> str:
  ...
  Then start the gateway: doppel gateway run
"""
...
def webhook_command(args):
    """Entry point for 'doppel webhook' subcommand."""
    sub = getattr(args, "webhook_action", None)

    if not sub:
        print("Usage: doppel webhook {subscribe|list|remove|test}")
        print("Run 'doppel webhook --help' for details.")
        return
...
def _cmd_subscribe(args):
    ...
    print(f"  The gateway must be running to receive events (doppel gateway run).\n")
...
def _cmd_list(args):
    ...
    if not subs:
        ...
        print("  Create one with: doppel webhook subscribe <name>")
        return
...
def _cmd_test(args):
    ...
    payload = args.payload or '{"test": true, "event_type": "test", "message": "Hello from doppel webhook test"}'
    ...
    try:
        ...
        print(f"  Error: {e}")
        print("  Is the gateway running? (doppel gateway run)")
