...
Let the Doppel agent join a Google Meet call, transcribe it, optionally speak
...
```
┌─ gateway (Linux box, where Doppel runs) ────────────────────────────┐
│                                                                      │
...
│                                                                      │
│   NodeServer (from `doppel meet node run`)                           │
│     │                                                                │
...
| `__init__.py` | `register(ctx)` — registers 5 tools + `on_session_end` hook + `doppel meet` CLI |
...
| `cli.py` | `doppel meet setup / auth / join / status / transcript / say / stop / node ...` |
...
| `node/registry.py` | v3: `$DOPPEL_HOME/workspace/meetings/nodes.json` |
| `node/server.py` | v3: `NodeServer` (runs on host machine) |
| `node/client.py` | v3: `NodeClient` (used by tool handlers + CLI on gateway) |
| `node/cli.py` | v3: `doppel meet node {run,list,approve,remove,status,ping}` |
...
doppel meet install                                      # pip + Chromium
doppel meet setup                                        # preflight
doppel meet auth                                         # optional
doppel meet join https://meet.google.com/abc-defg-hij    # transcribe
```
...
```bash
doppel meet install --realtime                     # installs pulseaudio-utils
echo 'OPENAI_API_KEY=sk-...' >> ~/.doppel/.env
doppel meet join https://meet.google.com/abc-defg-hij --mode realtime
# then from the agent or CLI:
doppel meet say "Good morning everyone, I'm the note-taker bot."
```

macOS:
```bash
doppel meet install --realtime     # runs: brew install blackhole-2ch ffmpeg
# then — manually! — open System Settings → Sound → Input → BlackHole 2ch
echo 'OPENAI_API_KEY=sk-...' >> ~/.doppel/.env
doppel meet join https://meet.google.com/abc-defg-hij --mode realtime
```

On macOS, doppel will **not** switch your system audio input automatically — the
...
doppel plugins enable google_meet
doppel meet node run --display-name my-mac --host 0.0.0.0 --port 18789
