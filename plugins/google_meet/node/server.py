...
Launched by ``doppel meet node run``.

Token handling
--------------
On first boot we mint 32 hex chars of entropy and persist them at
``$DOPPEL_HOME/workspace/meetings/node_token.json``. Subsequent boots
...
via ``doppel meet node approve <name> <url> <token>``.
...
class NodeServer:
    ...
    def __init__(
        ...
        display_name: str = "doppel-meet-node",
    ) -> None:
        self.host = host
        self.port = port
