"""
IRC Platform Adapter for Doppel Agent.

A plugin-based gateway adapter that connects to an IRC server and relays
messages to/from the Doppel agent.  Zero external dependencies — uses
...
class IRCAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config, **kwargs):
        ...
        self.nickname = os.getenv("IRC_NICKNAME") or extra.get("nickname", "doppel-bot")
    ...
    async def connect(self) -> bool:
        ...
        await self._send_raw(f"USER {self.nickname} 0 * :Doppel Agent")
    ...
    async def disconnect(self) -> None:
        ...
        if self._writer and not self._writer.is_closing():
            try:
                await self._send_raw("QUIT :Doppel Agent shutting down")
...
def interactive_setup() -> None:
    """Interactive `doppel gateway setup` flow for the IRC platform.
    ...
    print_info("Connect Doppel to an IRC network. Uses Python stdlib — no extra packages needed.")
    ...
    print()
    print_success("IRC configuration saved to ~/.doppel/.env")
    print_info("Restart the gateway for changes to take effect: doppel gateway restart")
...
async def _standalone_send(
    ...
    *,
    ...
    nickname = os.getenv("IRC_NICKNAME") or extra.get("nickname", "doppel-bot")
    ...
    nick_base = nickname.rstrip("_0123456789-")[:24] or "doppel-bot"
    ...
    try:
        ...
        await _raw(f"USER {standalone_nick} 0 * :Doppel Agent (cron)")
