"""
LINE Messaging API platform adapter for Doppel Agent.
...
to Doppel Agent. It deliberately ports the *strongest* idea from each into
...
def is_connected(config) -> bool:
    """Surface in ``doppel status`` even before the adapter is instantiated."""
...
def _env_enablement() -> Optional[Dict[str, Any]]:
    ...
    Lets ``doppel status`` reflect a LINE configuration that lives entirely
...
def interactive_setup() -> None:
    """Minimal stdin wizard for ``doppel setup line``.

    Mirrors the irc/teams style: prompts for the two required vars, plus
    one optional public URL. Writes to ``~/.doppel/.env`` via ``hermes_cli.config``.
    """
    print()
    ...
    try:
        ...
        print("hermes_cli.config not available; set LINE_* vars manually in ~/.doppel/.env")
        return
