"""SimpleX Chat platform adapter (Doppel plugin).
...
``plugins/platforms/simplex/``. The Doppel plugin loader scans the
...
discoverable and `doppel setup` can describe it even when websockets is
...
# Imported at module top because they're stdlib-only inside Doppel — no
...
def interactive_setup() -> None:
    ...
    try:
        ...
        print("hermes_cli.config not available; set SIMPLEX_* vars manually in ~/.doppel/.env")
        return
...
def register(ctx) -> None:
    ...
    ctx.register_platform(
        ...
        # env-only setups show up in `doppel gateway status` without
