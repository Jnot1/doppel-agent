...
AUTHOR_MAP = {
    "macshelton@MacSheltons-Mac-mini.local": "Jnot1",
...
def _update_acp_registry_versions(semver: str) -> None:
    ...
    if ACP_REGISTRY_MANIFEST.exists():
        ...
        if "package" in uvx:
            uvx["package"] = f"doppel-agent[acp]=={semver}"
...
def main():
    ...
    if args.publish:
        ...
        if result and result.returncode == 0:
            ...
            print(
                f"    gh release create {tag_name} --title 'Doppel Agent v{new_version} ({calver_date})' "
