...
PREFERRED_AUTH_HOME = Path.home() / ".doppel"
LEGACY_AUTH_HOME = Path.home() / ".hermes"
ORIGINAL_AUTH_HOME = PREFERRED_AUTH_HOME if PREFERRED_AUTH_HOME.exists() else LEGACY_AUTH_HOME
ORIGINAL_AUTH = ORIGINAL_AUTH_HOME / "auth.json"
...
def setup_isolated_home(enabled: bool) -> Path:
    ...
    doppel_home.mkdir(parents=True)

    if ORIGINAL_AUTH.exists():
        shutil.copy(ORIGINAL_AUTH, doppel_home / "auth.json")

    # Copy .env so OPENROUTER_API_KEY (or others) are visible to the agent
    # running inside the isolated home.
    real_env_file = ORIGINAL_AUTH_HOME / ".env"
    if real_env_file.exists():
        shutil.copy(real_env_file, doppel_home / ".env")
        ...
        load_hermes_dotenv(hermes_home=str(ORIGINAL_AUTH_HOME))
    ...
    (doppel_home / "config.yaml").write_text(_yaml_dump(cfg), encoding="utf-8")
    return doppel_home
