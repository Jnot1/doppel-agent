"""Tests for ACP Registry metadata shipped with Doppel."""
...
def test_agent_json_matches_official_registry_required_fields():
    ...
    assert data["id"] == "doppel-agent"
    assert re.fullmatch(r"[a-z][a-z0-9-]*", data["id"])
    assert data["name"] == "Doppel Agent"
    assert data["description"]
    assert data["repository"] == "https://github.com/Jnot1/doppel-agent"
    assert data["website"] == "https://github.com/Jnot1/doppel-agent/blob/main/website/docs/user-guide/features/acp.md"
...
def test_agent_json_uses_uvx_distribution_without_local_command_fields():
    ...
    assert uvx["package"] == f"doppel-agent[acp]=={data['version']}"
    assert uvx["args"] == ["doppel-acp"]
...
def test_agent_json_pins_uvx_package_to_pyproject_version():
    ...
    assert _manifest()["distribution"]["uvx"]["package"] == (
        f"doppel-agent[acp]=={_pyproject_version()}"
    )
