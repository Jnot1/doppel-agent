...
class TestLocalStreamReadTimeout:
    ...
    def test_local_endpoint_bumps_read_timeout(self, base_url):
        ...
        with patch.dict(os.environ, {}, clear=False):
            os.environ.pop("DOPPEL_STREAM_READ_TIMEOUT", None)
            os.environ.pop("HERMES_STREAM_READ_TIMEOUT", None)
            _base_timeout = float(os.getenv("HERMES_API_TIMEOUT", 1800.0))
            _stream_read_timeout_raw = (
                os.getenv("DOPPEL_STREAM_READ_TIMEOUT")
                or os.getenv("HERMES_STREAM_READ_TIMEOUT")
            )
    ...
    def test_user_override_respected_for_local(self):
        """Legacy stream timeout override still wins for local providers."""
    ...
    def test_preferred_doppel_override_respected_for_local(self):
        """Preferred Doppel timeout override wins for local providers."""
        with patch.dict(os.environ, {"DOPPEL_STREAM_READ_TIMEOUT": "420"}, clear=False):
            _base_timeout = float(os.getenv("HERMES_API_TIMEOUT", 1800.0))
            _stream_read_timeout_raw = (
                os.getenv("DOPPEL_STREAM_READ_TIMEOUT")
                or os.getenv("HERMES_STREAM_READ_TIMEOUT")
            ...
            if _stream_read_timeout == 120.0 and base_url and is_local_endpoint(base_url):
                _stream_read_timeout = _base_timeout
            assert _stream_read_timeout == 420.0
