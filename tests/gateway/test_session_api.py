...
async def test_session_crud_and_message_history(adapter, session_db):
    ...
    async with TestClient(TestServer(app)) as cli:
        ...
        assert created["object"] == API_SERVER_SESSION_OBJECT
        ...
        assert deleted == {"object": API_SERVER_SESSION_DELETED_OBJECT, "id": session_id, "deleted": True}
...
async def test_session_fork_uses_current_sessiondb_branch_primitives(adapter, session_db):
    ...
    assert payload["object"] == API_SERVER_SESSION_OBJECT
    ...
    with patch.object(auth_adapter, "_run_agent", mock_run):
        async with TestClient(TestServer(app)) as cli:
            resp = await cli.post(
                ...
                headers={"Authorization": "Bearer sk-test", API_SERVER_SESSION_KEY_HEADER: "client-42"},
            )
            assert resp.status == 200
            payload = await resp.json()

    assert resp.headers[API_SERVER_SESSION_ID_HEADER] == session_id
    assert resp.headers[API_SERVER_SESSION_KEY_HEADER] == "client-42"
...
async def test_session_header_rejected_without_api_key(adapter, session_db):
    ...
    async with TestClient(TestServer(app)) as cli:
        resp = await cli.post(
            f"/api/sessions/{session_id}/chat",
            json={"message": "hello"},
            headers={API_SERVER_SESSION_KEY_HEADER: "client-42"},
        )
        assert resp.status == 403
        data = await resp.json()
        assert f"{API_SERVER_SESSION_KEY_HEADER} requires API key" in data["error"]["message"]
