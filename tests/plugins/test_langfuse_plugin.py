...
class TestManifest:
    ...
    def test_manifest_fields(self):
        ...
        # Required env vars are the user-facing DOPPEL_ prefixed keys.
        assert "DOPPEL_LANGFUSE_PUBLIC_KEY" in data["requires_env"]
        assert "DOPPEL_LANGFUSE_SECRET_KEY" in data["requires_env"]
...
class TestRuntimeGate:
    ...
    def test_get_langfuse_returns_none_without_credentials(self, monkeypatch):
        for k in (
            "DOPPEL_LANGFUSE_PUBLIC_KEY", "DOPPEL_LANGFUSE_SECRET_KEY",
    ...
    def test_get_langfuse_caches_failure_no_config_load(self, monkeypatch):
        ...
        def tracking_get(key, default=None):
            if key.startswith(("DOPPEL_LANGFUSE_", "HERMES_LANGFUSE_", "LANGFUSE_")):
                called["n"] += 1
...
# DOPPEL_LANGFUSE_PUBLIC_KEY / SECRET_KEY at a template value like
...
# Doppel logs.  The fix in `_get_langfuse()` validates the documented
...
class TestPlaceholderKeyDetection:
    ...
    def test_validate_langfuse_key_accepts_documented_prefix(self, monkeypatch):
        ...
        assert plugin._validate_langfuse_key(
            "DOPPEL_LANGFUSE_PUBLIC_KEY", "pk-lf-real-public-xyz"
        ) is None
        assert plugin._validate_langfuse_key(
            "DOPPEL_LANGFUSE_SECRET_KEY", "sk-lf-real-secret-xyz"
        ) is None
    ...
    def test_validate_langfuse_key_rejects_wrong_prefix(self, monkeypatch):
        ...
        msg = plugin._validate_langfuse_key(
            "DOPPEL_LANGFUSE_PUBLIC_KEY", "placeholder"
        )
        assert msg is not None
        assert "DOPPEL_LANGFUSE_PUBLIC_KEY" in msg
        assert "pk-lf-" in msg
    ...
    def test_validate_langfuse_key_unknown_name_passes(self, monkeypatch):
        ...
        self._clear_env(monkeypatch)
        plugin = self._fresh_plugin()
        assert plugin._validate_langfuse_key("DOPPEL_LANGFUSE_BASE_URL", "anything") is None
    ...
    def test_placeholder_public_key_warns_and_skips(self, monkeypatch, caplog):
        self._clear_env(monkeypatch)
        monkeypatch.setenv("DOPPEL_LANGFUSE_PUBLIC_KEY", "placeholder")
        monkeypatch.setenv("DOPPEL_LANGFUSE_SECRET_KEY", "sk-lf-real-secret-xyz")
        plugin = self._fresh_plugin(monkeypatch)
        with caplog.at_level(logging.WARNING, logger=self.LOGGER_NAME):
            assert plugin._get_langfuse() is None
        text = caplog.text
        assert "DOPPEL_LANGFUSE_PUBLIC_KEY" in text
    ...
    def test_placeholder_secret_key_warns_and_skips(self, monkeypatch, caplog):
        self._clear_env(monkeypatch)
        monkeypatch.setenv("DOPPEL_LANGFUSE_PUBLIC_KEY", "pk-lf-real-public-xyz")
        monkeypatch.setenv("DOPPEL_LANGFUSE_SECRET_KEY", "test-key")
        plugin = self._fresh_plugin(monkeypatch)
        with caplog.at_level(logging.WARNING, logger=self.LOGGER_NAME):
            assert plugin._get_langfuse() is None
        text = caplog.text
        assert "DOPPEL_LANGFUSE_SECRET_KEY" in text
    ...
    def test_both_placeholders_one_warning_with_both_keys(self, monkeypatch, caplog):
        self._clear_env(monkeypatch)
        monkeypatch.setenv("DOPPEL_LANGFUSE_PUBLIC_KEY", "placeholder")
        monkeypatch.setenv("DOPPEL_LANGFUSE_SECRET_KEY", "placeholder")
        plugin = self._fresh_plugin(monkeypatch)
        with caplog.at_level(logging.WARNING, logger=self.LOGGER_NAME):
            assert plugin._get_langfuse() is None
    ...
    def test_common_placeholders_detected(self, monkeypatch, caplog, placeholder):
        ...
        with caplog.at_level(logging.WARNING, logger=self.LOGGER_NAME):
            assert plugin._get_langfuse() is None
        assert "DOPPEL_LANGFUSE_PUBLIC_KEY" in caplog.text

    def test_legacy_LANGFUSE_PUBLIC_KEY_also_validated(self, monkeypatch, caplog):
        """The plugin reads both the canonical DOPPEL_-prefixed env var and
        ...
        # DOPPEL_-prefixed one — operators set the DOPPEL_-prefixed one).
        assert "DOPPEL_LANGFUSE_PUBLIC_KEY" in caplog.text
...
class _FakePropagateAttributes:
    def __init__(self, **kwargs):
        self.kwargs = kwargs
...
class _FakeTraceClient:
    ...
    def start_as_current_observation(self, **kwargs):
        ...
        return _FakeObservationContext(kwargs)


class TestDoppelAliases:
    def _fresh_plugin(self, monkeypatch=None):
        mod_name = "plugins.observability.langfuse"
        sys.modules.pop(mod_name, None)
        mod = importlib.import_module(mod_name)
    ...
    def test_doppel_prefixed_envs_override_legacy_hermes_values(self, monkeypatch):
        self._clear_env(monkeypatch)
        monkeypatch.setenv("DOPPEL_LANGFUSE_PUBLIC_KEY", "pk-lf-doppel-public-xyz")
        monkeypatch.setenv("DOPPEL_LANGFUSE_SECRET_KEY", "sk-lf-doppel-secret-xyz")
        monkeypatch.setenv("DOPPEL_LANGFUSE_BASE_URL", "https://doppel.example")
        monkeypatch.setenv("DOPPEL_LANGFUSE_ENV", "production")
        monkeypatch.setenv("DOPPEL_LANGFUSE_RELEASE", "v2.0.0")
        monkeypatch.setenv("DOPPEL_LANGFUSE_SAMPLE_RATE", "0.25")
        monkeypatch.setenv("HERMES_LANGFUSE_PUBLIC_KEY", "pk-lf-hermes-public-xyz")
        monkeypatch.setenv("HERMES_LANGFUSE_SECRET_KEY", "sk-lf-hermes-secret-xyz")
        monkeypatch.setenv("HERMES_LANGFUSE_BASE_URL", "https://hermes.example")
        monkeypatch.setenv("HERMES_LANGFUSE_ENV", "staging")
        monkeypatch.setenv("HERMES_LANGFUSE_RELEASE", "v1.0.0")
        monkeypatch.setenv("HERMES_LANGFUSE_SAMPLE_RATE", "0.75")

        plugin = self._fresh_plugin(monkeypatch)
        client = plugin._get_langfuse()

        assert isinstance(client, _FakeLangfuse)
        assert client.kwargs["public_key"] == "pk-lf-doppel-public-xyz"
        assert client.kwargs["secret_key"] == "sk-lf-doppel-secret-xyz"
        assert client.kwargs["base_url"] == "https://doppel.example"
        assert client.kwargs["environment"] == "production"
        assert client.kwargs["release"] == "v2.0.0"
        assert client.kwargs["sample_rate"] == 0.25

    def test_root_trace_uses_doppel_turn_contract(self, monkeypatch):
        self._clear_env(monkeypatch)
        plugin = self._fresh_plugin()
        trace_ctx = {}

        def fake_propagate_attributes(**kwargs):
            trace_ctx.update(kwargs)
            return _FakePropagateAttributes(**kwargs)

        monkeypatch.setattr(plugin, "propagate_attributes", fake_propagate_attributes)
        ...
        assert client.start_kwargs["name"] == "Doppel turn"
        assert client.start_kwargs["metadata"]["source"] == "doppel"
        assert trace_ctx["trace_name"] == "Doppel turn"
        assert trace_ctx["tags"] == ["doppel", "langfuse"]
        assert state.root_span.trace_io_calls[0]["input"] == "hello"
