...
def test_daily_briefing_bot_guides_prefer_doppel_customer_facing_surfaces():
    en = (
        REPO_ROOT / "website" / "docs" / "guides" / "daily-briefing-bot.md"
    ).read_text(encoding="utf-8")
    zh = (
        REPO_ROOT
        / "website"
    ...
    assert "`doppel setup --portal`" in en
    assert "`doppel setup --portal`" in zh
    assert "**Doppel spins up**" in en
    assert "**Doppel 启动**" in zh
    assert "**Doppel Agent installed**" in en
    assert "**已安装 Doppel Agent**" in zh
    assert "doppel gateway install" in en
    assert "doppel gateway install" in zh
    assert "sudo doppel gateway install --system" in en
    assert "sudo doppel gateway install --system" in zh
    assert "doppel gateway" in en
    assert "doppel gateway" in zh
    assert '`~/.doppel/cron/output/`' in en
    assert '`~/.doppel/cron/output/`' in zh
    assert "\n```bash\ndoppel\n```" in en
    assert "\n```bash\ndoppel\n```" in zh
    assert "Doppel will search the web" in en
    assert "Doppel 将搜索网页" in zh
    assert "Generated by Doppel Agent" in en
    assert "Generated by Doppel Agent" in zh
    assert "ensure Doppel has a default model and provider configured globally" in en
    assert "请确保 Doppel 已全局配置了默认模型和 provider" in zh
    assert "Just tell Doppel what you want" in en
    assert "直接告诉 Doppel 你想要什么" in zh
    assert "Doppel will create the cron job" in en
    assert "Doppel 将使用统一的 `cronjob` 工具为你创建 cron 任务" in zh
    assert "tell Doppel to delegate each topic" in en
    assert "告诉 Doppel 将每个主题委托给子 agent" in zh
    assert "doppel cron list" in en
    assert "doppel cron list" in zh
    assert "Doppel will use `cronjob(action=\"list\")`" in en
    assert "Doppel 将使用 `cronjob(action=\"list\")`" in zh
    assert "doppel cron status" in en
    assert "doppel cron status" in zh
    ...
    for stale in (
        ...
        "`~/.hermes/cron/output/`",
        "\n```bash\nhermes\n```",
        "Hermes will search the web",
        "Hermes 将搜索网页",
        "Generated by Hermes Agent",
        "ensure Hermes has a default model and provider configured globally",
        "请确保 Hermes 已全局配置了默认模型和 provider",
        "Just tell Hermes what you want",
        "直接告诉 Hermes 你想要什么",
        "Hermes will create the cron job",
        "Hermes 将使用统一的 `cronjob` 工具为你创建 cron 任务",
        "tell Hermes to delegate each topic",
        "告诉 Hermes 将每个主题委托给子 agent",
        "hermes cron list",
        "Hermes will use `cronjob(action=\"list\")`",
        "Hermes 将使用 `cronjob(action=\"list\")`",
        "hermes cron status",
    ):
        assert stale not in en
        assert stale not in zh
