...
provider** — turn on a backend by enabling a plugin (``doppel plugins
enable video_gen/<name>``) and selecting it in ``doppel tools`` → Video
Generation.
        ...
        "properties": {
            ...
            "model": {
                ...
                "description": (
                    ...
                    "configured ``video_gen.model`` (set via `doppel tools` "
...
def _missing_provider_error(configured: Optional[str]) -> str:
    if configured:
        msg = (
            ...
            f"registered that name. Run `doppel plugins list` to see "
            f"installed video gen backends, or `doppel tools` → Video "
    ...
    msg = (
        "No video generation backend is configured. Run `doppel tools` → "
...
# `doppel tools` or `/skills`, the schema rebuilds automatically.
...
_GENERIC_DESCRIPTION = (
    ...
    "`doppel tools` → Video Generation; the agent does not pick them. "
...
def _build_dynamic_video_schema() -> Dict[str, Any]:
    ...
    if not configured:
        parts.append(
            ...
            "until the user picks one via `doppel tools` → Video Generation."
        )
        return {"description": "\n".join(parts)}
