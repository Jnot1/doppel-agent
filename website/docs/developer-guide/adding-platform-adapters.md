...
This guide covers adding a new messaging platform to the Doppel gateway. A platform adapter connects Doppel Agent to an external messaging service (Telegram, Discord, WeCom, etc.) so users can interact with the agent through that service.

:::tip
There are two ways to add a platform:
- **Plugin** (recommended for community/third-party): Drop a plugin directory into `~/.doppel/plugins/` — zero core code changes needed. See [Plugin Path](#plugin-path-recommended) below.
...
The plugin system lets you add a platform adapter without modifying any core Doppel Agent code. Your plugin is a directory with two files:
...
Plugin metadata. The `requires_env` and `optional_env` blocks auto-populate `doppel config` UI entries (see [Surfacing Env Vars](#surfacing-env-vars-in-hermes-config) below).
...
def register(ctx):
    """Plugin entry point — called by the Doppel plugin system."""
...
| `doppel config` UI entries | `requires_env` / `optional_env` in `plugin.yaml` auto-populate |
...
| `doppel status` | Shows plugin platforms with `(plugin)` tag |
| `doppel gateway setup` | Plugin platforms appear in setup menu |
| `doppel tools` / `doppel skills` | Plugin platforms in per-platform config |
...
Most users set up a platform by dropping env vars into `~/.doppel/.env` rather than editing `config.yaml`. The `env_enablement_fn` hook lets your plugin pick those env vars up **before** the adapter is constructed, so `doppel gateway status`, `get_connected_platforms()`, and cron delivery see the correct state without instantiating the platform SDK.
...
`cron_deliver_env_var` makes your platform a recognized `deliver=` target. To make the actual send succeed when the cron job runs in a separate process from the gateway (i.e., `doppel cron run` separate from `doppel gateway`), register a `standalone_sender_fn`:
...
## Surfacing Env Vars in `doppel config`
...
version: 1.0.0
description: >
  My Platform gateway adapter for Doppel Agent.
author: Your Name
requires_env:
  ...
  - name: MY_PLATFORM_CHANNEL
    description: "Channel to join (e.g. #doppel)"
    prompt: "Channel"
    password: false
optional_env:
  - name: MY_PLATFORM_HOME_CHANNEL
...
:::note
This checklist is for adding a platform directly to the Doppel Agent core codebase — typically done by core contributors for officially supported platforms. Community/third-party platforms should use the [Plugin Path](#plugin-path-recommended) above.
:::
