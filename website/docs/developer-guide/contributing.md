...
sidebar_position: 4
title: "Contributing"
description: "How to contribute to Doppel Agent — dev setup, code style, PR process"
---

# Contributing

Thank you for contributing to Doppel Agent! This guide covers setting up your dev environment, understanding the codebase, and getting your PR merged.
...
- Building a custom/local tool without modifying Doppel core? Start with [Build a Doppel Plugin](../guides/build-a-hermes-plugin.md)
- Building a new built-in core tool for Doppel Agent itself? Start with [Adding Tools](./adding-tools.md)
...
```bash
git clone --recurse-submodules https://github.com/Jnot1/doppel-agent.git
cd doppel-agent
...
```bash
mkdir -p ~/.doppel/{cron,sessions,logs,memories,skills}
cp cli-config.yaml.example ~/.doppel/config.yaml
touch ~/.doppel/.env

# Add at minimum an LLM provider key:
echo 'OPENROUTER_API_KEY=sk-or-v1-your-key' >> ~/.doppel/.env
```
...
mkdir -p ~/.local/bin
ln -sf "$(pwd)/venv/bin/doppel" ~/.local/bin/doppel
...
- **Profile-safe paths**: Never hardcode `~/.doppel` — use `get_hermes_home()` from `hermes_constants` for code paths and `display_hermes_home()` for user-facing messages. See [AGENTS.md](https://github.com/Jnot1/doppel-agent/blob/main/AGENTS.md#profiles-multi-instance-support) for full rules.

## Cross-Platform Compatibility

Doppel Agent officially supports **Linux, macOS, WSL2, and native Windows (early beta — via PowerShell install)**. Native Windows uses Git Bash (from [Git for Windows](https://git-scm.com/download/win)) for shell commands. A few features require POSIX kernel primitives and are gated: the dashboard's embedded PTY terminal pane (`/chat` tab) is WSL2-only. The native-Windows path is new and moves fast — if you're doing Windows-heavy dev, expect to hit and fix rough edges.
...
Doppel Agent has terminal access. Security matters.
...
2. **Test manually**: Run `doppel` and exercise the code path you changed
...
- Use [GitHub Issues](https://github.com/Jnot1/doppel-agent/issues)
- Include: OS, Python version, Doppel Agent version (`doppel version`), full error traceback
...
- **Discord (upstream community)**: [discord.gg/NousResearch](https://discord.gg/NousResearch)
- **GitHub Discussions**: For design proposals and architecture discussions
- **Skills Hub**: Upload specialized skills and share with the community

## License

By contributing, you agree that your contributions will be licensed under the [MIT License](https://github.com/Jnot1/doppel-agent/blob/main/LICENSE).
