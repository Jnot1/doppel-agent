...
Jobs are stored in `~/.doppel/cron/jobs.json` with atomic write semantics (write to temp file, then rename). Each job record contains:
...
In CLI mode, cron jobs only fire when `doppel cron` commands are run or during active CLI sessions.
...
2. **Environment variable** — `DOPPEL_CRON_SCRIPT_TIMEOUT`
...
| Local file | `local` | Save to `~/.doppel/cron/output/` |
...
The scheduler uses cross-process file-based locking (`fcntl.flock` on Unix, `msvcrt.locking` on Windows) to prevent overlapping ticks from executing the same due-job batch twice — even between the gateway's in-process ticker and a standalone `doppel cron` / manual `tick()` call. If the lock cannot be acquired, `tick()` returns 0 immediately.
...
```bash
doppel cron list                    # Show all jobs
doppel cron create                  # Interactive job creation (alias: add)
doppel cron edit <job_id>           # Edit job configuration
doppel cron pause <job_id>          # Pause a running job
doppel cron resume <job_id>         # Resume a paused job
doppel cron run <job_id>            # Trigger immediate execution
doppel cron remove <job_id>         # Delete a job
```
