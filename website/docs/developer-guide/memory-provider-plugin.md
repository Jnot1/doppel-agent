...
description: "How to build a memory provider plugin for Doppel Agent"
---

# Building a Memory Provider Plugin

Memory provider plugins give Doppel Agent persistent, cross-session knowledge beyond the built-in MEMORY.md and USER.md. This guide covers how to build one.

:::tip
Memory providers are one of two **provider plugin** types. The other is [Context Engine Plugins](/developer-guide/context-engine-plugin), which replace the built-in context compressor. Both follow the same pattern: single-select, config-driven, managed via `doppel plugins`.
:::
...
|--------|---------|-----------------|
| `get_config_schema()` | Declare config fields for `doppel memory setup` | **Yes** |
...
`get_config_schema()` returns a list of field descriptors used by `doppel memory setup`:
...
Memory provider plugins can register their own CLI subcommand tree (e.g. `doppel my-provider status`, `doppel my-provider config`). This uses a convention-based discovery system — no changes to core files needed.
...
4. Your commands appear under `doppel <provider-name> <subcommand>`

**Active-provider gating:** Your CLI commands only appear when your provider is the active `memory.provider` in config. If a user hasn't configured your provider, your commands won't show in `doppel --help`.
...
def my_command(args):
    ...
    if sub == "status":
        ...
        print("Usage: doppel my-provider <status|config>")

def register_cli(subparser) -> None:
    """Build the doppel my-provider argparse tree.
