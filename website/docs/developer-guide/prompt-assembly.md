...
2. **context** — caller-supplied `system_message` plus project context files (`.doppel.md` / `AGENTS.md` / `CLAUDE.md` / `.cursorrules`)
...
`SOUL.md` lives at `~/.doppel/SOUL.md` by default. It serves as the agent's identity — the very first section of the system prompt. The loading logic in `prompt_builder.py` works as follows:
...
|----------|-------|-------------|-------|
| 1 | `.doppel.md`, `DOPPEL.md` | CWD up to git root | Doppel-native project config |
...
- **YAML frontmatter stripped** — `.doppel.md` frontmatter is removed (reserved for future config overrides)
...
1. `.doppel.md` / `DOPPEL.md` (walks to git root)
...
- Project context files such as `.doppel.md`, `DOPPEL.md`, `AGENTS.md`, `CLAUDE.md`, or `.cursorrules` — inject repo-specific working rules.
