...
| `requires_env` | Interactive credential prompt during `doppel plugins install` (see [Build a Doppel Plugin](/guides/build-a-hermes-plugin#gate-on-environment-variables) for the rich format) |
...
If your provider wraps a third-party SDK (like DDGS does with the `ddgs` package), don't `import` it at module top level. Use `tools.lazy_deps.ensure(...)` inside `is_available()` or `search()` — Doppel will install the package on first use, gated by `security.allow_lazy_installs`. See [Build a Doppel Plugin → Lazy-install](/guides/build-a-hermes-plugin#lazy-install-optional-python-dependencies) for the security model.
...
- [Build a Doppel Plugin](/guides/build-a-hermes-plugin) — general tools/hooks/slash commands guide
