...
sidebar_position: 2
title: "Installation"
description: "Install Doppel Agent on Linux, macOS, WSL2, native Windows (early beta), or Android via Termux"
---

# Installation

Get Doppel Agent up and running in under two minutes with the one-line installer.
...
:::warning Early BETA
Native Windows support is **early beta**. It installs and works for the common paths, but hasn't been road-tested as broadly as our POSIX installers. Please [file issues](https://github.com/Jnot1/doppel-agent/issues) when you hit rough edges. For the most battle-tested setup on Windows today, use the Linux/macOS one-liner above inside **WSL2** instead.
:::
...
2. Otherwise it downloads portable **PortableGit** (~50MB, from the official `git-for-windows` GitHub release) and unpacks it to `%LOCALAPPDATA%\doppel\git`. No admin rights required. Completely isolated — it won't interfere with any system Git install, broken or otherwise. (On 32-bit Windows it falls back to MinGit because PortableGit ships only 64-bit and ARM64 assets; bash-dependent Doppel features won't work on 32-bit hosts.)
...
**Desktop installer (alternative):** A thin GUI installer is also available — download Doppel Desktop, run the `.exe`, and on first launch it calls `install.ps1` under the hood to provision Python (via `uv`), Node, PortableGit, and the rest of the dependencies. The desktop app and the PowerShell-installed CLI share the same install and data directories, so you can use either or both. See the [Windows (Native) guide](../user-guide/windows-native#desktop-installer-alternative) for details.

### Android / Termux

Doppel now ships a Termux-aware installer path too:
...
- **CLI (`doppel chat`, `doppel setup`, `doppel gateway`, …)** — native, uses your default terminal
...
The installer handles everything automatically — all dependencies (Python, Node.js, ripgrep, ffmpeg), the repo clone, virtual environment, global `doppel` command setup, and LLM provider configuration. By the end, you're ready to chat.
...
doppel gateway setup  # Set up messaging platforms
doppel config set     # Set individual config values
doppel setup          # Or run the full setup wizard to configure everything at once
```
...
2. **As the unprivileged service user**, run the regular installer. It will detect the missing sudo, skip `--with-deps`, and install Chromium into the user's local Playwright cache:
   ...
   ```bash
   curl -fsSL https://raw.githubusercontent.com/Jnot1/doppel-agent/main/scripts/install.sh | bash -s -- --skip-browser
   ```

3. **Make `doppel` available to the service user's shells.** The installer writes the launcher to `~/.local/bin/doppel`. System service accounts often have a minimal PATH that doesn't include `~/.local/bin`. Either add it to the user's environment, or symlink the launcher into a system location:
   ```bash
...
| Problem | Solution |
|---------|----------|
| `doppel: command not found` | Reload your shell (`source ~/.bashrc`) or check PATH |
| `API key not set` | Run `doppel model` to configure your provider, or `doppel config set OPENROUTER_API_KEY your_key` |
| Missing config after update | Run `doppel config check` then `doppel config migrate` |

For more diagnostics, run `doppel doctor` — it will tell you exactly what's missing and how to fix it.
