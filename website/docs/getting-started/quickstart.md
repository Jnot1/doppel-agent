...
sidebar_position: 1
title: "Quickstart"
description: "Your first conversation with Doppel Agent — from install to chatting in under 5 minutes"
---

# Quickstart

This guide gets you from zero to a working Doppel setup that survives real use. Install, choose a provider, verify a working chat, and know exactly what to do when something breaks.

## Prefer to watch?

**Onchain AI Garage** put together a Masterclass walkthrough of installation, setup, and basic commands — a good companion to this page if you'd rather follow along on video. For more, see the full [Doppel Agent Tutorials & Use Cases](https://www.youtube.com/playlist?list=PLmpUb_PWAkDxewld5ZYyKifuHxgIbiq2d) playlist.

<div style={{position: 'relative', paddingBottom: '56.25%', height: 0, overflow: 'hidden', maxWidth: '100%', marginBottom: '1.5rem'}}>
  <iframe
    style={{position: 'absolute', top: 0, left: 0, width: '100%', height: '100%'}}
    src="https://www.youtube-nocookie.com/embed/R3YOGfTBcQg"
    title="Doppel Agent Masterclass: Installation, Setup, Basic Commands"
    frameBorder="0"
...
- Setting up Doppel for a team, bot, or always-on workflow
...
|---|---|---|
| I just want Doppel working on my machine | `doppel setup` | Run a real chat and verify it responds |
| I already know my provider | `doppel model` | Save the config, then start chatting |
| I want a bot or always-on setup | `doppel gateway setup` after CLI works | Connect Telegram, Discord, Slack, or another platform |
| I want a local or self-hosted model | `doppel model` → custom endpoint | Verify the endpoint, model name, and context length |
| I want multi-provider fallback | `doppel model` first | Add routing and fallback only after the base chat works |

**Rule of thumb:** if Doppel cannot complete a normal chat, do not add more features yet. Get one clean conversation working first, then layer on gateway, cron, skills, voice, or routing.

---

## 1. Install Doppel Agent
...
The single most important setup step. Use `doppel model` to walk through the choice interactively:
...
|----------|-----------|---------------|
| **Nous Portal** | Subscription-based, zero-config | OAuth login via `doppel model` |
| **OpenAI Codex** | ChatGPT OAuth, uses Codex models | Device code auth via `doppel model` |
| **Anthropic** | Claude models directly — Max plan + extra usage credits (OAuth), or API key for pay-per-token | `doppel model` → OAuth login (requires Max + extra credits), or an Anthropic API key |
...
| **MiniMax (OAuth)** | MiniMax frontier model via browser OAuth — no API key needed (model name may change between releases) | `doppel model` → MiniMax (OAuth) |
...
| **Google Gemini (OAuth)** | Gemini via the `google-gemini-cli` OAuth flow — no key needed | `doppel model` → Google Gemini (OAuth) |
| **xAI** | Grok models via direct API | Set `XAI_API_KEY` |
| **xAI Grok OAuth** | SuperGrok / Premium+ subscription, no API key needed | `doppel model` → xAI Grok OAuth |
...
| **Qwen OAuth** | Qwen Portal browser OAuth — no API key needed | `doppel model` → Qwen OAuth |
...
| **GitHub Copilot** | GitHub Copilot subscription (GPT-5.x, Claude, Gemini, etc.) | OAuth via `doppel model`, or `COPILOT_GITHUB_TOKEN` / `GH_TOKEN` |
| **GitHub Copilot ACP** | Copilot ACP agent backend (spawns local `copilot` CLI) | `doppel model` (requires `copilot` CLI + `copilot login`) |
...
Doppel Agent requires a model with at least **64,000 tokens** of context. Models with smaller windows cannot maintain enough working memory for multi-step tool-calling workflows and will be rejected at startup. Most hosted models (Claude, GPT, Gemini, Qwen, DeepSeek) meet this easily. If you're running a local model, set its context size to at least 64K (e.g. `--ctx-size 65536` for llama.cpp or `-c 65536` for Ollama).
:::

:::tip
You can switch providers at any time with `doppel model` — no lock-in. For a full list of all supported providers and setup details, see [AI Providers](../integrations/providers.md).
:::

### How settings are stored

Doppel separates secrets from normal config:

- **Secrets and tokens** → `~/.doppel/.env`
- **Non-secret settings** → `~/.doppel/config.yaml`

The easiest way to set values correctly is through the CLI:

```bash
doppel config set model anthropic/claude-opus-4.6
doppel config set terminal.backend docker
doppel config set OPENROUTER_API_KEY sk-or-...
```
...
```bash
doppel            # classic CLI
doppel --tui      # modern TUI (recommended)
```

You'll see a welcome banner with your model, available tools, and skills. Use a prompt that's specific and easy to verify:

:::tip Pick your interface
Doppel ships with two terminal interfaces: the classic `prompt_toolkit` CLI and a newer [TUI](../user-guide/tui.md) with modal overlays, mouse selection, and non-blocking input. Both share the same sessions, slash commands, and config — try each with `doppel` vs `doppel --tui`.
:::
...
```bash
doppel --continue    # Resume the most recent session
doppel -c            # Short form
```
...
```bash
doppel gateway setup    # Interactive platform configuration
```
...
- `doppel tools` — tune tool access per platform
- `doppel skills` — browse and install reusable workflows
...
```bash
doppel config set terminal.backend docker    # Docker isolation
doppel config set terminal.backend ssh       # Remote server
```
...
|---|---|---|
| Doppel opens but gives empty or broken replies | Provider auth or model selection is wrong | Run `doppel model` again and confirm provider, model, and auth |
...
| Gateway starts but nobody can message it | Bot token, allowlist, or platform setup is incomplete | Re-run `doppel gateway setup` and check `doppel gateway status` |
| `doppel --continue` can't find old session | Switched profiles or session never saved | Check `doppel sessions list` and confirm you're in the right profile |
...
| `doppel doctor` flags config problems | Config values are missing or stale | Fix the config, retest a plain chat before adding features |
...
2. `doppel model`
3. `doppel setup`
4. `doppel sessions list`
5. `doppel --continue`
6. `doppel gateway status`
...
| `doppel tools` | Configure which tools are enabled per platform |
| `doppel setup` | Full setup wizard (configures everything at once) |
| `doppel doctor` | Diagnose issues |
| `doppel update` | Update to latest version |
| `doppel gateway` | Start the messaging gateway |
| `doppel --continue` | Resume last session |
