...
description: "Run Doppel Agent directly on an Android phone with Termux"
---

# Doppel Agent on Android with Termux

This is the tested path for running Doppel Agent directly on an Android phone through [Termux](https://termux.dev/).
...
That does not stop Doppel from working well as a phone-native CLI agent — it just means the recommended mobile install is intentionally narrower than the desktop/server install.

---

## Option 1: One-line installer

Doppel Agent now ships a Termux-aware installer path:
...
- links `doppel` into `$PREFIX/bin` so it stays on your Termux PATH (the legacy `hermes` alias still works)
...
### 5. Put `doppel` on your Termux PATH

```bash
ln -sf "$PWD/venv/bin/doppel" "$PREFIX/bin/doppel"
```

`$PREFIX/bin` is already on PATH in Termux, so this makes the `doppel` command persist across new shells without re-activating the venv every time.
...
Or set keys directly in `~/.doppel/.env` on fresh installs. Legacy `~/.hermes/.env` homes are still supported.
...
### `doppel doctor` says ripgrep or Node is missing
