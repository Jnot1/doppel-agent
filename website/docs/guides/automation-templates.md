...
Copy-paste recipes for common automation patterns. Each template uses Doppel's built-in [cron scheduler](/user-guide/features/cron) for time-based triggers and [webhook platform](/user-guide/messaging/webhooks) for event-driven triggers.
...
| **GitHub Event** | Fires on PR opens, pushes, issues, CI results | Webhook platform (`doppel webhook subscribe`) |
| **API Call** | External service POSTs JSON to your endpoint | Webhook platform (config.yaml routes or `doppel webhook subscribe`) |
...
```bash
doppel cron create "0 2 * * *" \
  "You are a project manager triaging the Jnot1/doppel-agent GitHub repo.

1. Run: gh issue list --repo Jnot1/doppel-agent --state open --json number,title,labels,author,createdAt --limit 30
...
```bash
doppel webhook subscribe github-pr-review \
  --events "pull_request" \
...
```bash
doppel cron create "0 9 * * 1" \
  "Scan the Jnot1/doppel-agent repo for documentation drift.

1. Run: gh pr list --repo Jnot1/doppel-agent --state merged --json number,title,files,mergedAt --limit 30
...
```bash
doppel cron create "0 6 * * *" \
  "Run a dependency security audit on the doppel-agent project.

1. cd /path/to/your/doppel-agent/checkout && source .venv/bin/activate
...
```bash
doppel webhook subscribe deploy-verify \
  --events "deployment" \
...
```bash
doppel webhook subscribe alert-triage \
...
```python title="~/.doppel/scripts/check-uptime.py"
...
for ep in ENDPOINTS:
    try:
        start = time.time()
        req = urllib.request.Request(ep["url"], headers={"User-Agent": "Doppel-Monitor/1.0"})
...
doppel cron create "every 30m" \
  ...
  --script ~/.doppel/scripts/check-uptime.py \
  --name "Uptime monitor" \
  --deliver telegram
```
...
```bash
doppel cron create "0 8 * * *" \
  "Search arXiv for the 3 most interesting papers on 'language model reasoning' OR 'tool-use agents' from the past day. For each paper, create an Obsidian note with the title, authors, abstract summary, key contribution, and potential relevance to Doppel Agent development." \
...
```bash
doppel webhook subscribe auto-port \
  --events "pull_request" \
...
```bash
doppel cron create "0 3 * * 0" \
  "Run a comprehensive security audit of the doppel-agent codebase.
