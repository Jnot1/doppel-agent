...
description: "Diagnose and fix common Doppel cron issues — jobs not firing, delivery failures, skill loading errors, and performance problems"
---
...
If you're expecting jobs to fire automatically, you need a running gateway (`doppel gateway` for foreground, or `doppel gateway start` for the installed service). For one-off debugging, you can manually trigger a tick with `doppel cron tick`.
...
```bash
date
doppel cron list   # Compare next_run times with local time
```
...
| Target | Requires |
|--------|----------|
| `telegram` | `TELEGRAM_BOT_TOKEN` in `~/.doppel/.env` |
| `discord` | `DISCORD_BOT_TOKEN` in `~/.doppel/.env` |
| `slack` | `SLACK_BOT_TOKEN` in `~/.doppel/.env` |
...
| `local` | Write access to `~/.doppel/cron/output/` |
...
If delivery fails, the job still runs — it just won't send anywhere. Check `doppel cron list` for updated `last_error` field (if available).
...
Skills must be installed before they can be attached to cron jobs. If a skill is missing, install it first with `doppel skills install <skill-name>` or via `/skills` in the CLI.

### Check 2: Check skill name vs. skill folder name

Skill names are case-sensitive and must match the installed skill's folder name. If your job specifies `ai-funding-daily-report` but the skill folder is `ai-funding-daily-report`, confirm the exact name from `doppel skills list`.
...
2. `~/.doppel/logs/agent.log` for scheduler messages (or `errors.log` for warnings)
3. The job's `last_run` metadata via `doppel cron list`

### Check 2: Common error patterns

**"No such file or directory" for scripts**
The `script` path must be an absolute path (or relative to the Doppel config directory). Verify:
```bash
ls ~/.doppel/scripts/your-script.py   # Must exist
doppel cron edit <job_id> --script ~/.doppel/scripts/your-script.py
```

**"Skill not found" at job execution**
The skill must be installed on the machine running the scheduler. If you move between machines, skills don't automatically sync — reinstall them with `doppel skills install <skill-name>`.
...
Jobs are stored in `~/.doppel/cron/jobs.json`. If this file is not readable/writable by your user, the scheduler will fail silently:

```bash
ls -la ~/.doppel/cron/jobs.json
chmod 600 ~/.doppel/cron/jobs.json   # Your user should own it
```
...
```bash
doppel cron list                    # Show all jobs, states, next_run times
doppel cron run <job_id>            # Schedule for next tick (for testing)
doppel cron edit <job_id>           # Fix configuration issues
doppel logs                         # View recent Doppel logs
doppel skills list                  # Verify installed skills
```
...
1. Run the job with `doppel cron run <job_id>` (fires on next gateway tick) and watch for errors in the chat output
2. Check `~/.doppel/logs/agent.log` for scheduler messages and `~/.doppel/logs/errors.log` for warnings
3. Open an issue at [github.com/Jnot1/doppel-agent](https://github.com/Jnot1/doppel-agent) with:
