...
You need **tenant admin rights** (or an admin to grant consent on your behalf) to finish this. Bookmark the values you collect — they go into your agent-home `.env` file at the end (`~/.doppel/.env` on fresh installs; legacy `~/.hermes/.env` still works).
...
4. Fill in:
   - **Name:** `Doppel Teams Meeting Pipeline` (or any name you'll recognize).
...
3. **Description:** `doppel-graph-secret`. **Expires:** pick a value that matches your rotation policy (6-24 months is typical).
4. Click **Add**.
...
Put the three values you collected into your agent-home `.env` file (`~/.doppel/.env` on fresh installs; legacy `~/.hermes/.env` still works):
...
Doppel Agent ships a Graph auth smoke-test. From your Doppel Agent install:
...
2. Update `MSGRAPH_CLIENT_SECRET` in your agent-home `.env` file (`~/.doppel/.env` on fresh installs; legacy `~/.hermes/.env` still works).
3. Restart the gateway so the new secret is picked up: `doppel gateway restart`.
