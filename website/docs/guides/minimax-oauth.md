...
sidebar_position: 15
title: "MiniMax OAuth"
description: "Log into MiniMax via browser OAuth and use MiniMax-M2.7 models in Doppel Agent — no API key required"
---

# MiniMax OAuth

Doppel Agent supports **MiniMax** through a browser-based OAuth login flow, using the same credentials as the [MiniMax portal](https://www.minimax.io). No API key or credit card is required — log in once and Doppel automatically refreshes your session.
...
# → Doppel opens your browser to the MiniMax authorization page
...
After the first login, credentials are stored under `~/.doppel/auth.json` on fresh installs; legacy installs may still use `~/.hermes/auth.json`. They are refreshed automatically before each session.
...
```bash
# Fresh installs prefer ~/.doppel/.env; legacy ~/.hermes/.env still works
echo 'MINIMAX_CN_API_KEY=your-key' >> ~/.doppel/.env
```
...
```bash
doppel auth add minimax-oauth --no-browser
```

Doppel will print the verification URL and user code — open the URL on any device and enter the code when prompted.

## The OAuth Flow

Doppel implements a PKCE browser OAuth flow against the MiniMax OAuth endpoints:

1. Doppel generates a PKCE verifier / challenge pair and a random state value.
...
4. Doppel polls `{base_url}/oauth/token` until the token arrives (or the deadline passes).
5. Tokens (`access_token`, `refresh_token`, expiry) are saved to `~/.doppel/auth.json` on fresh installs (legacy installs may still use `~/.hermes/auth.json`) under the `minimax-oauth` key.
...
```bash
doppel config set model.default MiniMax-M2.7
doppel config set model.provider minimax-oauth
```

## Configuration Reference

After login, `~/.doppel/config.yaml` on fresh installs (legacy installs may still use `~/.hermes/config.yaml`) will contain entries similar to:
...
- `minimax-portal` — alias
- `minimax-global` — alias
- `minimax_oauth` — alias (underscore form)
...
To use `minimax-oauth` as the active provider, set `model.provider: minimax-oauth` in `config.yaml`, or use `doppel model` for the guided flow.
...
Doppel refreshes the token on every session start if it is within 60 seconds of expiry. If the access token is already expired (for example, after a long offline period), the refresh happens automatically on the next request. If refresh fails with `refresh_token_reused` or `invalid_grant`, Doppel marks the session as requiring re-login.

When the refresh failure is terminal (HTTP 4xx, `invalid_grant`, revoked grant, etc.), Doppel marks the refresh token as dead and quarantines it locally so it doesn't keep replaying the doomed exchange. The agent surfaces a single "re-authentication required" message and stays out of the way until you log in again.

**Fix:** run `doppel auth add minimax-oauth` again to start a fresh login. The quarantine clears on the next successful exchange.

### Authorization timed out

The device-code flow has a finite expiry window. If you don't approve the login in time, Doppel raises a timeout error.

**Fix:** re-run `doppel auth add minimax-oauth` (or `doppel model`). The flow starts fresh.

### State mismatch (possible CSRF)

Doppel detected that the `state` value returned by the authorization server does not match what it sent.
...
If `doppel` cannot open a browser window, use `--no-browser`:

```bash
doppel auth add minimax-oauth --no-browser
```

Doppel prints the URL and code. Open the URL on any device and complete the flow there.
...
**Fix:** run `doppel model` and select MiniMax (OAuth), or run `doppel auth add minimax-oauth`.
...
```bash
doppel auth remove minimax-oauth
```
...
- [doppel doctor](../reference/cli-commands.md)
