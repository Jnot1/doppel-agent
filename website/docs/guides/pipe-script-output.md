...
description: "Send text from any shell script, cron job, CI hook, or monitoring daemon to Telegram, Discord, Slack, Signal, and other platforms using `doppel send`."
---

# Pipe Script Output to Messaging Platforms

`doppel send` is a small, scriptable CLI that pushes a message to any
messaging platform Doppel is already configured for. Think of it as a
...
- Piping any tool's output anywhere (`make | doppel send --to slack:#builds`)

The command reuses the same credentials and platform adapters that `doppel
...
doppel send --to slack:#eng --subject "[CI] build.log" --file build.log

# Thread target (Telegram topic, Discord thread)
doppel send --to telegram:-1001234567890:17585 "threaded reply"

# List every configured target
doppel send --list

# Filter by platform
doppel send --list telegram
```
...
Any platform Doppel ships adapters for works as a target:
...
`doppel send` resolves the message body in this order:

1. **Positional argument** — `doppel send --to telegram "hi"`
2. **`--file PATH`** — `doppel send --to telegram --file msg.txt`
3. **Piped stdin** — `echo hi | doppel send --to telegram`

When stdin is a TTY (no pipe), Doppel does **not** wait for input — you'll
...
if [ "$ram_pct" -ge 85 ]; then
  doppel send --to telegram --subject "⚠ MEMORY WARNING" \
    "RAM ${ram_pct}% on $(hostname)"
fi
```

Because `doppel send` reuses your Doppel config, the same script works on
any host where Doppel is installed — no need to export bot tokens into
...
`doppel send`. If the Python interpreter can't load because the box is
...
if ./scripts/deploy.sh; then
  doppel send --to slack:#deploys "✅ ${CI_COMMIT_SHA:0:7} deployed"
else
  tail -n 100 deploy.log | doppel send \
...
0 9 * * * /usr/local/bin/generate-metrics.sh \
  | /home/me/.doppel/bin/doppel send \
...
```bash
./train.py --epochs 200 && \
  doppel send --to telegram "training done" || \
  doppel send --to telegram "training failed (exit $?)"
```
...
doppel send --to telegram --quiet "keepalive" || {
  echo "Telegram delivery failed" >&2
  exit 1
}

# Capture the message ID for later editing / threading
msg_id=$(doppel send --to discord:#ops --json "build started" \
  | jq -r .message_id)
```

---

## Does `doppel send` Need the Gateway Running?

**Usually no.** For any bot-token platform — Telegram, Discord, Slack,
Signal, SMS, WhatsApp Cloud API, and most others — `doppel send` calls
the platform's REST endpoint directly using credentials from
`~/.doppel/.env` and `~/.doppel/config.yaml`. It's a standalone subprocess
...
pointing at the gateway; start it with `doppel gateway start` and retry.
...
# Machine-readable
doppel send --list --json
```

The listing is built from `~/.doppel/channel_directory.json`, which the
gateway refreshes every few minutes while it's running. If you see
"no channels discovered yet", start the gateway once (`doppel gateway
...
| Approach | Multi-platform | Reuses Doppel creds | Needs gateway | Best for |
|----------|----------------|---------------------|---------------|----------|
| `doppel send` | ✅ | ✅ | No (bot-token) | Everything below |
...
`doppel send` is intentionally the simplest possible surface. If you need
...
If you just need to pipe a raw string, reach for `doppel send`.
...
- [Gateway Internals](/developer-guide/gateway-internals) —
  the delivery router that `doppel send` shares with cron delivery.
