...
OAuth needs a browser, but the loopback callback runs on the machine where Doppel is running. Two options:

```bash
# Option A: SSH port forwarding (preferred)
ssh -N -L 8642:127.0.0.1:8642 user@remote-host    # in a local terminal
doppel setup --portal                             # on the remote, open the printed URL in your local browser

# Option B: manual paste (for Cloud Shell, Codespaces, EC2 Instance Connect)
doppel auth add nous --type oauth --manual-paste
# Then re-run `doppel setup --portal` to wire the provider + gateway
```
...
```
Hey, search the web for "Doppel Agent release notes" and summarize the top 3 hits.
```

You should see Doppel call `web_search` (Firecrawl-backed, through the gateway) and respond with a summary. If the search runs and the response makes sense, you're done — the Portal is wired up end to end.

## 5. Pick the model you actually want

The default after `doppel setup --portal` is a sensible general-purpose model, but the whole point of the subscription is access to the full catalog. Switch with `/model` mid-session:
...
```bash
doppel setup tts
# → pick "Nous Subscription" for TTS if you want Portal-backed voice output
# → pick a speech-to-text backend separately (local faster-whisper is free, no setup)
```

Then in any messaging-platform session (Telegram, Discord, Signal, etc.), send a voice message and Doppel will transcribe it, respond, and reply with synthesized voice — all on your Portal subscription.
...
```bash
doppel cron create "every day at 9am" \
...
### `doppel portal status` shows "not logged in" after `doppel setup --portal`
...
```bash
doppel config set model.provider nous
```
...
Re-verify with `doppel portal status`.
...
Your Portal refresh token was invalidated (password change, manual revoke, session expiry). The token is now quarantined locally so Doppel doesn't replay it endlessly. Just log in again:

```bash
doppel auth add nous
```
...
`doppel portal status` will tell you whether you're actually routing through the Portal or some other provider. Common causes:
...
- Multiple Doppel profiles where you're using the wrong one (check `doppel profile`)

### Want to revoke and start clean

```bash
doppel auth remove nous       # wipes the local refresh token
...
- **[Subscription proxy](/user-guide/features/subscription-proxy)** — Use your Portal subscription from non-Doppel tools
...
- **[Profiles](/user-guide/profiles)** — Share one Portal login across multiple Doppel configurations
