...
This tutorial walks you through setting up a Telegram bot powered by Doppel Agent that multiple team members can use. By the end, your team will have a shared AI assistant they can message for help with code, research, system administration, and anything else — secured with per-user authorization.
...
- **Doppel Agent installed** on a server or VPS (not your laptop — the bot needs to stay running). Follow the [installation guide](/getting-started/installation) if you haven't yet.
- **A Telegram account** for yourself (the bot owner)
- **An LLM provider configured** — at minimum, an API key for OpenAI, Anthropic, or another supported provider in `~/.doppel/.env`

:::tip
A $5/month VPS is plenty for running the gateway. Doppel itself is lightweight — the LLM API calls are what cost money, and those happen remotely.
:::
...
2. **Send `/newbot`** — BotFather will ask you two things:
   - **Display name** — what users see (e.g., `Team Doppel Assistant`)
   - **Username** — must end in `bot` (e.g., `myteam_doppel_bot`)
...
4. **Set a description** (optional but recommended):
   ...
   ```
   Team AI assistant powered by Doppel Agent. DM me for help with code, research, debugging, and more.
   ```
...
```
[Gateway] Doppel Gateway Starting...
...
# View live logs
journalctl --user -u doppel-gateway -f
...
:::tip macOS PATH
The launchd plist captures your shell PATH at install time so gateway subprocesses can find tools like Node.js and ffmpeg. If you install new tools later, re-run `doppel gateway install` to update the plist.
:::
...
```bash
doppel gateway stop && doppel gateway start
```
...
doppel pairing revoke telegram 987654321
...
**Option 2:** Set it manually in `~/.doppel/.env`:
...
Control how much detail the bot shows when using tools. In `~/.doppel/config.yaml`:
...
```bash
# From the CLI
doppel cron list          # View all scheduled jobs
doppel cron status        # Check if scheduler is running
...
| What | Location |
|------|----------|
| Gateway logs | `journalctl --user -u doppel-gateway` (Linux) or `~/.doppel/logs/gateway.log` (macOS) |
| Cron job output | `~/.doppel/cron/output/{job_id}/{timestamp}.md` |
| Cron job definitions | `~/.doppel/cron/jobs.json` |
| Pairing data | `~/.doppel/pairing/` |
| Session history | `~/.doppel/sessions/` |
