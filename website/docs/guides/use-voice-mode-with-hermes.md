...
# Use Voice Mode with Doppel Agent
...
- you want Doppel Agent sitting in a Discord voice channel for live conversation
...
There are really three different voice experiences in Doppel Agent.
...
## Step 1: make sure normal Doppel Agent works first
...
Doppel Agent supports both local and cloud speech stacks.
...
4. Doppel Agent transcribes and responds
...
- asking Doppel Agent to structure your thoughts in real time

#### Accessibility / low-typing sessions

If typing is inconvenient, voice mode is one of the fastest ways to stay in the full Doppel loop.

## Tuning CLI behavior

### Silence threshold

If Doppel Agent starts/stops too aggressively, tune:
...
Doppel Agent stays a normal chat bot, but can speak replies.
...
- you want Doppel Agent to function like a portable research or ops assistant
...
Doppel Agent joins a Discord VC, listens to user speech, transcribes it, runs the normal agent pipeline, and speaks replies back into the channel.
...
- Doppel Agent detects speech boundaries
- transcripts are posted in the associated text channel
- Doppel Agent responds in text and audio
...
1. get text Doppel working
2. install the voice extra with `cd ~/.doppel/doppel-agent && uv pip install -e ".[voice]"`
