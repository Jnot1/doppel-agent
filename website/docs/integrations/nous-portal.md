...
sidebar_position: 1
title: "Nous Portal"
description: "One subscription, 300+ frontier models, the Tool Gateway, and Nous Chat — the recommended way to run Doppel Agent"
---
...
That single command runs the Portal OAuth, sets Nous as your inference provider in `config.yaml`, and turns on the Tool Gateway. You're ready to `doppel chat` immediately after.
...
The same subscription unlocks the [Tool Gateway](/user-guide/features/tool-gateway), which routes Doppel Agent's tool calls through Nous-managed infrastructure. Five backends, one login:
...
Because everything routes through one OAuth-authenticated Portal session, you don't accumulate a `.env` file with a dozen long-lived API keys. The refresh token at `~/.doppel/auth.json` on fresh installs (legacy installs may still use `~/.hermes/auth.json`) is the only credential on disk, and Doppel mints short-lived JWTs from it per request — see [Token handling](#token-handling) below.
...
The Portal's own [model info page](https://portal.nousresearch.com/info) carries the same warning, so this isn't just a Doppel-side opinion — it's the official guidance from Nous Research.
...
3. Sets Nous as your inference provider in `~/.doppel/config.yaml` on fresh installs (legacy installs may still use `~/.hermes/config.yaml`)
4. Turns on the Tool Gateway (web, image, TTS, browser routing)
5. Returns you to your terminal ready to `doppel chat`
...
If you already have Doppel configured with OpenRouter, Anthropic, or any other provider and you want to add the Portal alongside them:
...
Your existing providers stay configured. You can switch between them with `/model` mid-session or `doppel model` between sessions — the Portal becomes one of your available providers, not your only one.

### Headless / SSH / remote setup

OAuth needs a browser, but the loopback callback runs on the machine where Doppel is running. For remote hosts, see [OAuth over SSH / Remote Hosts](/guides/oauth-over-ssh) — the same patterns work for the Portal as for any other OAuth-based provider (`ssh -L` port forwarding, `--manual-paste` for browser-only environments like Cloud Shell / Codespaces).

### Profile setup

If you use [Doppel profiles](/user-guide/profiles), the Portal refresh token is automatically shared across all profiles via a shared token store. Sign in once on any profile, and the rest pick it up automatically — no need to repeat the OAuth flow per profile.
...
```bash
doppel portal status     # login status, subscription info, model + gateway routing
doppel portal tools      # detailed Tool Gateway catalog with per-tool routing
doppel portal open       # open the subscription management page in your browser
```

`doppel portal status` (or just `doppel portal`) gives you the high-level overview:
...
If you already have, say, a Browserbase account and want to keep using it while routing web search and image generation through Nous, that's supported. Use `doppel tools` to pick backends per tool:
...
- **CLI shortcut:** `doppel portal open` (opens the same page in your default browser)

## Configuration reference

After `doppel setup --portal`, `~/.doppel/config.yaml` on fresh installs (legacy installs may still use `~/.hermes/config.yaml`) will look like:
...
The OAuth refresh token is stored separately at `~/.doppel/auth.json` on fresh installs (legacy installs may still use `~/.hermes/auth.json`) (not in `config.yaml` — credentials and configuration are kept separate by design).

## Token handling

Doppel mints a short-lived JWT from your stored Portal refresh token on each inference call rather than reusing a long-lived API key. The token lifecycle is fully automatic — refresh, mint, retry on transient 401 — and you never see it.

If the Portal invalidates the refresh token (password change, manual revoke, session expiry), the invalid refresh token is **quarantined locally** so Doppel stops replaying it and you don't see a stream of identical 401s. The next call surfaces a clear "re-authentication required" message. Run `doppel auth add nous` to log in again; the quarantine clears on the next successful login.

## Troubleshooting

### `doppel portal status` shows "not logged in"

You haven't completed the OAuth flow, or your refresh token was wiped. Run:

```bash
doppel auth add nous --type oauth
```

or use `doppel model` and re-select Nous Portal.

### Got a "re-authentication required" message mid-session

Your Portal refresh token was invalidated (password change, manual revoke, or session expiry). Run `doppel auth add nous` and your next request will use the new credentials. Any quarantine on the old token clears automatically on successful re-login.
...
If a model is genuinely missing, [open an issue](https://github.com/NousResearch/hermes-agent/issues) — we surface the Portal's catalog to Doppel and gaps usually mean a routing config we can update.

### Bills not appearing on my Portal account

Check `doppel portal status` first — if it shows you're using a different provider (`Model: currently openrouter` instead of `using Nous as inference provider`), your local config has drifted. Run `doppel model`, pick Nous Portal, and the next request will route through your subscription.

## See also

- **[Tool Gateway](/user-guide/features/tool-gateway)** — Full details on every gateway tool, per-tool config, and pricing
- **[Subscription proxy](/user-guide/features/subscription-proxy)** — Use your Portal subscription from non-Doppel tools (other agents, scripts, third-party clients)
...
- **[Profiles](/user-guide/profiles)** — Multiple Doppel configurations sharing one Portal login
