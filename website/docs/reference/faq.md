...
description: "Frequently asked questions and solutions to common issues with Doppel Agent"
---
...
### What LLM providers work with Doppel?

Doppel Agent works with any OpenAI-compatible API. Supported providers include:

- **[OpenRouter](https://openrouter.ai/)** — access hundreds of models through one API key (recommended for flexibility)
- **[Doppel Portal](/integrations/nous-portal)** — Doppelme's subscription gateway — 300+ models plus web/image/TTS/browser through one OAuth login (recommended for newcomers)
- **OpenAI** — GPT-5.4, GPT-5-codex, GPT-4.1, GPT-4o, etc.
- **Anthropic** — Claude models (direct API, OAuth via `doppel auth add anthropic`, OpenRouter, or any compatible proxy)
...
Yes, but treat it as **early beta**. Doppel Agent runs natively on Windows 10 and 11 through the PowerShell installer, and the most battle-tested Windows path is still WSL2 if you want full POSIX behavior.
...
See the [Windows (Native) guide](../user-guide/windows-native.md) for the native path and the [Windows (WSL2) guide](../user-guide/windows-wsl-quickstart.md) for the more conservative one.

### I run Doppel in WSL2. What's the best way to control my normal Windows Chrome?

Prefer an MCP bridge over `/browser connect`.

Recommended pattern:

- run Doppel inside WSL2
...
- let Doppel use the resulting MCP browser tools

This is more reliable than trying to force Doppel's core browser transport to attach directly across the WSL2/Windows boundary.
...
Yes — Doppel now has a tested Termux install path for Android phones.
...
Yes. Run `doppel model`, select **Custom endpoint**, and enter your server's URL:
...
# Context length: 64000   ← Doppel minimum; set this to match your server's actual context window
```
...
Doppel persists the endpoint, provider, and base URL in `config.yaml` so it survives restarts. If your local server has exactly one model loaded, `/model custom` auto-detects it. You can also set `provider: custom` in config.yaml — it's a first-class provider, not an alias for anything else.
...
:::tip Ollama users
If you set a custom `num_ctx` in Ollama (e.g., `ollama run --num_ctx 64000`), make sure to set the matching context length in Doppel — Ollama's `/api/show` reports the model's *maximum* context, not the effective `num_ctx` you configured.
:::
...
Doppel Agent itself is **free and open-source** (MIT license). You pay only for the LLM API usage from your chosen provider. Local models are completely free to run.

### Can multiple people use one instance?

Yes. The [messaging gateway](../user-guide/messaging/index.md) lets multiple users interact with the same Doppel Agent instance via Telegram, Discord, Slack, WhatsApp, or Home Assistant. Access is controlled through allowlists (specific user IDs) and DM pairing (first user to message claims access).
...
Yes. Import the `AIAgent` class and use Doppel programmatically:
...
#### `doppel: command not found` after installation
...
**Cause:** Doppel requires Python 3.11 or newer.
...
**Solution:** Exit your session and use `doppel model` from your terminal to add new providers:

```bash
# Exit the Doppel chat session first (Ctrl+C or /quit)
...
After adding a new provider via `doppel model`, start a new chat session — `/model` will now show all your configured providers.

:::tip Quick reference
| Want to... | Use |
|-----------|-----|
| Add a new provider | `doppel model` (from terminal) |
| Enter/change API keys | `doppel model` (from terminal) |
...
:::warning
Make sure the key matches the provider. An OpenAI key won't work with OpenRouter and vice versa. Check your agent-home `.env` (`~/.doppel/.env` on fresh installs) for conflicting entries.
:::
...
- Using `doppel chat --provider <alternative>` to route to a different backend

#### Context length exceeded

**Cause:** The conversation has grown too long for the model's context window, or Doppel detected the wrong context length for your model.
...
If this happens on the first long conversation, Doppel may have the wrong context length for your model. Check what it detected:
...
**Cause:** Doppel detected a potentially destructive command (e.g., `rm -rf`, `DROP TABLE`). This is a safety feature.
...
:::tip
This is working as intended — Doppel never silently runs destructive commands. The approval prompt shows you exactly what will execute.
:::
...
- Or switch to the terminal interface for administrative tasks: `doppel chat`
...
**Solution:**
- Verify your bot token is valid with `doppel gateway setup`
- Check gateway logs: `cat ~/.doppel/logs/gateway.log | tail -50`
...
Configure it in your agent-home `config.yaml` under your gateway's settings (`~/.doppel/config.yaml` on fresh installs). See the [Messaging docs](../user-guide/messaging/index.md).
...
#### WSL: Gateway keeps disconnecting or `doppel gateway start` fails
...
# Reattach later: tmux attach -t doppel
...
1. Create a task that runs `wsl -d Ubuntu -- bash -lc 'doppel gateway run'`
...
**Solution:** The gateway captures your shell PATH when you run `doppel gateway install`. If you installed tools after setting up the gateway, re-run the install to capture the updated PATH:

```bash
doppel gateway install    # Re-snapshots your current PATH
doppel gateway start      # Detects the updated plist and reloads
```
...
**Solution:**
- Try a faster/smaller model: `doppel chat --model openrouter/meta-llama/llama-3.1-8b-instruct`
- Reduce active toolsets: `doppel chat -t "terminal"`
...
Verify your `~/.doppel/config.yaml` MCP configuration:
```yaml
mcp_servers:
  filesystem:
    command: "npx"
...
# Restart Doppel or reload MCP after config changes
doppel chat
```
...
:::warning
If an MCP server crashes mid-request, Doppel will report a timeout. Check the server's own logs (not just Doppel logs) to diagnose the root cause.
:::
...
No. Each profile has its own memory store, session database, and skills directory. They are completely isolated. If you want to start a new profile with existing memories and sessions, use `doppel profile create newname --clone-all` to copy everything from the current profile.

### What happens when I run `doppel update`?

`doppel update` pulls the latest code and reinstalls dependencies **once** (not per-profile). It then syncs updated skills to all profiles automatically. You only need to run `doppel update` once — it covers every profile on the machine.


### How many profiles can I run?

There is no hard limit. Each profile is just a directory under your agent home, usually `~/.doppel/profiles/` on fresh installs. The practical limit depends on your disk space and how many concurrent gateways your system can handle (each gateway is a lightweight Python process). Running dozens of profiles is fine; each idle profile uses no resources.
...
**Solution: Delegation config.** Doppel can route subagents to a different model automatically. Set this in `~/.doppel/config.yaml`:
...
Now when you tell Doppel "write me a Twitter thread about X" and it spawns a `delegate_task` subagent, that subagent runs on Gemini instead of your main model. Your primary conversation stays on GPT-5.4.
...
**Scenario:** In OpenClaw, you had multiple independent agents bound to specific WhatsApp chats — one for a family shopping list group, another for your private chat. Can Doppel do this?

**Current limitation:** Doppel profiles each require their own WhatsApp number/session. You cannot bind multiple profiles to different chats on the same WhatsApp number — the WhatsApp bridge (Baileys) uses one authenticated session per number.
...
**Scenario:** You see gateway exec logs, Doppel reasoning, and tool call details in Telegram instead of just the final output.
...
**Scenario:** Telegram has a 100 slash command limit, and your skills are pushing past it. You want to disable skills you don't need on Telegram, but `doppel skills config` settings don't seem to take effect.

**Solution:** Use `doppel skills config` to disable skills per-platform. This writes to `config.yaml`:
...
After changing this, **restart the gateway** (`doppel gateway restart` or kill and relaunch). The Telegram bot command menu rebuilds on startup.
...
**Current behavior:** Doppel creates sessions keyed by user ID on most platforms, so each person gets their own conversation context. This is by design for privacy and context isolation.
...
### Exporting Doppel to another machine
...
1. Install Doppel Agent on the new machine:
   ```bash
   curl -fsSL https://raw.githubusercontent.com/Jnot1/doppel-agent/main/scripts/install.sh | bash
   ```
...
### `doppel backup` vs `doppel profile export`

| Feature | `doppel backup` | `doppel profile export` |
| :--- | :--- | :--- |
| **Use Case** | **Full machine migration** | **Porting/sharing a specific profile** |
| **Scope** | Global (entire agent home directory) | Local (single profile directory) |
...
:::tip
`doppel backup` produces a consistent snapshot even while Doppel is actively running. The restored archive excludes machine-local runtime files like `gateway.pid` and `cron.pid`.
:::

### Permission denied when reloading shell after install

**Scenario:** After running the Doppel installer, `source ~/.zshrc` gives a permission denied error.

**Cause:** This usually happens when `~/.zshrc` (or `~/.bashrc`) has incorrect file permissions, or when the installer couldn't write to it cleanly. It's not a Doppel-specific issue — it's a shell config permissions problem.
...
1. **Search existing issues:** [GitHub Issues](https://github.com/Jnot1/doppel-agent/issues)
2. **Ask the community:** [Community Discord](https://discord.gg/nousresearch)
3. **File a bug report:** Include your OS, Python version (`python3 --version`), Doppel version (`doppel --version`), and the full error message
