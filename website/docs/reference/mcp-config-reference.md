...
description: "Reference for Doppel Agent MCP configuration keys, filtering semantics, and utility-tool policy"
---
...
- [Use MCP with Doppel Agent](/guides/use-mcp-with-hermes)
...
Doppel Agent may register these utility wrappers per MCP server:
...
Even when `resources: true` or `prompts: true`, Doppel Agent only registers those utility tools if the MCP session actually exposes the corresponding capability.
...
If filtering removes all server-native tools and no utility tools are registered, Doppel Agent does not create an empty MCP runtime toolset for that server.
