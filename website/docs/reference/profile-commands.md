...
This page covers all commands related to [Doppel profiles](../user-guide/profiles.md). For general CLI commands, see [CLI Commands Reference](./cli-commands.md).
...
Top-level command for managing profiles. Running `doppel profile` without a subcommand shows help.
...
```bash
doppel profile use <name>
```

Sets `<name>` as the active profile. All subsequent `doppel` commands (without `-p`) will use this profile.
...
doppel profile use default
```

## `doppel profile create`

```bash
doppel profile create <name> [options]
```
...
| `--description "<text>"` | One- or two-sentence description of what this profile is good at. Used by the kanban orchestrator to route tasks based on role instead of profile name alone. Skip and add later via `doppel profile describe`. Persisted in `<profile_dir>/profile.yaml`. |
| `--no-skills` | Create an **empty** profile with zero bundled skills enabled. Writes a `.no-skills` marker into the profile so future `doppel update` runs won't re-seed the bundled set, and refuses to combine with `--clone` / `--clone-all` (which would copy skills in anyway). Useful for narrow orchestrator profiles or sandbox profiles that should not inherit the full skill catalog. |
...
doppel profile create work2 --clone --clone-from work
```

## `doppel profile describe`

```bash
doppel profile describe [<name>] [options]
```
...
doppel profile describe researcher

# Set it explicitly
doppel profile describe researcher --text "Reads source code and writes findings."

# Let the LLM generate one
doppel profile describe researcher --auto

# Fill in descriptions for every profile that doesn't have one
doppel profile describe --all --auto
```

## `doppel profile delete`

```bash
doppel profile delete <name> [options]
```
...
```bash
doppel profile delete mybot
doppel profile delete mybot --yes
```
...
```bash
doppel profile show <name>
```

Displays details about a profile including its home directory, configured model, gateway status, skills count, and configuration file status.

This shows the profile's Doppel home directory, not the terminal working directory. Terminal commands start from `terminal.cwd` (or the launch directory on the local backend when `cwd: "."`).
...
```bash
$ doppel profile show work
Profile: work
Path:    ~/.doppel/profiles/work
...
## `doppel profile alias`

```bash
doppel profile alias <name> [options]
```

Regenerates the shell alias script at `~/.local/bin/<name>`. Useful if the alias was accidentally deleted or if you need to update it after moving your Doppel installation.
...
```bash
doppel profile alias work
# Creates/updates ~/.local/bin/work

doppel profile alias work --name mywork
# Creates ~/.local/bin/mywork

doppel profile alias work --remove
# Removes the wrapper script
```

## `doppel profile rename`

```bash
doppel profile rename <old-name> <new-name>
```
...
```bash
doppel profile rename mybot assistant
# ~/.doppel/profiles/mybot → ~/.doppel/profiles/assistant
# ~/.local/bin/mybot → ~/.local/bin/assistant
```

## `doppel profile export`

```bash
doppel profile export <name> [options]
```
...
```bash
doppel profile export work
# Creates work.tar.gz in the current directory

doppel profile export work -o ./work-2026-03-29.tar.gz
```

## `doppel profile import`

```bash
doppel profile import <archive> [options]
```
...
```bash
doppel profile import ./work-2026-03-29.tar.gz
# Infers profile name from the archive

doppel profile import ./work-2026-03-29.tar.gz --name work-restored
```
...
:::info
`doppel profile export` / `import` are still the right commands for
...
### `doppel profile install`

```bash
doppel profile install <source> [--name <name>] [--alias] [--force] [--yes]
```
...
| `--alias` | Also create a shell wrapper (e.g. `telemetry` → `doppel -p telemetry`). |
...
doppel profile install github.com/kyle/telemetry-distribution --alias

# Install from a full HTTPS git URL
doppel profile install https://github.com/kyle/telemetry-distribution.git

# Install from SSH
doppel profile install git@github.com:kyle/telemetry-distribution.git

# Install from a local directory during development
doppel profile install ./telemetry/
```

### `doppel profile update`
...
Doppel version, author, env var requirements, the source URL/path, and
...
`doppel profile list` also shows the distribution name and version in a
`Distribution` column, and `doppel profile show <name>` / `delete <name>`
...
doppel profile install git@github.com:your-org/internal-assistant.git

# Uses your git credential helper
doppel profile install https://github.com/your-org/internal-assistant.git
```
...
Doppel version doesn't satisfy the spec.
...
2. Initialize a git repo (or use an existing one) and push to GitHub /
   GitLab / any host Doppel can clone from.
3. Tell recipients to run `doppel profile install <your-repo-url>`.
...
## `doppel -p` / `doppel --profile`

```bash
doppel -p <name> <command> [options]
doppel --profile <name> <command> [options]
```

Global flag to run any Doppel command under a specific profile without changing the sticky default. This overrides the active profile for the duration of the command.
...
```bash
doppel -p work chat -q "Check the server status"
doppel --profile dev gateway start
doppel -p personal skills list
doppel -p work config edit
```
...
- `doppel profile <TAB>` — subcommands (list, use, create, etc.)
- `doppel profile use <TAB>` — profile names
- `doppel -p <TAB>` — profile names
