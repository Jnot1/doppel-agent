...
sidebar_position: 1
title: "CLI Interface"
description: "Master the Doppel terminal interface — commands, keybindings, personalities, and more"
---

# CLI Interface

Doppel's CLI is a full terminal user interface (TUI) — not a web UI. It features multiline editing, slash-command autocomplete, conversation history, interrupt-and-redirect, and streaming tool output. Built for people who live in the terminal.

:::tip First-time setup
One command — `doppel setup --portal` — and you're ready to `doppel chat`. See [Nous Portal](/integrations/nous-portal).
:::

:::tip
Doppel also ships a modern TUI with modal overlays, mouse selection, and non-blocking input. Launch it with `doppel --tui` — see the [TUI](tui.md) guide.
:::
...
doppel chat --model "anthropic/claude-sonnet-4"

# With a specific provider
doppel chat --provider nous        # Use Nous Portal
doppel chat --provider openrouter  # Force OpenRouter
...
doppel chat -s github-pr-workflow -q "open a draft PR"

# Resume previous sessions
doppel --continue             # Resume the most recent CLI session (-c)
doppel --resume <session_id>  # Resume a specific session by ID (-r)

# Verbose mode (debug output)
doppel chat --verbose

# Isolated git worktree (for running multiple agents in parallel)
doppel -w                         # Interactive mode in worktree
doppel -w -q "Fix issue #123"     # Single query in worktree
```

## Interface Layout

<img className="docs-terminal-figure" src="/docs/img/docs/cli-layout.svg" alt="Stylized preview of the Doppel CLI layout showing the banner, conversation area, and fixed input prompt." />
<p className="docs-figure-caption">The Doppel CLI banner, conversation stream, and fixed input prompt rendered as a stable docs figure instead of fragile text art.</p>
...
| ⚠ YOLO | **YOLO mode warning** — shown whenever `HERMES_YOLO_MODE` is on (either `doppel --yolo` at launch or `/yolo` toggled mid-session). Mirrors the banner-line warning so you can't forget you're in auto-approve mode. |
...
When resuming a previous session (`doppel -c` or `doppel --resume <id>`), a "Previous Conversation" panel appears between the banner and the input prompt, showing a compact recap of the conversation history. See [Sessions — Conversation Recap on Resume](sessions.md#conversation-recap-on-resume) for details and configuration.
...
| `Ctrl+D` | Exit |
| `Ctrl+Z` | Suspend Doppel to background (Unix only). Run `fg` in the shell to resume. |
...
Type `/` to see the autocomplete dropdown. Doppel supports a large set of CLI slash commands, dynamic skill commands, and user-defined quick commands.
...
| `/voice tts` | Toggle spoken playback for Doppel replies |
...
quick_commands:
  status:
    type: exec
    command: systemctl --user status hermes-gateway
    # Older installs may still use hermes.service instead.
  gpu:
    type: exec
...
```bash
doppel -s github-auth,github-pr-workflow
doppel chat -s github-pr-workflow -s github-auth
```

Doppel loads each named skill into the session prompt before the first turn. The same flag works in interactive mode and single-query mode.

## Skill Slash Commands

Every installed skill in `~/.doppel/skills/` is automatically registered as a slash command. The skill name becomes the command:
...
You can also define custom personalities in `~/.doppel/config.yaml`:

```yaml
personalities:
  helpful: "You are a helpful, friendly AI assistant."
  kawaii: "You are a kawaii assistant! Use cute expressions..."
  pirate: "Arrr! Ye be talkin' to Captain Doppel..."
  # Add your own!
```
...
Most terminals send the same byte sequence for `Enter` and `Shift+Enter` by default, so applications cannot distinguish them. Doppel recognises `Shift+Enter` only when the terminal sends a distinct sequence via the [Kitty keyboard protocol](https://sw.kovidgoyal.net/kitty/keyboard-protocol/) or xterm's `modifyOtherKeys` mode.
...
Where the terminal cannot distinguish them, `Alt+Enter` and `Ctrl+J` continue to work everywhere. **On Windows Terminal specifically, `Alt+Enter` is captured by the terminal (toggles fullscreen) and never reaches Doppel — use `Ctrl+Enter` (delivered as `Ctrl+J`) or `Ctrl+J` directly for a newline.**
...
The very first time you press Enter while Doppel is working, Doppel prints a one-line reminder explaining the `/busy` knob (`"(tip) Your message interrupted the current run…"`). It only fires once per install — a flag in `config.yaml` under `onboarding.seen.busy_input_prompt` latches it. Delete that key to see the tip again.
:::

### Suspending to Background

On Unix systems, press **`Ctrl+Z`** to suspend Doppel to the background — just like any terminal process. The shell prints a confirmation:
...
Resume this session with:
  doppel --resume 20260225_143052_a1b2c3
...
```bash
doppel --continue                          # Resume the most recent CLI session
doppel -c                                  # Short form
doppel -c "my project"                     # Resume a named session (latest in lineage)
doppel --resume 20260225_143052_a1b2c3     # Resume a specific session by ID
doppel --resume "refactoring auth"         # Resume by title
doppel -r 20260225_143052_a1b2c3           # Short form
```

Resuming restores the full conversation history from SQLite. The agent sees all previous messages, tool calls, and responses — just as if you never left.

Use `/title My Session Name` inside a chat to name the current session, or `doppel sessions rename <id> <title>` from the command line. Use `doppel sessions list` to browse past sessions.

### Session Storage

CLI sessions are stored in Doppel's SQLite state database under `~/.doppel/state.db`. The database keeps:
...
Doppel immediately confirms the task and gives you back the prompt:
