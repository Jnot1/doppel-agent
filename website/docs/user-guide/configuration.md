...
You can set `providers.<id>.request_timeout_seconds` for a provider-wide request timeout, plus `providers.<id>.models.<model>.timeout_seconds` for a model-specific override. Applies to the primary turn client on every transport (OpenAI-wire, native Anthropic, Anthropic-compatible), the fallback chain, rebuilds after credential rotation, and (for OpenAI-wire) the per-request timeout kwarg — so the configured value wins over the `DOPPEL_API_TIMEOUT` env var.

You can also set `providers.<id>.stale_timeout_seconds` for the non-streaming stale-call detector, plus `providers.<id>.models.<model>.stale_timeout_seconds` for a model-specific override. This wins over the `DOPPEL_API_CALL_STALE_TIMEOUT` env var.

Leaving these unset keeps the current defaults (`DOPPEL_API_TIMEOUT=1800`s, `DOPPEL_API_CALL_STALE_TIMEOUT=300`s, native Anthropic 900s). Not currently wired for AWS Bedrock (both `bedrock_converse` and AnthropicBedrock SDK paths use boto3 with its own timeout configuration). See the commented example in [`cli-config.yaml.example`](https://github.com/NousResearch/hermes-agent/blob/main/cli-config.yaml.example).
...
**Requirements:** Docker Desktop or Docker Engine installed and running. Doppel Agent probes `$PATH` plus common macOS install locations (`/usr/local/bin/docker`, `/opt/homebrew/bin/docker`, Docker Desktop app bundle). Podman is supported out of the box: set `DOPPEL_DOCKER_BINARY=podman` (or the full path) to force it when both are installed. Legacy `HERMES_DOCKER_BINARY` still works.
...
| `DOPPEL_DOCKER_BINARY` | _none_ | Force a specific docker/podman binary path (legacy `HERMES_DOCKER_BINARY` still works) |
...
- **SSH** — Both `TERMINAL_SSH_HOST` and `TERMINAL_SSH_USER` must be set. Doppel Agent logs a clear error if either is missing.
- **Modal** — Needs `MODAL_TOKEN_ID` env var or `~/.modal.toml`. Run `doppel doctor` to check.
...
Plugin engines are **never auto-activated** — you must explicitly set `context.engine` to the plugin name. Available engines can be browsed and selected via `doppel plugins` → Provider Plugins → Context Engine.
...
`agent.api_max_retries` controls how many times Doppel Agent retries a provider API call on transient errors (rate limits, connection drops, 5xx) **before** fallback-provider switching engages. The default is `3` — four attempts total. If you have [fallback providers](/user-guide/features/fallback-providers) configured and want to fail over faster, drop this to `0` so the first transient error on your primary immediately hands off to the fallback instead of churning retries against the flaky endpoint.

### API Timeouts

Doppel Agent has separate timeout layers for streaming, plus a stale detector for non-streaming calls. The stale detectors auto-adjust for local providers only when you leave them at their implicit defaults.

| Timeout | Default | Local providers | Config / env |
|---------|---------|----------------|--------------|
| Socket read timeout | 120s | Auto-raised to 1800s | `DOPPEL_STREAM_READ_TIMEOUT` |
| Stale stream detection | 180s | Auto-disabled | `DOPPEL_STREAM_STALE_TIMEOUT` |
| Stale non-stream detection | 300s | Auto-disabled when left implicit | `providers.<id>.stale_timeout_seconds` or `DOPPEL_API_CALL_STALE_TIMEOUT` |
| API call (non-streaming) | 1800s | Unchanged | `providers.<id>.request_timeout_seconds` / `timeout_seconds` or `DOPPEL_API_TIMEOUT` |

The **socket read timeout** controls how long httpx waits for the next chunk of data from the provider. Local LLMs can take minutes for prefill on large contexts before producing the first token, so Doppel Agent raises this to 30 minutes when it detects a local endpoint. If you explicitly set `DOPPEL_STREAM_READ_TIMEOUT`, that value is always used regardless of endpoint detection.
...
The **stale non-stream detection** kills non-streaming calls that produce no response for too long. By default Doppel Agent disables this on local endpoints to avoid false positives during long prefills. If you explicitly set `providers.<id>.stale_timeout_seconds`, `providers.<id>.models.<model>.stale_timeout_seconds`, or `DOPPEL_API_CALL_STALE_TIMEOUT`, that explicit value is honored even on local endpoints.
...
Doppel Agent turns on cross-session prompt caching automatically when the active provider supports it — no user config needed.

For Claude on **native Anthropic**, **OpenRouter**, and **Nous Portal**, Doppel Agent attaches `cache_control` breakpoints with the 1-hour TTL (`ttl: "1h"`) on the system prompt and skill blocks. The first send within a fresh hour pays full input rates; subsequent sends across any session within the same hour pull from the cache at the discounted cached-read rate. This means the system prompt, loaded skill content, and the early portion of any long-context include get reused across Doppel sessions and across forked subagents for the first hour.

The Qwen Cloud (Alibaba DashScope) upstream caps cache TTL at 5 minutes, so Doppel Agent uses the 5-minute breakpoint TTL there instead. Other Claude-via-third-party paths (AWS Bedrock, Azure Foundry) fall back to the provider's own caching defaults. xAI Grok uses a separate session-pinned conversation-id mechanism — see [xAI prompt caching](/integrations/providers#xai-grok--responses-api--prompt-caching).
...
Every model slot in Doppel Agent — auxiliary tasks, compression, fallback — uses the same three knobs:
...
When `base_url` is set, Doppel Agent ignores the provider and calls that endpoint directly (using `api_key` or `OPENAI_API_KEY` for auth). When only `provider` is set, Doppel Agent uses that provider's built-in auth and base URL.
...
:::tip MiniMax OAuth
`minimax-oauth` logs in via browser OAuth (no API key needed). Run `doppel model` and select **MiniMax (OAuth)** to authenticate. Auxiliary tasks use `MiniMax-M2.7-highspeed` automatically. See the [MiniMax OAuth guide](../guides/minimax-oauth.md).
:::

:::tip xAI Grok OAuth
`xai-oauth` logs in via browser OAuth for SuperGrok and X Premium+ subscribers (no API key needed). Run `doppel model` and select **xAI Grok OAuth (SuperGrok / Premium+)** to authenticate. The same OAuth token is reused for every direct-to-xAI surface (chat, auxiliary tasks, TTS, image gen, video gen, transcription). See the [xAI Grok OAuth guide](../guides/xai-grok-oauth.md), and if Doppel Agent is on a remote host see [OAuth over SSH / Remote Hosts](../guides/oauth-over-ssh.md).
:::
...
### Auxiliary config reference {#auxiliary-config-reference}
...
auxiliary:
  ...
  # Context compression model/provider (separate from compression.* thresholds)
  compression:
    provider: "auto"
    model: ""
    base_url: ""
  ...
  # Kanban triage specifier — `doppel kanban specify <id>` (or the
  ...
  # Kanban task decomposition — `doppel kanban decompose <id>` (or the
  # dashboard's ⚗ Decompose button on Triage-column cards) uses this
  # slot to fan a task out into child tasks routed to specialist profiles.
  kanban_decomposer:
    provider: "auto"
    model: ""
    base_url: ""
  ...
  # Profile description auto-generation — `doppel profile describe <name> --auto`
  profile_describer:
    provider: "auto"
    model: ""
    base_url: ""
  ...
  # Background skill maintenance review
  curator:
    provider: "auto"
    model: ""
    base_url: ""
...
The shape mirrors what OpenRouter accepts in the chat completions request body. Doppel Agent forwards the entire `extra_body` verbatim, so any other OpenRouter request-body field documented at [openrouter.ai/docs](https://openrouter.ai/docs) works the same way.
...
Or via environment variable (in `~/.doppel/.env`; legacy `~/.hermes/.env` also works):
...
|----------|-------------|-------------|
| `"auto"` | Best available (default). Uses your main model first; vision then falls back through OpenRouter → Nous → custom → Codex → API-key providers when it needs a multimodal route. | — |
| `"openrouter"` | Force OpenRouter — routes to any model (Gemini, GPT-4o, Claude, etc.) | `OPENROUTER_API_KEY` |
| `"nous"` | Force Nous Portal | `doppel auth` |
| `"codex"` | Force Codex OAuth (ChatGPT account). Supports vision (gpt-5.3-codex). | `doppel model` → Codex |
| `"minimax-oauth"` | Force MiniMax OAuth (browser login, no API key). Uses MiniMax-M2.7-highspeed for auxiliary tasks. | `doppel model` → MiniMax (OAuth) |
| `"xai-oauth"` | Force xAI Grok OAuth (browser login for SuperGrok or X Premium+ subscribers, no API key). Same OAuth token covers chat, TTS, image, video, and transcription. | `doppel model` → xAI Grok OAuth (SuperGrok / Premium+) |
| `"main"` | Use your active custom/main endpoint. This can come from `OPENAI_BASE_URL` + `OPENAI_API_KEY` or from a custom endpoint saved via `doppel model` / `config.yaml`. Works with OpenAI, local models, or any OpenAI-compatible API. **Auxiliary tasks only — not valid for `model.provider`.** | Custom endpoint credentials + base URL |
...
`base_url` takes precedence over `provider`, so this is the most explicit way to route an auxiliary task to a specific endpoint. For direct endpoint overrides, Doppel Agent uses the configured `api_key` or falls back to `OPENAI_API_KEY`; it does not reuse `OPENROUTER_API_KEY` for that custom endpoint.

**Using OpenAI API key for vision:**
```yaml
# In ~/.doppel/.env (legacy ~/.hermes/.env also works):
...
```
Run `doppel model` and select **MiniMax (OAuth)** to log in and set this automatically. For the China region, the base URL will be `https://api.minimaxi.com/anthropic`. See the [MiniMax OAuth guide](../guides/minimax-oauth.md) for the full walkthrough.
...
`provider: "main"` uses whatever provider Doppel Agent uses for normal chat — whether that's a named custom provider (e.g. `beans`), a built-in provider like `openrouter`, or a legacy `OPENAI_BASE_URL` endpoint.
...
| Approval model | `AUXILIARY_APPROVAL_MODEL` |
| Approval endpoint | `AUXILIARY_APPROVAL_BASE_URL` |
| Approval API key | `AUXILIARY_APPROVAL_API_KEY` |

Compression and fallback model settings are config.yaml-only.

:::tip
Run `doppel config` to see your current auxiliary model settings. Overrides only show up when they differ from the defaults.
:::
...
When `display.file_mutation_verifier` is `true` (default), Doppel Agent appends a one-line advisory to the assistant's final response whenever a `write_file` or `patch` call failed during the turn and was never superseded by a successful write to the same path. This catches the "batch of parallel patches, half silently fail, model summarises success" class of over-claim without requiring you to manually run `git status` after every edit.
...
When `display.runtime_footer.enabled: true`, Doppel Agent appends a small runtime-context footer to the **final** message of each gateway turn — same info the CLI shows in its status bar (model, context %, cwd, session duration, tokens, cost). Off by default; opt in per-gateway if your team wants every reply to include the provenance.
...
`interim_assistant_messages` is gateway-only. When enabled, Doppel Agent sends completed mid-turn assistant updates as separate chat messages. This is independent from `tool_progress` and does not require gateway streaming.
...
If the requested provider is unavailable, Doppel Agent falls back automatically in this order: `local` → `groq` → `openai`.
...
- `false` reverts to the old shared-room behavior. That can be useful if you explicitly want Doppel Agent to treat a channel like one collaborative conversation, but it also means users share context, token costs, and interrupt state.
- Direct messages are unaffected. Doppel Agent still keys DMs by chat/DM ID as usual.
...
- `pair` is the default. Doppel Agent denies access, but replies with a one-time pairing code in DMs.
...
- **`strict`** — scripts run in a temp staging directory with `sys.executable` (Doppel Agent's own python). Maximum reproducibility, but project deps and relative paths won't resolve.
...
The `web_search` and `web_extract` tools support five backend providers. Configure the backend in `config.yaml` or via `doppel tools`:
...
browser:
  ...
  # Optional CDP override — when set, Doppel Agent attaches directly to your own
  ...
  camofox:
    ...
    session_key: ""              # Optional session key sent when Doppel Agent creates a tab
...
Control how Doppel Agent handles potentially dangerous commands:
...
```yaml
checkpoints:
  enabled: false                 # Enable automatic checkpoints (also: doppel chat --checkpoints). Default: false (opt-in).
...
Doppel Agent uses two different context scopes:
...
| `.doppel.md` / `DOPPEL.md` | Project-specific instructions (highest priority) | Walks to git root |
...
- **Project context files use a priority system** — only ONE type is loaded (first match wins): `.doppel.md` → `AGENTS.md` → `CLAUDE.md` → `.cursorrules`. SOUL.md is always loaded independently.
- **AGENTS.md** is hierarchical: if subdirectories also have AGENTS.md, all are combined.
- Doppel Agent automatically seeds a default `SOUL.md` if one does not already exist.
...
| Context | Default |
|---------|---------|
| **CLI (`doppel`)** | Current directory where you run the command |
...
`MESSAGING_CWD` and direct `TERMINAL_CWD` entries in `~/.doppel/.env` remain supported, and legacy `~/.hermes/.env` installs still work. New configurations should use `terminal.cwd`.
