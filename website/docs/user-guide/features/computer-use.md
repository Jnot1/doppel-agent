...
Doppel Agent can drive your Mac's desktop — clicking, typing, scrolling,
...
Use `doppel computer-use status` to verify the install.

**Option 2: enable the toolset interactively.**

1. Run `doppel tools`, pick `🖱️ Computer Use (macOS)` → `cua-driver (background)`.
...
4. Start a session with the toolset enabled:
   ...
   ```
   or add `computer_use` to your enabled toolsets in `~/.doppel/config.yaml`.
   Legacy installs may still keep the same setting in `~/.hermes/config.yaml`.

## Keeping cua-driver up to date

The cua-driver project ships fixes regularly (e.g. v0.1.6 fixed a Safari
window-focus bug for UTM workflows). Doppel refreshes the binary in two
places so you don't get stuck on a stale release:

- **`doppel update`** — when you update Doppel Agent itself, if `cua-driver` is
...
- **`doppel computer-use install --upgrade`** — manual force-refresh.
...
`doppel computer-use status` shows the installed version next to the
binary path.
...
Doppel Agent applies multi-layer guardrails:
...
Pair with `approvals.mode: manual` in `~/.doppel/config.yaml` if you want every action confirmed.
Legacy installs may still keep that setting in `~/.hermes/config.yaml`.

## Token efficiency

Screenshots are expensive. Doppel Agent applies three layers of optimisation:
...
`doppel computer-use install` to fetch the cua-driver binary, or run
`doppel tools` and enable the Computer Use toolset.
...
- [Universal skill: `macos-computer-use`](https://github.com/Jnot1/doppel-agent/blob/main/skills/apple/macos-computer-use/SKILL.md)
