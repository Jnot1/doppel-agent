...
sidebar_position: 8
title: "Context Files"
description: "Project context files — .doppel.md, AGENTS.md, CLAUDE.md, global SOUL.md, and .cursorrules — automatically injected into every Doppel Agent conversation"
---
...
|------|---------|-----------| 
| **.doppel.md** / **DOPPEL.md** | Project instructions (highest priority) | Walks to git root |
...
Only **one** project context type is loaded per session (first match wins): `.doppel.md` → `AGENTS.md` → `CLAUDE.md` → `.cursorrules`. **SOUL.md** is always loaded independently as the agent identity (slot #1).
:::
...
Doppel Agent is compatible with Cursor IDE's `.cursorrules` file and `.cursor/rules/*.mdc` rule modules. If these files exist in your project root and no higher-priority context file (`.doppel.md`, `AGENTS.md`, or `CLAUDE.md`) is found, they're loaded as the project context.
...
1. **Scan working directory** — checks for `.doppel.md` → `AGENTS.md` → `CLAUDE.md` → `.cursorrules` (first match wins)
