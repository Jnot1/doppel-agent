...
When Doppel Agent runs inside a messaging gateway (Slack, Discord, Telegram,
...
persona in `~/.doppel/SOUL.md`, or as a named preset under
`agent.personalities` in `~/.doppel/config.yaml` (switchable per session
via `/personality`).
...
If you use Doppel Agent's kanban multi-agent workflow, workers can attach
...
back into the thread as a native attachment. Doppel Agent's deliverable
...
OAuth tokens stay on the user's machine in `~/.doppel/auth.json` / `~/.doppel/.env`. No hosted
