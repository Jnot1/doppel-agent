...
Doppel Agent runs full language servers — pyright, gopls, rust-analyzer,
...
This is the same architecture top-tier coding agents use. Doppel Agent
...
1. Doppel Agent captures a baseline of current diagnostics for the file.
...
| Nix | `nixd` | manual |
| Zig | `zls` | manual |
| Gleam | `gleam` (`gleam lsp`) | manual (gleam install) |
...
…). Doppel Agent auto-detects the binary on PATH or in
`<DOPPEL_HOME>/lsp/bin/` (legacy `<HERMES_HOME>/lsp/bin/` still works).
...
`node_modules` tree — Doppel Agent installs both packages together when you
run `doppel lsp install typescript` or auto-install fires on first
use.

## CLI

```
doppel lsp status          # service state + per-server install status
doppel lsp list            # registry, optionally --installed-only
doppel lsp install <id>    # eagerly install one server
doppel lsp install-all     # try every server with a known recipe
doppel lsp restart         # tear down running clients
doppel lsp which <id>      # print resolved binary path
```

`doppel lsp status` is the best starting point — it shows which
...
lsp:
  ...
  #   auto    — install via npm/pip/go install into <DOPPEL_HOME>/lsp/bin
  #   manual  — only use binaries already on PATH
  #   off     — same as manual today; reserved for a stricter future mode
...
* Server IDs match `doppel lsp list` / the `servers:` map keys
  (for example `dockerfile-ls` and `gleam`).

## Installation locations

When `install_strategy: auto`, Doppel Agent installs binaries into
`<DOPPEL_HOME>/lsp/bin/`. NPM packages land in
`<DOPPEL_HOME>/lsp/node_modules/` with bin symlinks one level up.
Go binaries come from `go install` with `GOBIN` pointed at the
staging dir.

Nothing is ever installed to `/usr/local/`, `~/.local/`, or any other
shared location — the staging dir is fully Doppel-owned and is
...
Servers are kept alive for the life of the Doppel Agent process. There's
...
**`doppel lsp status` shows a server as "missing"**

The binary isn't on PATH and isn't in `<DOPPEL_HOME>/lsp/bin/`. Run
`doppel lsp install <server_id>` to attempt an auto-install, or
install the binary manually through the language's normal toolchain.

**`Backend warnings` section in `doppel lsp status`**
...
When `doppel lsp status` shows a `Backend warnings` section, install
...
`~/.doppel/logs/agent.log`. Legacy installs may still log to
`~/.hermes/logs/agent.log`.

**Server starts but never returns diagnostics**

Check `~/.doppel/logs/agent.log` for `[agent.lsp.client]` entries —
both stderr from the language server and protocol errors land
there. Legacy installs may still write to `~/.hermes/logs/agent.log`.
Some servers (rust-analyzer especially) need to finish a
...
the rest of the session. Run `doppel lsp restart` to clear the set;
