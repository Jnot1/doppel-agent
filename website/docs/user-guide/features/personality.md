...
description: "Customize Doppel Agent's personality with a global SOUL.md, built-in personalities, and custom persona definitions"
---

# Personality & SOUL.md

Doppel Agent's personality is fully customizable. `SOUL.md` is the **primary identity** — it's the first thing in the system prompt and defines who the agent is.

- `SOUL.md` — a durable persona file that lives in the resolved Doppel home root and serves as the agent's identity (slot #1 in the system prompt)
- built-in or custom `/personality` presets — session-level system-prompt overlays

If you want to change who Doppel Agent is — or replace it with an entirely different agent persona — edit `SOUL.md`.

## How SOUL.md works now

Doppel Agent now seeds a default `SOUL.md` automatically in:
...
If Doppel Agent loaded `SOUL.md` from whatever directory you happened to launch it in, your personality could change unexpectedly between projects. By loading only from the resolved home root, the personality belongs to the Doppel Agent instance itself.
...
- how Doppel Agent should handle uncertainty, disagreement, or ambiguity
...
## What Doppel Agent injects into the prompt
...
If the file is empty, whitespace-only, or cannot be read, Doppel Agent falls back to a built-in default identity ("You are Hermes Agent, an intelligent AI assistant created by Nous Research..."). This fallback also applies when `skip_context_files` is set (e.g., in subagent/delegation contexts).
...
Doppel Agent ships with built-in personalities you can switch to with `/personality`.
...
| **pirate** | Captain Doppel, tech-savvy buccaneer |
...
These are convenient overlays, but your global `SOUL.md` still gives Doppel Agent its persistent default personality unless the overlay meaningfully changes it.
...
1. Keep a thoughtful global `SOUL.md` in `~/.doppel/SOUL.md`
...
- `SOUL.md`, `agent.system_prompt`, and `/personality` affect how Doppel Agent speaks
- `display.skin` and `/skin` affect how Doppel Agent looks in the terminal
