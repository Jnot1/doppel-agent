...
title: "Plugins"
description: "Extend Doppel Agent with custom tools, hooks, and integrations via the plugin system"
---

# Plugins

Doppel Agent has a plugin system for adding custom tools, hooks, and integrations without modifying core code.
...
[Adding Tools](/developer-guide/adding-tools) page is for built-in Doppel
core tools that live in `tools/` and `toolsets.py`.

**→ [Build a Doppel Plugin](/guides/build-a-doppel-plugin)** — step-by-step guide with a complete working example.
...
Start Doppel — your tools appear alongside built-in tools. The model can call them immediately.
...
| Add CLI commands | `ctx.register_cli_command(name, help, setup_fn, handler_fn)` — adds `doppel <plugin> <subcommand>` |
...
Within each source, Doppel Agent also recognizes sub-category directories that route plugins to specialized discovery systems:
...
```bash
doppel plugins                    # interactive toggle (space to check/uncheck)
doppel plugins enable <name>      # add to allow-list
doppel plugins disable <name>     # remove from allow-list + add to disabled
```

After `doppel plugins install owner/repo`, you're asked `Enable 'name' now? [y/N]` — defaults to no. Skip the prompt for scripted installs with `--enable` or `--no-enable`.

### What the allow-list does NOT gate

Several categories of plugin bypass `plugins.enabled` — they're part of Doppel Agent's built-in surface and would break basic functionality if gated off by default:
...
Doppel Agent has four kinds of plugins:
...
The table above shows the four plugin categories, but within "General plugins" the `PluginContext` exposes several distinct extension points — and Doppel also accepts extensions outside the Python plugin system (config-driven backends, shell-hooked commands, external servers, etc.). Use this table to find the right doc for what you want to build:

| Want to add… | How | Authoring guide |
|---|---|---|
| A **tool** the LLM can call | Python plugin — `ctx.register_tool()` | [Build a Doppel Plugin](/guides/build-a-doppel-plugin) · [Adding Tools](/developer-guide/adding-tools) |
| A **lifecycle hook** (pre/post LLM, session start/end, tool filter) | Python plugin — `ctx.register_hook()` | [Hooks reference](/user-guide/features/hooks) · [Build a Doppel Plugin](/guides/build-a-doppel-plugin) |
| A **slash command** for the CLI / gateway | Python plugin — `ctx.register_command()` | [Build a Doppel Plugin](/guides/build-a-doppel-plugin) · [Extending the CLI](/developer-guide/extending-the-cli) |
| A **subcommand** for `doppel <thing>` | Python plugin — `ctx.register_cli_command()` | [Extending the CLI](/developer-guide/extending-the-cli) |
...
On NixOS, plugins can be installed declaratively via the module options — no `doppel plugins install` needed. See the **[Nix Setup guide](/getting-started/nix-setup#plugins)** for full details.
...
```bash
doppel plugins                                       # unified interactive UI
doppel plugins list                                  # table: enabled / disabled / not enabled
doppel plugins install user/repo                     # install from Git, then prompt Enable? [y/N]
doppel plugins install user/repo --enable            # install AND enable (no prompt)
doppel plugins install user/repo --no-enable         # install but leave disabled (no prompt)
doppel plugins update my-plugin                      # pull latest
doppel plugins remove my-plugin                      # uninstall
doppel plugins enable my-plugin                      # add to allow-list (flat plugin)
doppel plugins enable observability/langfuse         # add to allow-list (sub-category plugin)
doppel plugins disable my-plugin                     # remove from allow-list + add to disabled
```

For plugins under a sub-category directory (e.g. `plugins/observability/langfuse/`, `plugins/image_gen/openai/`), use the full `<category>/<plugin>` key — that's exactly what `doppel plugins list` shows in the **Name** column.

### Interactive UI

Running `doppel plugins` with no arguments opens a composite interactive screen:
...
The default for a newly-installed or bundled plugin is `not enabled`. `doppel plugins list` shows all three distinct states so you can tell what's been explicitly turned off vs. what's just waiting to be enabled.
...
See the **[full guide](/guides/build-a-doppel-plugin)** for handler contracts, schema format, hook behavior, error handling, and common mistakes.
