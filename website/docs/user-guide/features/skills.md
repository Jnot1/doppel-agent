...
Skills are on-demand knowledge documents Doppel Agent can load when needed. They follow a **progressive disclosure** pattern to minimize token usage and are compatible with the [agentskills.io](https://agentskills.io/specification) open standard.

All skills live in **`~/.doppel/skills/`** — the primary directory and source of truth. On fresh install, bundled skills are copied from the repo. Hub-installed and agent-created skills also go here. The agent can modify or delete any skill.

You can also point Doppel Agent at **external skill directories** — additional folders scanned alongside the local one. See [External Skill Directories](#external-skill-directories) below.
...
The bundled `plan` skill is a good example. Running `/plan [request]` loads the skill's instructions, telling Doppel Agent to inspect context if needed, write a markdown implementation plan instead of executing the task, and save the result under `.hermes/plans/` relative to the active workspace/backend working directory.

You can also interact with skills through natural conversation:

```bash
doppel chat --toolsets skills -q "What skills do you have?"
doppel chat --toolsets skills -q "Show me the axolotl skill"
```
...
When a missing value is encountered, Doppel Agent asks for it securely only when the skill is actually loaded in the local CLI. You can skip setup and keep using the skill. Messaging surfaces never ask for secrets in chat — they tell you to use `doppel setup` or `~/.doppel/.env` locally instead.
...
Settings are stored under `skills.config` in your config.yaml. `doppel config migrate` prompts for unconfigured settings, and `doppel config show` displays them. When a skill loads, its resolved config values are injected into the context so the agent knows the configured values automatically.
...
```text
~/.doppel/skills/                  # Single source of truth
...
If you maintain skills outside of Doppel Agent — for example, a shared `~/.agents/skills/` directory used by multiple AI tools — you can tell Doppel Agent to scan those directories too.

Add `external_dirs` under the `skills` section in `~/.doppel/config.yaml`:
...
- **Create locally, update in place**: New agent-created skills are written to `~/.doppel/skills/`. Existing skills are modified where they are found, including skills under `external_dirs`, when the agent uses `skill_manage` actions such as `patch`, `edit`, `write_file`, `remove_file`, or `delete`.
- **External dirs are not a write-protection boundary**: If an external skill directory is writable by the Doppel Agent process, agent-managed skill updates can change files in that directory. Use filesystem permissions or a separate profile/toolset setup if shared external skills must stay read-only.
- **Local precedence**: If the same skill name exists in both the local dir and an external dir, the local version wins.
- **Full integration**: External skills appear in the system prompt index, `skills_list`, `skill_view`, and as `/skill-name` slash commands — no different from local skills.
- **Non-existent paths are silently skipped**: If a configured directory doesn't exist, Doppel Agent ignores it without errors. Useful for optional shared directories that may not be present on every machine.

### Example

```text
~/.doppel/skills/               # Local (primary, read-write)
├── devops/deploy-k8s/
│   └── SKILL.md
└── mlops/axolotl/
    └── SKILL.md
...
Bundles live in **`~/.doppel/skill-bundles/<slug>.yaml`** and look like this:
...
- `description` (optional) — short text shown in `/bundles` and `doppel bundles list`.
...
doppel bundles create research

# Overwrite an existing bundle
doppel bundles create backend-dev --skill ... --force

# Delete a bundle
doppel bundles delete backend-dev

# Re-scan ~/.doppel/skill-bundles/ and report changes
doppel bundles reload
```
...
- You want to ship a team-wide "task profile" by checking the bundle YAML into a shared dotfiles repo and symlinking it into `~/.doppel/skill-bundles/`.

A bundle is just a YAML alias — it doesn't install skills for you. The skills themselves must already be present (in `~/.doppel/skills/` or an external skill directory). Otherwise the bundle invocation just skips the missing ones.
...
```bash
doppel skills browse                              # Browse all hub skills (official first)
doppel skills browse --source official            # Browse only official optional skills
doppel skills search kubernetes                   # Search all sources
doppel skills search react --source skills-sh     # Search the skills.sh directory
doppel skills search https://mintlify.com/docs --source well-known
doppel skills inspect openai/skills/k8s           # Preview before installing
doppel skills install openai/skills/k8s           # Install with security scan
doppel skills install official/security/1password
doppel skills install skills-sh/vercel-labs/json-render/json-render-react --force
doppel skills install well-known:https://mintlify.com/docs/.well-known/skills/mintlify
...
doppel skills list --source hub                   # List hub-installed skills
doppel skills check                               # Check installed hub skills for upstream updates
doppel skills update                              # Reinstall hub skills with upstream changes when needed
doppel skills audit                               # Re-scan all hub skills for security
doppel skills uninstall k8s                       # Remove a hub skill
doppel skills reset google-workspace              # Un-stick a bundled skill from "user-modified" (see below)
doppel skills reset google-workspace --restore    # Also restore the bundled version, deleting your local edits
doppel skills publish skills/my-skill --to github --repo owner/repo
doppel skills snapshot export setup.json          # Export skill config
doppel skills tap add myorg/skills-repo           # Add a custom GitHub source
```
...
|--------|---------|-------|
| `official` | `official/security/1password` | Optional skills shipped with Doppel Agent. |
| `skills-sh` | `skills-sh/vercel-labs/agent-skills/vercel-react-best-practices` | Searchable via `doppel skills search <query> --source skills-sh`. Doppel Agent resolves alias-style skills when the skills.sh slug differs from the repo folder. |
...
Doppel Agent currently integrates with these skills ecosystems and discovery sources:

#### 1. Official optional skills (`official`)

These are maintained in the doppel-agent repository itself and install with built-in trust.
...
This is Vercel's public skills directory. Doppel Agent can search it directly, inspect skill detail pages, resolve alias-style slugs, and install from the underlying source repo.
...
```bash
doppel skills search https://mintlify.com/docs --source well-known
doppel skills inspect well-known:https://mintlify.com/docs/.well-known/skills/mintlify
doppel skills install well-known:https://mintlify.com/docs/.well-known/skills/mintlify
```

#### 4. Direct GitHub skills (`github`)

Doppel Agent can install directly from GitHub repositories and GitHub-based taps. This is useful when you already know the repo/path or want to add your own custom source repo.
...
doppel skills tap add myorg/skills-repo
```
...
categorization, no Doppel-Agent-side changes required.
...
- Doppel source id: `clawhub`

#### 6. Claude marketplace-style repos (`claude-marketplace`)

Doppel Agent supports marketplace repos that publish Claude-compatible plugin/marketplace manifests.

Known integrated sources include:
- [anthropics/skills](https://github.com/anthropics/skills)
- [aiskillstore/marketplace](https://github.com/aiskillstore/marketplace)

Doppel source id: `claude-marketplace`

#### 7. LobeHub (`lobehub`)

Doppel Agent can search and convert agent entries from LobeHub's public catalog into installable skills for Doppel Agent.

- Site: [LobeHub](https://lobehub.com/)
- Public agents index: [chat-agents.lobehub.com](https://chat-agents.lobehub.com/)
- Backing repo: [lobehub/lobe-chat-agents](https://github.com/lobehub/lobe-chat-agents)
- Doppel source id: `lobehub`

#### 8. browse.sh (`browse-sh`)

Doppel Agent integrates with [browse.sh](https://browse.sh), Browserbase's catalog of 200+ site-specific browser-automation SKILL.md files (Airbnb, Amazon, arXiv, 12306.cn, Etsy, Xero, and many more). Each skill describes how to drive one website end-to-end and is suitable for use with Doppel Agent's browser tools and any browser-automation skills you already have installed.

- Site: [browse.sh](https://browse.sh/)
- Catalog API: `https://browse.sh/api/skills`
- Doppel source id: `browse-sh`
- Trust level: `community`

```bash
doppel skills search airbnb --source browse-sh
doppel skills inspect browse-sh/airbnb.com/search-listings-ddgioa
doppel skills install browse-sh/airbnb.com/search-listings-ddgioa
```
...
Install a single-file `SKILL.md` directly from any HTTP(S) URL — useful when an author hosts a skill on their own site (no hub listing, no GitHub path to type). Doppel Agent fetches the URL, parses the YAML frontmatter, security-scans it, and installs.

- Doppel source id: `url`
...
```bash
doppel skills install https://sharethis.chat/SKILL.md
doppel skills install https://example.com/my-skill/SKILL.md --category productivity
```
...
doppel skills install https://example.com/SKILL.md --name sharethis-chat

# Or inside a chat session:
/skills install https://example.com/SKILL.md --name sharethis-chat
```

Trust level is always `community` — the same security scan runs as for every other source. The URL is stored as the install identifier, so `doppel skills update` re-fetches from the same URL automatically when you want to refresh.
...
`doppel skills inspect ...` now also surfaces upstream metadata when available:
- repo URL
...
| Level | Source | Policy |
|-------|--------|--------|
| `builtin` | Ships with Doppel Agent | Always trusted |
...
```bash
doppel skills check          # Report which installed hub skills changed upstream
doppel skills update         # Reinstall only the skills with updates available
doppel skills update react   # Update one specific installed hub skill
```
...
If you want to share a curated set of skills — for your team, your org, or publicly — you can publish them as a **tap**: a GitHub repository other Doppel Agent users add with `doppel skills tap add <owner/repo>`. No server, no registry sign-up, no release pipeline. Just a directory of `SKILL.md` files.
...
Doppel Agent discovers skills by listing every subdirectory of the tap path and probing each for `SKILL.md`.

#### Minimal tap example

```
my-org/doppel-skills
└── skills/
    └── deploy-runbook/
        └── SKILL.md
```
...
After pushing that to GitHub, any Doppel Agent user can subscribe and install:

```bash
doppel skills tap add my-org/doppel-skills
doppel skills search deploy
doppel skills install my-org/doppel-skills/deploy-runbook
```

#### Non-default paths

If your skills don't live under `skills/` (common when you're adding a `skills/` subtree to an existing project), edit the tap entry in `~/.doppel/.hub/taps.json`:
...
The `doppel skills tap add` CLI defaults new taps to `path: "skills/"`; edit the file directly if you need a different path. `doppel skills tap list` shows the effective path per tap.
...
```bash
doppel skills install owner/repo/skills/my-workflow
```
...
Taps are stored in `~/.doppel/.hub/taps.json` (created on demand).

## Bundled skill updates (`doppel skills reset`)

Doppel Agent ships with a set of bundled skills in `skills/` inside the repo. On install and on every `doppel update`, a sync pass copies those into `~/.doppel/skills/` and records a manifest at `~/.doppel/skills/.bundled_manifest` mapping each skill name to the content hash at the time it was synced (the **origin hash**).

On each sync, Doppel Agent recomputes the hash of your local copy and compares it to the origin hash:
...
doppel skills reset google-workspace --restore
...
:::note Profiles
Each profile has its own `.bundled_manifest` under its own `HERMES_HOME`, so `doppel -p coder skills reset <name>` only affects that profile.
:::
