...
sidebar_position: 10
title: "Skins & Themes"
description: "Customize the Doppel CLI with built-in and user-defined skins"
---

# Skins & Themes

Skins control the **visual presentation** of the Doppel CLI: banner colors, spinner faces and verbs, response-box labels, branding text, and the tool activity prefix.
...
/skin mytheme        # switch to a custom skin from ~/.doppel/skins/mytheme.yaml
```

Or set the default skin in `~/.doppel/config.yaml` (legacy installs may still use `~/.hermes/config.yaml`):
...
|------|-------------|----------------|------------------|
| `default` | Classic Doppel — gold and kawaii | `Doppel Agent` | Warm gold borders, cornsilk text, kawaii faces in spinners. The familiar caduceus banner. Clean and inviting. |
...
| `mono` | Monochrome — clean grayscale | `Doppel Agent` | All grays — no color. Borders are `#555555`, text is `#c9d1d9`. Ideal for minimal terminal setups or screen recordings. |
| `slate` | Cool blue — developer-focused | `Doppel Agent` | Royal blue borders (`#4169e1`), soft blue text. Calm and professional. No custom spinner — uses default faces. |
| `daylight` | Light theme for bright terminals with dark text and cool blue accents | `Doppel Agent` | Designed for white or bright terminals. Dark slate text with blue borders, pale status surfaces, and a light completion menu that stays readable in light terminal profiles. |
| `warm-lightmode` | Warm brown/gold text for light terminal backgrounds | `Doppel Agent` | Warm parchment tones for light terminals. Dark brown text with saddle-brown accents, cream-colored status surfaces. An earthy alternative to the cooler daylight theme. |
...
|-----|-------------|---------|
| `agent_name` | Name shown in banner title and status display | `Doppel Agent` |
| `welcome` | Welcome message shown at CLI startup | `Welcome to Doppel Agent! Type your message or /help for commands.` |
| `goodbye` | Message shown on exit | `Goodbye! ⚕` |
| `response_label` | Label on the response box header | ` ⚕ Doppel ` |
...
| `banner_logo` | string | Rich-markup ASCII art logo (replaces the default Doppel banner) | `""` |
| `banner_hero` | string | Rich-markup hero art (replaces the default caduceus art) | `""` |

## Custom skins

Create YAML files under `~/.doppel/skins/`. User skins inherit missing values from the built-in `default` skin, so you only need to specify the keys you want to change. Legacy installs may still keep the same files under `~/.hermes/skins/`.

### Full custom skin YAML template

```yaml
# ~/.doppel/skins/mytheme.yaml
...
- Opens any skin into a visual editor with all Doppel skin fields (colors, spinner, branding, tool prefix, tool emojis)
...
- Saves directly to `~/.doppel/skins/`
- Activates a skin by updating `~/.doppel/config.yaml`
...
6. Click **Save** to write the skin YAML to `~/.doppel/skins/`.
7. Click **Activate** to set it as the current skin (updates `display.skin` in `~/.doppel/config.yaml`).

Hermes Mod respects the `HERMES_HOME` environment variable, so it works with [profiles](/user-guide/profiles) too. Fresh installs usually resolve that through `DOPPEL_HOME` / `~/.doppel`, while legacy setups may still use `~/.hermes`.
...
- User skins in `~/.doppel/skins/` take precedence over built-in skins with the same name. Legacy installs may still keep those files under `~/.hermes/skins/`.
- Skin changes via `/skin` are session-only. To make a skin your permanent default, set it in `~/.doppel/config.yaml` (or the matching legacy home).
