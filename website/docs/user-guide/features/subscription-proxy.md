...
chat completions — use your Doppel-managed provider subscription as their
...
| Use case | "Use Doppel Agent as a chat backend" | "Use my Portal sub from another app" |
...
This opens your browser for the Nous Portal OAuth flow. Doppel Agent stores
the refresh token in `~/.doppel/auth.json` — the same place all Doppel
provider logins live. Legacy installs may still keep this in `~/.hermes/auth.json`.
...
If you see `not logged in`, run `doppel auth add nous`. If you see
