...
sidebar_position: 10
title: "Voice Mode"
description: "Real-time voice conversations with Doppel Agent — CLI, Telegram, Discord (DMs, text channels, and voice channels)"
---
...
:::tip
The preferred `~/.doppel/` directory and default `config.yaml` are created automatically the first time you run `doppel`. Legacy installs may still keep the same files in `~/.hermes/`. You only need to create `~/.doppel/.env` manually for API keys on a fresh install.
:::

:::tip Nous Portal covers both
A paid [Nous Portal](/user-guide/features/tool-gateway) subscription supplies the LLM (step 2) **and** OpenAI TTS via the Tool Gateway — no separate OpenAI key needed. On a fresh install, `doppel setup --portal` wires both up at once.
:::
...
Add to `~/.doppel/.env` (legacy installs may still keep the same file in `~/.hermes/.env`):
...
Voice mode is available in both the **classic CLI** (`doppel chat`) and the **TUI** (`doppel --tui`). Behavior is identical across both — same slash commands, same VAD silence detection, same streaming TTS, same hallucination filter. The TUI additionally forwards crash-forensic logs to `~/.doppel/logs/` so push-to-talk failures on exotic audio backends can be reported with a full stack trace rather than disappearing silently. Legacy installs may still write the same logs under `~/.hermes/logs/`.
...
```bash
doppel                # Start the interactive CLI
```
...
1. Start the CLI with `doppel` and enable voice mode with `/voice on`
...
:::tip
The record key is configurable via `voice.record_key` in `~/.doppel/config.yaml` (default: `ctrl+b`). Legacy installs may still keep the same setting in `~/.hermes/config.yaml`.
:::
...
```bash
doppel gateway        # Start the gateway (connects to configured platforms)
doppel gateway setup  # Interactive setup wizard for first-time configuration
```
...
**Server channels:** The bot only responds when you @mention it (e.g. `@your-bot-name hello`). Make sure you select the **bot user** from the mention popup, not the role with the same name.

:::tip
To disable the mention requirement in server channels, add to `~/.doppel/.env` (legacy installs may still keep the same file in `~/.hermes/.env`):
```bash
...
The bot first tries your system library loader (`find_library("opus")`). If that fails, it falls back to:
...
```bash
doppel gateway        # Start with existing configuration
```
...
If you are running Doppel Agent inside Docker on a Linux desktop, the container also needs access to your host audio socket. See the [Docker audio bridge](/user-guide/docker#optional-linux-desktop-audio-bridge) notes for a PulseAudio/PipeWire-compatible setup.
...
3. Or set `DISCORD_REQUIRE_MENTION=false` in `~/.doppel/.env` (legacy installs may still keep the same file in `~/.hermes/.env`)
...
- Review gateway logs: `tail -f ~/.doppel/logs/gateway.log` (legacy installs may still keep the same log at `~/.hermes/logs/gateway.log`)
