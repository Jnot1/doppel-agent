...
The web dashboard is a browser-based UI for managing your Doppel Agent installation. Instead of editing YAML files or running CLI commands, you can configure settings, manage API keys, and monitor sessions from a clean web interface.

:::tip
Hosted-mode auth uses Nous Portal OAuth; if you also want the dashboard to talk to a real backend, `doppel setup --portal` wires up the model and tool gateway too. See [Nous Portal](/integrations/nous-portal).
:::
...
| `--tui` | off | Expose the in-browser Chat tab (embedded `doppel --tui` via PTY/WebSocket). Alternatively set `HERMES_DASHBOARD_TUI=1`. |

```bash
# Custom port
doppel dashboard --port 8080

# Bind to all interfaces (use with caution on shared networks)
doppel dashboard --host 0.0.0.0

# Start without opening browser
doppel dashboard --no-open
...
The current published `doppel-agent` package does not ship the HTTP stack or PTY helper by default — those are optional extras. The **web dashboard** needs FastAPI and Uvicorn (`web` extra). The **Chat** tab also needs `ptyprocess` to spawn the embedded TUI behind a pseudo-terminal (`pty` extra on POSIX). Install both with:

```bash
pip install 'doppel-agent[web,pty]'
```

The `web` extra pulls in FastAPI/Uvicorn; `pty` pulls in `ptyprocess` (POSIX) or `pywinpty` (native Windows — note that the embedded TUI itself still requires WSL). `pip install doppel-agent[all]` includes both extras and is the easiest path if you also want messaging/voice/etc.

When you run `doppel dashboard` without the dependencies, it will tell you what to install. If the frontend hasn't been built yet and `npm` is available, it builds automatically on first launch.

The Chat tab is intentionally off for a plain `doppel dashboard` launch. Start the dashboard with `doppel dashboard --tui` or set `HERMES_DASHBOARD_TUI=1` when you want the embedded browser chat pane.
...
- Node.js (same requirement as `doppel --tui`; the TUI bundle is built on first launch)
- `ptyprocess` — installed by the `pty` extra (`pip install 'doppel-agent[web,pty]'`, or `[all]` covers both)
...
:::tip
Config changes take effect on the next agent session or gateway restart. The web dashboard edits the same `config.yaml` file that `doppel config set` and the gateway read from.
:::
...
Browse, search, and toggle skills and toolsets. Skills are loaded from `~/.doppel/skills/` and grouped by category. Legacy installs may still keep the same directory at `~/.hermes/skills/`.
...
This re-reads `~/.doppel/.env` into the running process's environment. Legacy installs may still keep the same file at `~/.hermes/.env`. Useful when you've added a new provider key via the dashboard and want to use it immediately.
...
When the dashboard is bound to a public address — anything other than `127.0.0.1` / `localhost` — Doppel Agent engages an OAuth-based auth gate. Every request must carry a verified session cookie or it's bounced through a full OAuth round-trip via the Nous Portal.
...
|-------|-----------|----------|
| `doppel dashboard` (default — binds to `127.0.0.1`) | OFF | Local development |
| `doppel dashboard --host 0.0.0.0` | **ON** | Production / Fly.io deployment |
| `doppel dashboard --host 192.168.1.10 --insecure` | OFF | Trusted LAN; user opts into legacy session-token auth |
...
If the gate would engage but **no** `DashboardAuthProvider` is registered (no Nous plugin, no custom plugin), `doppel dashboard` refuses to bind with an explicit error message. There is no "default-deny but accept everything" fallback — a misconfigured gated dashboard never starts.
...
```python
# ~/.doppel/plugins/dashboard-auth-myidp/__init__.py
...
When you run `doppel update`, the web frontend is automatically rebuilt if `npm` is available. This keeps the dashboard in sync with code updates. If `npm` isn't installed, the update skips the frontend build and `doppel dashboard` will build it on first launch.
...
| Theme | Character |
|-------|-----------|
| **Doppel Teal** (`default`) | Dark teal + cream, system fonts, comfortable spacing |
| **Doppel Teal (Large)** (`default-large`) | Same as default with 18px text and roomier spacing |
