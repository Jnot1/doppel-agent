...
Connect Doppel Agent to Apple iMessage via [BlueBubbles](https://bluebubbles.app/) — a free, open-source macOS server that bridges iMessage to any device.
...
- Network connectivity between Doppel Agent and the BlueBubbles server
...
Or set environment variables directly in your agent-home `.env` file (`~/.doppel/.env` on fresh installs; legacy `~/.hermes/.env` still works):
...
When someone messages your iMessage, Doppel Agent automatically sends them a pairing code. Approve it with:
```bash
doppel pairing approve bluebubbles <CODE>
```
Use `doppel pairing list` to see pending codes and approved users.

**Pre-authorize specific users** (in your agent-home `.env` file):
```bash
BLUEBUBBLES_ALLOWED_USERS=user@icloud.com,+15551234567
```

**Open access** (in your agent-home `.env` file):
```bash
...
Doppel Agent will connect to your BlueBubbles server, register a webhook, and start listening for iMessage messages.

## How It Works

```
iMessage → Messages.app → BlueBubbles Server → Webhook → Doppel Agent
Doppel Agent → BlueBubbles REST API → Messages.app → iMessage
```

- **Inbound:** BlueBubbles sends webhook events to a local listener when new messages arrive. No polling — instant delivery.
- **Outbound:** Doppel Agent sends messages via the BlueBubbles REST API.
...
Auto-marking messages as read is controlled by the `send_read_receipts` key under `platforms.bluebubbles.extra` in your agent-home `config.yaml` (`~/.doppel/config.yaml` on fresh installs; legacy `~/.hermes/config.yaml` still works). There is no corresponding environment variable.
...
### Chat Addressing
You can address chats by email or phone number — Doppel Agent resolves them to BlueBubbles chat GUIDs automatically. No need to use raw GUID format.
...
- Check `doppel logs gateway` for webhook errors (or `doppel logs -f` to follow in real-time)
