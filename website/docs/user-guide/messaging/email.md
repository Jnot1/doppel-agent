...
sidebar_position: 7
title: "Email"
description: "Set up Doppel Agent as an email assistant via IMAP/SMTP"
---

# Email Setup

Doppel Agent can receive and reply to emails using standard IMAP and SMTP protocols. Send an email to the agent's address and it replies in-thread — no special client or bot API needed. Works with Gmail, Outlook, Yahoo, Fastmail, or any provider that supports IMAP/SMTP.
...
|---|---|---|
| Let people email the Doppel Agent and receive replies | Email gateway adapter on this page | None beyond an IMAP/SMTP email account |
...
- **A dedicated email account** for your Doppel Agent (don't use your personal email)
...
```bash
# Required
EMAIL_ADDRESS=doppel@gmail.com
...
Email access follows the same pattern as Doppel Agent's other gateway platforms:
...
| **Duplicate replies** | Ensure only one gateway instance is running. Check `doppel gateway status`. |
...
- The password is stored in your agent-home `.env` file (`~/.doppel/.env` on fresh installs; legacy `~/.hermes/.env` still works) — protect this file (`chmod 600`)
