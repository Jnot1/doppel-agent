...
sidebar_position: 12
title: "Google Chat"
description: "Set up Doppel Agent as a Google Chat bot using Cloud Pub/Sub"
---

# Google Chat Setup

Connect Doppel Agent to Google Chat as a bot. The integration uses Cloud Pub/Sub
pull subscriptions for inbound events and the Chat REST API for outbound messages.
Equivalent ergonomics to Slack Socket Mode or Telegram long-polling: your Doppel Agent
...
> Run `doppel gateway setup` and pick **Google Chat** for a guided walk-through.
...
download the file. Save it somewhere only the host running Doppel Agent can read (for example,
`~/.doppel/google-chat-sa.json` on fresh installs; legacy `~/.hermes/google-chat-sa.json` still works, with `chmod 600`).
...
- Delivery type: **Pull**
- Message retention: **7 days** (so backlog survives a Doppel gateway restart)
...
- Principal: `doppel-chat-bot@<your-project>.iam.gserviceaccount.com`
- Role: `Pub/Sub Subscriber`

Also grant `Pub/Sub Viewer` on the same subscription — Doppel Agent calls
...
- **App name**: whatever you want users to see ("Doppel" is reasonable).
...
- **Connection settings**: select **Cloud Pub/Sub**, enter the topic name
  `projects/<your-project>/topics/doppel-chat-events`.
...
`ADDED_TO_SPACE` event that Doppel Agent uses to cache the bot's own `users/{id}` for
self-message filtering.

---

## Step 9: Configure Doppel Agent

Add the Google Chat section to your agent-home `.env` file (`~/.doppel/.env` on fresh installs; legacy `~/.hermes/.env` still works):
...
Install the dependencies the Google Chat adapter needs (no dedicated extra is currently published — install them directly):
...
Send "hola" in the test DM. The bot posts a "Doppel is thinking…" marker, then
...
Thread support: when a user replies inside a thread, Doppel Agent detects the
`thread.name` and posts its reply in the same thread, so each thread gets a
separate Doppel session.
...
3. Download the JSON. Move it onto the host that runs Doppel Agent.
4. On the host, register the client with Doppel Agent:
...
That writes `~/.doppel/google_chat_user_client_secret.json` on fresh installs (legacy `~/.hermes/google_chat_user_client_secret.json` still works). This is shared
...
The token lands at `~/.doppel/google_chat_user_tokens/<sanitized_email>.json` on fresh installs (legacy `~/.hermes/google_chat_user_tokens/<sanitized_email>.json` still works).
...
single-user token at `~/.doppel/google_chat_user_token.json` (or legacy `~/.hermes/google_chat_user_token.json`, if present from
...
1. Check the Pub/Sub subscription has undelivered messages in the console.
   If it does, Doppel Agent isn't authenticated — verify `GOOGLE_CHAT_SERVICE_ACCOUNT_JSON`
...
3. Check `doppel gateway` logs for `[GoogleChat] Connected`. If you see
...
- **Attachment download protection**: Doppel Agent will only attach the SA bearer
...
- **User OAuth scope**: the per-user attachment flow requests *only*
  ...
  `~/.doppel/google_chat_user_tokens/<sanitized_email>.json` on fresh installs
  (legacy `~/.hermes/google_chat_user_tokens/<sanitized_email>.json` still
  works). Filesystem permissions are the protection — same model as the SA key
  file. Each
