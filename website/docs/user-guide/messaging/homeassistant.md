...
description: Control your smart home with Doppel Agent via Home Assistant integration.
...
Doppel Agent integrates with [Home Assistant](https://www.home-assistant.io/) in two ways:
...
4. Click **Create Token**, give it a name like "Doppel Agent"
5. Copy the token
...
Doppel Agent registers four tools for smart home control:
...
Configure which events the agent sees in your agent-home `config.yaml` (`~/.doppel/config.yaml` on fresh installs; legacy `~/.hermes/config.yaml` still works) under the Home Assistant platform's `extra` section:
...
Outbound messages from the agent are delivered as **Home Assistant persistent notifications** (via `persistent_notification.create`). These appear in the HA notification panel like any other Home Assistant persistent notification.
...
The adapter reads credentials from your agent-home `.env` file (auto-merged at startup) or
from `config.yaml`. Double-check the file lives under the active Doppel profile
...
running Doppel Agent — `curl -H "Authorization: Bearer <token>" <url>/api/` should
