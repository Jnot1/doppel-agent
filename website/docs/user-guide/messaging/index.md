...
description: "Chat with Doppel Agent from Telegram, Discord, Slack, WhatsApp, Signal, SMS, Email, Home Assistant, Mattermost, Matrix, DingTalk, Yuanbao, Microsoft Teams, LINE, Webhooks, or any OpenAI-compatible frontend via the API server — architecture and setup overview"
---

# Messaging Gateway

Chat with Doppel Agent from Telegram, Discord, Slack, WhatsApp, Signal, SMS, Email, Home Assistant, Mattermost, Matrix, DingTalk, Feishu/Lark, WeCom, Weixin, BlueBubbles (iMessage), QQ, Yuanbao, Microsoft Teams, LINE, ntfy, or your browser. The gateway is a single background process that connects to all your configured platforms, handles sessions, runs cron jobs, and delivers voice messages.

For the full voice feature set — including CLI microphone mode, spoken replies in messaging, and Discord voice-channel conversations — see [Voice Mode](/user-guide/features/voice-mode) and [Use Voice Mode with Doppel Agent](/guides/use-voice-mode-with-doppel-agent).
...
```mermaid
flowchart TB
    subgraph Gateway["Doppel Gateway"]
...
```bash
doppel gateway setup        # Interactive setup for all messaging platforms
```
...
```bash
doppel gateway              # Run in foreground
doppel gateway setup        # Configure messaging platforms interactively
doppel gateway install      # Install as a user service (Linux) / launchd service (macOS)
sudo doppel gateway install --system   # Linux only: install a boot-time system service
doppel gateway start        # Start the default service
doppel gateway stop         # Stop the default service
doppel gateway status       # Check default service status
doppel gateway status --system         # Linux only: inspect the system service explicitly
```
...
| `/update` | Update Doppel Agent to the latest version |
...
Configure per-platform overrides in `~/.doppel/gateway.json` on fresh installs (legacy `~/.hermes/gateway.json` still works):
...
doppel pairing approve telegram XKGH5N7P

# Other pairing commands:
doppel pairing list          # View pending + approved users
doppel pairing revoke telegram 123456789  # Remove access
```
...
The first time you message a busy agent on any platform, Doppel appends a one-line reminder to the busy-ack explaining the knob (`"💡 First-time tip — …"`). The reminder fires once per install — a flag under `onboarding.seen.busy_input_prompt` latches it. Delete that key to see the tip again.
...
Control how much tool activity is displayed in your agent-home `config.yaml` (`~/.doppel/config.yaml` on fresh installs; legacy `~/.hermes/config.yaml` still works):
...
When the agent running a background session uses `terminal(background=true)` to start long-running processes (servers, builds, etc.), the gateway can push status updates to your chat. Control this with `display.background_process_notifications` in your agent-home `config.yaml` (`~/.doppel/config.yaml` on fresh installs; legacy `~/.hermes/config.yaml` still works):
...
```bash
doppel gateway install               # Install as user service
doppel gateway start                 # Start the service
doppel gateway stop                  # Stop the service
doppel gateway status                # Check status
journalctl --user -u doppel-gateway -f  # View logs

# Enable lingering (keeps running after logout)
sudo loginctl enable-linger $USER

# Or install a boot-time system service that still runs as your user
sudo doppel gateway install --system
sudo doppel gateway start --system
sudo doppel gateway status --system
journalctl -u doppel-gateway -f
```

Use the user service on laptops and dev boxes. Use the system service on VPS or headless hosts that should come back at boot without relying on systemd linger.

:::info Multiple installations
Avoid keeping both the user and system gateway units installed at once unless you really mean to. Doppel warns if it detects both because start/stop/status behavior gets ambiguous.
:::

:::info Multiple installations
If you run multiple Doppel installations on the same machine, each gets its own systemd service name. Fresh installs use `DOPPEL_HOME` and default to `~/.doppel`, which maps to `doppel-gateway`; named profiles use suffixes like `doppel-gateway-coder`. Legacy `HERMES_HOME` installs and `hermes-gateway*` units are still detected so upgrades keep working, and `doppel gateway` automatically targets the correct service for the active home.
:::

### macOS (launchd)

```bash
doppel gateway install               # Install as launchd agent
doppel gateway start                 # Start the service
doppel gateway stop                  # Stop the service
doppel gateway status                # Check status
tail -f ~/.doppel/logs/gateway.log   # View logs on fresh installs
```

The generated plist lives at `~/Library/LaunchAgents/ai.doppel.gateway.plist` on fresh installs. Legacy `ai.hermes.gateway*.plist` files are still detected and migrated when present. It includes three environment variables:
...
- **DOPPEL_HOME** — the preferred home var for fresh installs. Legacy launchd flows may still export `HERMES_HOME`, and the gateway honors both during migration.

:::tip PATH changes after install
launchd plists are static — if you install new tools (e.g. a new Node.js version via nvm, or ffmpeg via Homebrew) after setting up the gateway, run `doppel gateway install` again to capture the updated PATH. The gateway will detect the stale plist and reload automatically.
:::

:::info Multiple installations
Like the Linux systemd service, each agent-home directory gets its own launchd label. Fresh installs use `ai.doppel.gateway` (and profile variants such as `ai.doppel.gateway-coder`). Legacy `ai.hermes.gateway*` labels may still exist on upgraded systems and are handled automatically.
:::
...
1. **Gateway log** (`~/.doppel/logs/gateway.log` on fresh installs, or the legacy `~/.hermes/logs/gateway.log`, or the systemd / launchd unit log). Search for the platform name and `circuit breaker`, `paused`, or `disabled`. The trip event includes the failure count and the last error.
