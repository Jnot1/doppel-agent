...
sidebar_position: 9
title: "Matrix"
description: "Set up Doppel Agent as a Matrix bot"
---

# Matrix Setup

Doppel Agent integrates with Matrix, the open, federated messaging protocol. Matrix lets you run your own homeserver or use a public one like matrix.org — either way, you keep control of your communications. The bot connects via the `mautrix` Python SDK, processes messages through the Doppel Agent pipeline (including tool use, memory, and reasoning), and responds in real time. It supports text, file attachments, images, audio, video, and optional end-to-end encryption (E2EE).

Doppel works with any Matrix homeserver — Synapse, Conduit, Dendrite, or matrix.org.

Before setup, here's the part most people want to know: how Doppel Agent behaves once it's connected.

## How Doppel Agent Behaves

| Context | Behavior |
|---------|----------|
| **DMs** | Doppel responds to every message. No `@mention` needed. Each DM has its own session. Set `MATRIX_DM_MENTION_THREADS=true` to start a thread when the bot is `@mentioned` in a DM. |
| **Rooms** | By default, Doppel requires an `@mention` to respond. Set `MATRIX_REQUIRE_MENTION=false` or add room IDs to `MATRIX_FREE_RESPONSE_ROOMS` for free-response rooms. Room invites are auto-accepted. |
| **Threads** | Doppel supports Matrix threads (MSC3440). If you reply in a thread, Doppel keeps the thread context isolated from the main room timeline. Threads where the bot has already participated do not require a mention. |
| **Auto-threading** | By default, Doppel auto-creates a thread for each message it responds to in a room. This keeps conversations isolated. Set `MATRIX_AUTO_THREAD=false` to disable. |
| **Shared rooms with multiple users** | By default, Doppel isolates session history per user inside the room. Two people talking in the same room do not share one transcript unless you explicitly disable that. |
...
You can also run Doppel Agent as your own user. This means the bot posts as you — useful for personal assistants.

## Step 2: Get an Access Token

Doppel Agent needs an access token to authenticate with the homeserver. You have two options:
...
Instead of providing an access token, you can give Doppel Agent the bot's user ID and password. Doppel will log in automatically on startup. This is simpler but means the password is stored in your `.env` file.
...
Doppel Agent uses your Matrix User ID to control who can interact with the bot. Matrix User IDs follow the format `@username:server`.
...
Optional behavior settings in your agent-home `config.yaml` (`~/.doppel/config.yaml` on fresh installs; legacy `~/.hermes/config.yaml` still works):
...
Doppel Agent supports Matrix end-to-end encryption, so you can chat with your bot in encrypted rooms.
...
Add to your agent-home `.env` file:

```bash
MATRIX_ENCRYPTION=true
```

When E2EE is enabled, Doppel Agent:
...
On each startup, if `MATRIX_RECOVERY_KEY` is set, Doppel imports cross-signing keys from the homeserver's secure secret storage and signs the current device. This is idempotent and safe to leave enabled permanently.
...
Doppel Agent detects this condition on startup and refuses to enable E2EE, logging: `device XXXX has stale one-time keys on the server signed with a previous identity key`.
...
2. Delete the local crypto store and restart Doppel Agent:
   ```bash
...
Add this to your agent-home `.env` file:
...
If you previously used Doppel Agent with `MATRIX_ENCRYPTION=true` and are upgrading to
...
1. **Generate a new access token** to get a fresh device ID. The simplest way:
   ...
   Copy the new `access_token` and update `MATRIX_ACCESS_TOKEN` in your agent-home `.env` file.
...
3. **Set your recovery key** (if you use cross-signing — most Element users do). Add to your agent-home `.env` file:
...
a working E2EE setup with a previous version of Doppel Agent and are upgrading.
...
| Setting | Description |
|---------|-------------|
| `GATEWAY_PROXY_URL` | URL of the remote Doppel API server (e.g., `http://192.168.1.100:8642`) |
...
**Fix**: The sync loop automatically retries every 5 seconds on error. Check the Doppel logs for sync-related warnings. If the bot consistently falls behind, ensure your homeserver has adequate resources.

### Bot is offline

**Cause**: The Doppel gateway isn't running, or it failed to connect.

**Fix**: Check that `doppel gateway` is running. Look at the terminal output for error messages. Common issues: wrong homeserver URL, expired access token, homeserver unreachable.

### "User not allowed" / Bot ignores you

**Cause**: Your User ID isn't in `MATRIX_ALLOWED_USERS`.

**Fix**: Add your User ID to `MATRIX_ALLOWED_USERS` in your agent-home `.env` file and restart the gateway. Use the full `@user:server` format.
...
- **Media support**: Doppel Agent can send and receive images, audio, video, and file attachments. Media is uploaded to your homeserver using the Matrix content repository API.
