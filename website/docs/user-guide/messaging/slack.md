...
sidebar_position: 4
title: "Slack"
description: "Set up Doppel Agent as a Slack bot using Socket Mode"
---

# Slack Setup

Connect Doppel Agent to Slack as a bot using Socket Mode. Socket Mode uses WebSockets instead of
public HTTP endpoints, so your Doppel instance doesn't need to be publicly accessible — it works
behind firewalls, on your laptop, or on a private server.

:::warning Classic Slack Apps Deprecated
Classic Slack apps (using RTM API) were **fully deprecated in March 2025**. Doppel uses the modern
...
The fastest path is to paste a manifest Doppel generates for you. It
...
### Option A: From a Doppel-generated manifest (recommended)

1. Generate the manifest:
   ```bash
   doppel slack manifest --write
   ```
   This writes `~/.doppel/slack-manifest.json` on fresh installs (legacy `~/.hermes/slack-manifest.json` still works) and prints paste-in instructions.
...
4. Enter an app name (e.g., "Doppel Agent") and select your workspace
5. Click **Create App**
...
it will only work in DMs. Without `files:read`, Doppel can chat but **cannot reliably read user-uploaded attachments**.
...
Even with all the correct scopes and event subscriptions, Slack will not allow users to send direct messages to the bot unless the Messages Tab is enabled. This is a Slack platform requirement, not a Doppel configuration issue.
:::
...
Doppel uses Slack **Member IDs** (not usernames or display names) for the allowlist.
...
```bash
doppel gateway setup    # Select Slack when prompted
```

Then start the gateway:

```bash
doppel gateway              # Foreground
doppel gateway install      # Install as a user service
sudo doppel gateway install --system   # Linux only: boot-time system service
```
...
Every Doppel command (`/btw`, `/stop`, `/new`, `/model`, `/help`, ...)
...
Doppel command with its description.

Under the hood: Doppel ships with a generated Slack app manifest (see
Step 1, Option A) that declares every command in
[`COMMAND_REGISTRY`](https://github.com/Jnot1/doppel-agent/blob/main/hermes_cli/commands.py)
...
When Doppel adds new commands (e.g. after `doppel update`), regenerate
...
3. Paste the new contents of `~/.doppel/slack-manifest.json` (or the legacy `~/.hermes/slack-manifest.json` path if that is still your active home)
...
Slack never delivers them to Doppel.

As a workaround, Doppel recognises a leading `!` as an alternate
command prefix that works in threads (and anywhere else). Type
`!queue`, `!stop`, `!model gpt-5.4`, etc. as a regular thread reply —
Doppel treats it identically to the slash form and replies in the same
thread.
...
```bash
doppel slack manifest --slashes-only > /tmp/slashes.json
```
...
Understanding how Doppel behaves in different contexts:

| Context | Behavior |
|---------|----------|
| **DMs** | Bot responds to every message — no @mention needed |
| **Channels** | Bot **only responds when @mentioned** (e.g., `@Doppel Agent what time is it?`). In channels, Doppel replies in a thread attached to that message. |
| **Threads** | If you @mention Doppel inside an existing thread, it replies in that same thread. Once the bot has an active session in a thread, **subsequent replies in that thread do not require @mention** — the bot follows the conversation naturally. |
...
Beyond the required environment variables from Step 8, you can customize Slack bot behavior through your agent-home `config.yaml` (`~/.doppel/config.yaml` on fresh installs; legacy `~/.hermes/config.yaml` still works).
...
When `true` (the default), each user in a shared channel gets their own isolated conversation session. Two people talking to Doppel in `#general` will have separate histories and contexts.
...
slack:
  ...
  # must @mention the bot before Doppel will respond.
...
Set `SLACK_HOME_CHANNEL` to a channel ID where Doppel will deliver scheduled messages,
...
Make sure the bot has been **invited to the channel** (`/invite @Doppel Agent`).

---

## Multi-Workspace Support

Doppel can connect to **multiple Slack workspaces** simultaneously using a single gateway instance. Each workspace is authenticated independently with its own bot user ID.
...
Or in your agent-home `config.yaml` (`~/.doppel/config.yaml` on fresh installs; legacy `~/.hermes/config.yaml` still works):
...
In addition to tokens in the environment or config, Doppel also loads tokens from an **OAuth token file** at:

```
~/.doppel/slack_tokens.json
```

Legacy installs may still use `~/.hermes/slack_tokens.json`.
...
- When a message arrives, Doppel uses the correct workspace-specific client to respond.
...
Doppel supports voice on Slack:
...
| Bot works in DMs but not in channels | **Most common issue.** Add `message.channels` and `message.groups` to event subscriptions, reinstall the app, and invite the bot to the channel with `/invite @Doppel Agent` |
...
| Bot responds but can't post in a channel | Invite the bot to the channel with `/invite @Doppel Agent` |
| Bot can chat but can't read uploaded images/files | Add `files:read`, then **reinstall** the app. Doppel now surfaces attachment access diagnostics in-chat when Slack returns scope/auth/permission failures. |
...
7. ✅ Bot was **invited** to the channel (`/invite @Doppel Agent`)
...
- Tokens should be stored in your agent-home `.env` file (`~/.doppel/.env` on fresh installs; legacy `~/.hermes/.env` still works)
- Rotate tokens periodically via the Slack app settings
- Audit who has access to your Doppel config directory
