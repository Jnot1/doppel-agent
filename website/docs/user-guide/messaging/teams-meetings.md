...
Use the Teams meeting pipeline when you want Doppel Agent to ingest Microsoft Graph meeting events, fetch transcripts first, fall back to recordings plus STT when needed, and deliver a structured summary to downstream sinks.

Prerequisites: see [Microsoft Teams](./teams.md) for the underlying bot/credential setup.

> Run `doppel gateway setup` and pick **Teams Meetings** for a guided walk-through.
...
Operator actions stay in the CLI (the `teams-pipeline` subcommand is registered by the `teams_pipeline` plugin — enable it via `doppel plugins enable teams_pipeline` or set `plugins.enabled: [teams_pipeline]` in `config.yaml`):
...
- a working Doppel Agent install
...
Add Graph app-only credentials to your agent-home `.env` file (`~/.doppel/.env` on fresh installs; legacy `~/.hermes/.env` still works):
...
Set the bind host under `platforms.msgraph_webhook.extra.host` in `config.yaml` when you need something other than the default loopback setup.
...
Example agent-home `config.yaml` (`~/.doppel/config.yaml` on fresh installs; legacy `~/.hermes/config.yaml` still works):
...
Use this when you want Doppel Agent to post the summary through Microsoft Graph into a Teams chat or channel.
...
Start Doppel Agent normally after updating config:

```bash
doppel gateway run
```

Or, if you run Doppel Agent in Docker, start the gateway the same way you already do for your deployment.
...
Microsoft Graph caps webhook subscriptions at 72 hours and will not auto-renew them. You MUST schedule `doppel teams-pipeline maintain-subscriptions` before going live, or notifications will silently stop three days after any manual subscription creation. See [Automating subscription renewal](/guides/operate-teams-meeting-pipeline#automating-subscription-renewal-required-for-production) in the operator runbook — three options (Doppel cron, systemd timer, plain crontab).
...
| Jobs do not appear in `doppel teams-pipeline list` | Confirm `msgraph_webhook` is enabled and that subscriptions point at the right notification URL |
