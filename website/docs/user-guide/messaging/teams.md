...
description: "Set up Doppel Agent as a Microsoft Teams bot"
---

# Microsoft Teams Setup

Connect Doppel Agent to Microsoft Teams as a bot. Unlike Slack's Socket Mode, Teams delivers messages by calling a **public HTTPS webhook**, so your instance needs a publicly reachable endpoint — either a dev tunnel (local dev) or a real domain (production).
...
> Run `doppel gateway setup` and pick **Microsoft Teams** for a guided walk-through.
...
Teams delivers @mentions as regular messages with `<at>BotName</at>` tags, which Doppel Agent strips automatically before processing.
...
devtunnel port create doppel-bot -p 3978 --protocol https  # replace 3978 with TEAMS_PORT if changed
...
Alternatively, configure via your agent-home `config.yaml` (`~/.doppel/config.yaml` on fresh installs; legacy `~/.hermes/config.yaml` still works):
...
| `No inference provider configured` | Check that `ANTHROPIC_API_KEY` (or another provider key) is set in your agent-home `.env` file |
...
| Tunnel URL changes on restart | devtunnel URLs are persistent if you use a named tunnel (`devtunnel create doppel-bot`). ngrok and cloudflared generate a new URL each run unless you have a paid plan — update the bot endpoint with `teams app update` when it changes |
...
- Store credentials in `~/.doppel/.env` on fresh installs (legacy `~/.hermes/.env` still works) with permissions `600`
