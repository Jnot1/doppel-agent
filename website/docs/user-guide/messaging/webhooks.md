...
<div style={{position: 'relative', width: '100%', aspectRatio: '16 / 9', marginBottom: '1.5rem'}}>
  <iframe
    ...
    title="Doppel Agent — Webhooks Tutorial"
...
1. Enable via `doppel gateway setup` or environment variables
2. Define routes in `config.yaml` **or** create them dynamically with `doppel webhook subscribe`
...
Add to your agent-home `.env` file (`~/.doppel/.env` on fresh installs; legacy `~/.hermes/.env` still works):
...
Add the `github-pr` route to your agent-home `config.yaml` (`~/.doppel/config.yaml` on fresh installs; legacy `~/.hermes/config.yaml` still works) as shown in the example above.
...
Open a pull request on the repository. The webhook fires, Doppel Agent processes the event, and posts a review comment on the PR.
...
The `deliver` field controls where Doppel Agent's response goes after processing the webhook event.
...
By default, every webhook POST triggers a Doppel Agent run — the payload becomes a prompt, the agent processes it, and the agent's response is delivered. This costs LLM tokens on every event.
...
In addition to static routes in `config.yaml`, you can create webhook subscriptions dynamically using the `doppel webhook` CLI command. This is especially useful when the agent itself needs to set up event-driven triggers.
...
```bash
doppel webhook remove github-issues
```

### Test a subscription

```bash
doppel webhook test github-issues
doppel webhook test github-issues --payload '{"issue": {"number": 42, "title": "Test"}}'
```

### How dynamic subscriptions work

- Subscriptions are stored in `~/.doppel/webhook_subscriptions.json` on fresh installs (legacy `~/.hermes/webhook_subscriptions.json` still works)
...
The agent can create subscriptions via the terminal tool when guided by the `webhook-subscriptions` skill. Ask the agent to "set up a webhook for GitHub issues" and it will run the appropriate `doppel webhook subscribe` command.
...
- Run the gateway in foreground to see logs: `doppel gateway run`
