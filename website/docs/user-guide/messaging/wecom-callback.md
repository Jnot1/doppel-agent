...
Connect Doppel Agent to WeCom (Enterprise WeChat) as a self-built enterprise application using the callback/webhook model.

:::info WeCom Bot vs WeCom Callback
Doppel Agent supports two WeCom integration modes:
...
> Run `doppel gateway setup` and pick **WeCom Callback** for a guided walk-through.
...
3. Doppel Agent decrypts the message, queues it for the agent
...
(Use `doppel gateway start` only after `doppel gateway install` has registered the systemd/launchd service.)
...
console. A mismatch between the token configured in Doppel Agent and the token the
...
in your agent-home `.env` file (`~/.doppel/.env` on fresh installs; legacy `~/.hermes/.env` still works) around `=` will also break signature checks. After
fixing, restart `doppel gateway run`.
...
Check `doppel gateway run` logs for the bound host/port. If the adapter bound to
