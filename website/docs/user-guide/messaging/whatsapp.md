...
sidebar_position: 5
title: "WhatsApp"
description: "Set up Doppel Agent as a WhatsApp bot via the built-in Baileys bridge"
---

# WhatsApp Setup

Doppel Agent connects to WhatsApp through a built-in bridge based on **Baileys**. This works by emulating a WhatsApp Web session — **not** through the official WhatsApp Business API. No Meta developer account or Business verification is required.

> Run `doppel gateway setup` and pick **WhatsApp** for a guided walk-through.
...
with third-party bridges. When this happens, Doppel Agent will update the bridge dependency. If the
bot stops working after a WhatsApp update, pull the latest Doppel version and re-pair.
:::
...
3. Run `doppel whatsapp` and scan the QR code from that WhatsApp account
...
The Baileys bridge saves its session under `~/.doppel/platforms/whatsapp/session` on fresh installs (legacy `~/.hermes/platforms/whatsapp/session` still works). This means:
...
Doppel Agent supports voice on WhatsApp:
...
| **QR code expires** | QR codes refresh every ~20 seconds. If it times out, restart `doppel whatsapp`. |
| **Session not persisting** | Check that your WhatsApp session directory exists and is writable (`~/.doppel/platforms/whatsapp/session` on fresh installs; legacy `~/.hermes/platforms/whatsapp/session` still works). If containerized, mount it as a persistent volume. |
| **Logged out unexpectedly** | WhatsApp unlinks devices after long inactivity. Keep the phone on and connected to the network, then re-pair with `doppel whatsapp` if needed. |
| **Bridge crashes or reconnect loops** | Restart the gateway, update Doppel Agent, and re-pair if the session was invalidated by a WhatsApp protocol change. |
| **Bot stops working after WhatsApp update** | Update Doppel Agent to get the latest bridge version, then re-pair. |
| **macOS: "Node.js not installed" but node works in terminal** | launchd services don't inherit your shell PATH. Run `doppel gateway install` to re-snapshot your current PATH into the plist, then `doppel gateway start`. See the [Gateway Service docs](./index.md#macos-launchd) for details. |
...
| **Bot replies to strangers with a pairing code** | Set `whatsapp.unauthorized_dm_behavior: ignore` in your agent-home `config.yaml` if you want unauthorized DMs to be silently ignored instead. |
...
- The WhatsApp session directory contains full session credentials — protect it like a password (`~/.doppel/platforms/whatsapp/session` on fresh installs; legacy `~/.hermes/platforms/whatsapp/session` still works)
- Set file permissions: `chmod 700 ~/.doppel/platforms/whatsapp/session` (or the legacy `~/.hermes/...` path if that is still your active home)
