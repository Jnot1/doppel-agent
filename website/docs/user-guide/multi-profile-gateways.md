...
If you only run one Doppel agent, you don't need this page — see
[Profiles](./profiles.md) for the basics.

## When to use this

You want this setup when you have two or more Doppel agents that should all
...
doppel profile create personal-bot
doppel profile create research
...
`~/.local/bin/doppel-gateways` and `chmod +x` it:
...
```bash
doppel-gateways start      # start every configured profile
doppel-gateways stop       # stop every configured profile
doppel-gateways restart    # restart all
doppel-gateways status     # status across all
doppel-gateways list       # delegates to `doppel gateway list`
```

:::tip
The `default` profile is targeted with `doppel gateway <action>` (no `-p`),
not `doppel -p default gateway <action>`. The wrapper above handles both forms.
:::
...
These are equivalent to `doppel -p coder gateway <action>` — useful if a
...
tail -f ~/.doppel/logs/gateway.error.log

# Named profile
tail -f ~/.doppel/profiles/<name>/logs/gateway.log
tail -f ~/.doppel/profiles/<name>/logs/gateway.error.log
```

Stream every profile's log simultaneously:

```bash
tail -f ~/.doppel/logs/gateway.log ~/.doppel/profiles/*/logs/gateway.log
```

The CLI also has a structured log viewer:

```bash
doppel logs --tail              # follow default profile
doppel -p coder logs --tail     # follow one profile
doppel logs --help              # filters, levels, JSON output
```

## Identify what's actually running

```bash
doppel profile list             # profiles + model + gateway state
doppel-gateways status          # full status across every profile
...
The default profile uses `~/.doppel/` directly with the same three files.

Edit them with any editor or via the CLI:

```bash
doppel config set model.model anthropic/claude-sonnet-4    # default profile
...
caffeinate -i -w $(cat ~/.doppel/gateway.pid) &   # awake while default gateway runs
...
systemd-inhibit --what=idle:sleep --who=doppel --why="gateways running" \
  sleep infinity &
...
`doppel update` pulls the latest code once and syncs new bundled skills into
every profile:
...
You ran `doppel gateway start` after a previous `doppel gateway stop`. The
...
cat ~/.doppel/profiles/<profile>/gateway.pid
...
```bash
doppel doctor                  # default profile
doppel -p <profile> doctor     # one profile
```
