...
A **profile distribution** packages a complete Doppel agent — personality, skills, cron jobs, MCP connections, config — as a git repository. Anyone with access to the repo can install the whole agent with one command, update it in place, and keep their own memories, sessions, and API keys untouched.

If a [profile](./profiles.md) is a local agent, a distribution is that agent made shareable.

## What this means

Before distributions, sharing a Doppel agent meant sending someone:
...
```bash
doppel profile install github.com/you/my-research-agent --alias
```

…and they now have the whole agent. They fill in their own API keys (`.env.EXAMPLE` → `.env`), and they can run `my-research-agent chat` or address it through Telegram / Discord / Slack / any gateway platform. When you push a new version, they run `doppel profile update my-research-agent` and pull your changes — their memories and sessions stay put.
...
The tradeoff: recipients need git installed. On any machine running Doppel in 2026, that's already true.
...
- **You just want to back up a profile on your own machine.** Use [`doppel profile export` / `import`](../reference/profile-commands.md#doppel-profile-export) — that's what those are for.
...
Recipients who run `doppel profile update research-bot` will pull the latest.
...
5. Copies distribution-owned files into `~/.doppel/profiles/research-bot/` (or wherever the manifest's `name` resolves).
...
```bash
# GitHub shorthand
doppel profile install github.com/you/research-bot

# Full HTTPS
doppel profile install https://github.com/you/research-bot.git

# SSH
doppel profile install git@github.com:you/research-bot.git

# Self-hosted, GitLab, Gitea, Forgejo — any Git host
doppel profile install https://git.example.com/team/research-bot.git

# Private repo using your configured git auth
doppel profile install git@github.com:your-org/internal-bot.git

# Local directory during development (no git push needed)
doppel profile install ~/my-profile-in-progress/
```
...
```bash
# Alice
doppel profile install github.com/acme/support-bot --name support-us --alias
# Bob (same distribution, different local name)
doppel profile install github.com/acme/support-bot --name support-eu --alias
```
...
`doppel profile list` also shows a `Distribution` column so at a glance you can see which of your profiles came from repos and which you hand-built:
...
Any iteration on the laptop (`git commit && push`) pulls onto the workstation with `doppel profile update research-bot`. Memories stay per-machine — the laptop remembers its own conversations, the workstation remembers its own, they don't collide.
...
```bash
# Engineering lead
cd ~/.doppel/profiles/pr-reviewer
# ... build and tune ...
...
# Each engineer
doppel profile install git@github.com:your-org/pr-reviewer.git --alias
# Fill in .env with their own API key (billed to them), .env.EXAMPLE points at what's required
pr-reviewer chat
```

When the lead ships v1.1 (better SOUL, new skill), engineers run `doppel profile update pr-reviewer` and everyone's on the new version within minutes.
...
```bash
# You
cd ~/.doppel/profiles/polymarket-trader
# Write a solid README.md at the repo root — GitHub shows it on the repo page
git init && git add . && git commit -m "v1.0"
git tag v1.0.0
# Publish to a public GitHub repo
git remote add origin https://github.com/you/doppel-polymarket-trader.git
git push -u origin main --tags

# Anyone
doppel profile install github.com/you/doppel-polymarket-trader --alias
```
...
You built Doppel-on-top — maybe a compliance-monitoring harness, a customer-support stack, a domain-specific research platform. You want to distribute it as a product.
...
doppel profile delete incident-2026-q2
```
...
:::note
Git ref pinning (`#v1.2.0`) is planned but not in the initial release — install currently tracks the default branch. Track your installed version via `doppel profile info <name>` and hold off on updates until you're ready.
:::
...
doppel profile info research-bot | grep Version
...
doppel profile delete research-bot --yes
doppel profile install github.com/you/research-bot --alias
...
doppel profile install github.com/yourname/forked-research-bot --alias
...
doppel profile delete research-bot-test --yes
doppel profile install ~/.doppel/profiles/research-bot --name research-bot-test
```
...
Cron jobs from a distribution are **not auto-scheduled** — the installer prints `doppel -p <name> cron list` and you enable them explicitly. SOUL.md and skills ARE active as soon as you start chatting with the profile, so read them before your first run if you're installing from someone you don't know.
...
- `install`, `update`, `info` live inside `doppel profile` — not a parallel command tree.
...
- Reserved profile names (`doppel`, `test`, `tmp`, `root`, `sudo`) are rejected at install time to avoid collisions with common binaries.
...
- [`doppel profile export` / `import`](../reference/profile-commands.md#doppel-profile-export) — local backup / restore (not distribution)
- [Using SOUL with Doppel Agent](/guides/use-soul-with-doppel-agent) — authoring personalities
