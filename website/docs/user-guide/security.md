...
Many features (Mistral TTS, ElevenLabs, Honcho memory, Bedrock, Slack, Matrix, …) depend on Python packages that not every user needs. Doppel Agent installs these **lazily** on first use rather than eagerly under the current distribution extra `hermes-agent[all]`. The implementation lives in `tools/lazy_deps.py`.
