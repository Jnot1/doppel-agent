...
Doppel Agent automatically saves every conversation as a session. Sessions enable conversation resume, cross-session search, and full conversation history management.
...
1. **SQLite database** (`~/.doppel/state.db`) — structured session metadata with FTS5 full-text search, plus full message history
...
Doppel stores session history so it can resume conversations, but it does not
...
Doppel explicitly injects for that turn.
...
For example, if a user sends an image and asks Doppel to make a meme from it,
Doppel may inspect that image once with vision and run an image-processing
...
`doppel sessions prune` only when you want to delete old ended sessions from
...
| Source | Description |
|--------|-------------|
| `cli` | Interactive CLI (`doppel` or `doppel chat`) |
...
doppel -c "my project"   # → resumes "my project #3"
```
...
doppel -r 20250305_091523_a1b2c3d4

# Resume by title
doppel --resume "refactoring auth"

# Or with the chat subcommand
doppel chat --resume 20250305_091523_a1b2c3d4
```

Session IDs are shown when you exit a CLI session, and can be found with `doppel sessions list`.

### Conversation Recap on Resume

When you resume a session, Doppel displays a compact recap of the previous conversation in a styled panel before the input prompt:

<img className="docs-terminal-figure" src={useBaseUrl('/img/docs/session-recap.svg')} alt="Stylized preview of the Previous Conversation recap panel shown when resuming a Doppel session." />
...
To disable the recap and keep the minimal one-liner behavior, set in `~/.doppel/config.yaml`:
...
**Resume back to CLI:** when you want to come back to a desktop, just run `/resume <title>` (or `doppel -r "<title>"` from the shell) and pick up where the platform left off.
...
Doppel automatically generates a short descriptive title (3–7 words) for each session after the first exchange. This runs in a background thread using a fast auxiliary model, so it adds no latency. You'll see auto-generated titles when browsing sessions with `doppel sessions list` or `doppel sessions browse`.
...
```bash
doppel sessions rename 20250305_091523_a1b2c3d4 "refactoring auth module"
```
...
When a session's context is compressed (manually via `/compress` or automatically), Doppel creates a new continuation session. If the original had a title, the new session automatically gets a numbered title:

```
"my project" → "my project #2" → "my project #3"
```

When you resume by name (`doppel -c "my project"`), it automatically picks the most recent session in the lineage.
...
# Filter by platform
doppel sessions list --source telegram

# Show more sessions
doppel sessions list --limit 50
```
...
doppel sessions export telegram-history.jsonl --source telegram

# Export a single session
doppel sessions export session.jsonl --session-id 20250305_091523_a1b2c3d4
```
...
doppel sessions delete 20250305_091523_a1b2c3d4 --yes
```
...
doppel sessions rename 20250305_091523_a1b2c3d4 "debugging auth flow"
...
doppel sessions prune --source telegram --older-than 60

# Skip confirmation
doppel sessions prune --older-than 30 --yes
```
...
For deeper analytics — token usage, cost estimates, tool breakdown, and activity patterns — use [`doppel insights`](/reference/cli-commands#doppel-insights).
...
When Doppel cannot get a participant identifier for a shared chat, it falls back to one shared session for that room.

### Shared vs Isolated Group Sessions

By default, Doppel uses `group_sessions_per_user: true` in `config.yaml`. That means:

- Alice and Bob can both talk to Doppel in the same Discord channel without sharing transcript history
...
|------|------|-------------|
| SQLite database | `~/.doppel/state.db` | All session metadata + messages with FTS5 |
| Gateway messages    | `~/.doppel/state.db`   | SQLite — canonical store for all session messages |
| Gateway routing index | `~/.doppel/sessions/sessions.json` | Maps session keys to active session IDs (origin metadata, expiry flags) |
...
`*.jsonl` files in `~/.doppel/sessions/`. They are no longer written or
read by Doppel. Safe to delete after verifying the corresponding session
exists in state.db.
:::
...
- Pruning runs at most once per `sessions.min_interval_hours` (default 24); the last-run timestamp is tracked inside `state.db` itself so it's shared across every Doppel process in the same `DOPPEL_HOME`

Default is **off** — session history is valuable for `session_search` recall, and silently deleting it could surprise users. Enable in `~/.doppel/config.yaml`:
...
doppel sessions delete <session_id>
