---
title: "Doppel Agent — Configure, extend, or contribute to Doppel Agent"
...
Configure, extend, or contribute to Doppel Agent.
...
| Path | `skills/autonomous-ai-agents/doppel-agent` |
| Version | `2.1.0` |
...
- **Self-improving through skills** — Doppel learns from experience by saving reusable procedures as skills. When it solves a complex problem, discovers a workflow, or gets corrected, it can persist that knowledge as a skill document that loads into future sessions. Skills accumulate over time, making the agent better at your specific tasks and environment.
- **Persistent memory across sessions** — remembers who you are, your preferences, environment details, and lessons learned. Pluggable memory backends (built-in, Honcho, Mem0, and more) let you choose how memory works.
- **Multi-platform gateway** — the same agent runs on Telegram, Discord, Slack, WhatsApp, Signal, Matrix, Email, and 10+ other platforms with full tool access, not just chat.
- **Provider-agnostic** — swap models and providers mid-workflow without changing anything else. Credential pools rotate across multiple API keys automatically.
- **Profiles** — run multiple independent Doppel instances with isolated configs, sessions, skills, and memory.
- **Extensible** — plugins, MCP servers, custom tools, webhook triggers, cron scheduling, and the full Python ecosystem.

People use Doppel for software development, research, system administration, data analysis, content creation, home automation, and everyday assistant workflows like email triage, calendar organization, groceries, to-dos, personal training, and cooking help.
...
doppel auth                 Interactive credential manager
doppel auth add PROVIDER    Add OAuth or API-key credential (e.g. nous, openai-codex, qwen-oauth)
doppel auth list            List stored credentials
doppel auth remove PROVIDER Remove a stored credential
...
```
~/.doppel/config.yaml       Main configuration (legacy ~/.hermes/config.yaml still supported)
~/.doppel/.env              API keys and secrets
$DOPPEL_HOME/skills/        Installed skills (legacy $HERMES_HOME still supported)
~/.doppel/sessions/         Gateway routing index, request dumps, *.jsonl transcripts (and optional per-session JSON snapshots when sessions.write_json_snapshots: true)
~/.doppel/state.db          Canonical session store (SQLite + FTS5)
~/.doppel/logs/             Gateway and error logs
~/.doppel/auth.json         OAuth tokens and credential pools
~/.doppel/doppel-agent/     Source code (if git-installed; legacy ~/.doppel/hermes-agent/ may still exist)
```

Profiles use `~/.doppel/profiles/<name>/` with the same layout.

### Config Sections

Edit with `doppel config edit` or `doppel config set section.key value`.
...
20+ providers supported. Set via `doppel model` or `doppel setup`.
...
Enable/disable via `doppel tools` (interactive) or `doppel tools enable/disable NAME`.
...
Common "why is Doppel doing X to my output / tool calls / commands?" toggles — and the exact commands to change them. Most of these need a fresh session (`/reset` in chat, or start a new `doppel` invocation) because they're read once at startup.
...
```bash
doppel config set security.redact_secrets true       # keep enabled globally
```
...
```bash
doppel config set privacy.redact_pii true    # enable
doppel config set privacy.redact_pii false   # disable (default)
```

### Command approval prompts

By default (`approvals.mode: manual`), Doppel prompts the user before running shell commands flagged as destructive (`rm -rf`, `git reset --hard`, etc.). The modes are:
...
```bash
doppel config set approvals.mode smart       # recommended middle ground
doppel config set approvals.mode off         # bypass everything (not recommended)
```
...
Some shell-hook integrations require explicit allowlisting before they fire. Managed via `~/.doppel/shell-hooks-allowlist.json` on fresh installs; legacy `~/.hermes/shell-hooks-allowlist.json` still works. The user is prompted interactively the first time a hook wants to run.

### Disabling the web/browser/image-gen tools

To keep the model away from network or media tools entirely, open `doppel tools` and toggle per-platform. Takes effect on next session (`/reset`). See the Tools & Skills section above.
...
## Spawning Additional Doppel Instances

Run additional Doppel processes as fully independent subprocesses — separate sessions, tools, and environments.

### When to Use This vs delegate_task

| | `delegate_task` | Spawning `doppel` process |
|-|-----------------|--------------------------|
...
Doppel uses prompt_toolkit, which requires a real terminal. Use tmux for interactive spawning:
...
- **Use `doppel chat -q` for fire-and-forget** — no PTY needed
...
the `cronjob` tool, the `doppel cron` CLI (`list`, `add`, `edit`,
...
- **CLI:** `doppel curator <verb>` — `status`, `run`, `pause`, `resume`,
...
- **Telemetry:** sidecar at `~/.doppel/skills/.usage.json` on fresh installs (legacy `~/.hermes/skills/.usage.json` still works) holds
...
Users drive it via `doppel kanban <verb>`; dispatcher-spawned workers
...
Doppel runs natively on Windows (PowerShell, cmd, Windows Terminal, git-bash
...
without BOM. `doppel config edit` writes without BOM; manual edits in
...
Doppel's own env scrubber dropping `SYSTEMROOT` / `WINDIR` / `COMSPEC`
...
POSIX venv layouts (`.venv/bin/activate`). The Doppel-installed venv at
...
every Doppel tool and most Windows APIs. Prefer forward slashes in code
...
### Tool not available
1. `doppel tools` — check if toolset is enabled for your platform
2. Some tools need env vars (check `.env`)
3. `/reset` after enabling tools

### Model/provider issues
1. `doppel doctor` — check config and dependencies
2. `doppel auth` — re-authenticate OAuth providers (or `doppel auth add <provider>`)
3. Check `.env` has the right API key
4. **Copilot 403**: `gh auth login` tokens do NOT work for Copilot API. You must use the Copilot-specific OAuth device code flow via `doppel model` → GitHub Copilot.
...
### Skills not showing
1. `doppel skills list` — verify installed
2. `doppel skills config` — check platform enablement
3. Load explicitly: `/skill name` or `doppel -s name`
...
- **Gateway crash loop**: Reset the failed state: `systemctl --user reset-failed doppel-gateway`
...
Config: `~/.doppel/config.yaml` (settings) and `~/.doppel/.env` (API keys). Legacy `~/.hermes/` paths still work during the transition.
