...
Create dynamic webhook subscriptions so external services (GitHub, GitLab, Stripe, CI/CD, IoT sensors, monitoring tools) can trigger Doppel agent runs by POSTing events to a URL.
...
```bash
doppel gateway run
# Or restart your configured systemd user service
```
...
All management is via the `doppel webhook` CLI command:
...
doppel webhook test <name> --payload '{"key": "value"}'
```
...
- Subscriptions persist to `~/.doppel/webhook_subscriptions.json`

## How It Works

1. `doppel webhook subscribe` writes to `~/.doppel/webhook_subscriptions.json`
...
1. **Is the gateway running?** Check with your service manager or `ps aux | grep gateway`
2. **Is the webhook server listening?** `curl http://localhost:8644/health` should return `{"status": "ok"}`
3. **Check gateway logs:** `grep webhook ~/.doppel/logs/gateway.log | tail -20`
4. **Signature mismatch?** Verify the secret in your service matches the one from `doppel webhook list`. GitHub sends `X-Hub-Signature-256`, GitLab sends `X-Gitlab-Token`.
5. **Firewall/NAT?** The webhook URL must be reachable from the service. For local development, use a tunnel (ngrok, cloudflared).
6. **Wrong event type?** Check `--events` filter matches what the service sends. Use `doppel webhook test <name>` to verify the route works.
