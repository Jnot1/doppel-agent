...
description: "Configure and use Honcho memory with Doppel -- cross-session user modeling, multi-profile peer isolation, observation config, dialectic reasoning, session su..."
---
...
Configure and use Honcho memory with Doppel -- cross-session user modeling, multi-profile peer isolation, observation config, dialectic reasoning, session summaries, and context budget enforcement. Use when setting up Honcho, troubleshooting memory, managing profiles with Honcho peers, or tuning observation, recall, and dialectic settings.

## Skill metadata

| | |
|---|---|
| Source | Optional — install with `doppel skills install official/autonomous-ai-agents/honcho` |
...
| Related skills | [`doppel-agent`](/docs/user-guide/skills/bundled/autonomous-ai-agents/autonomous-ai-agents-doppel-agent) |
...
Honcho provides AI-native cross-session user modeling. It learns who the user is across conversations and gives every Doppel profile its own peer identity while sharing a unified view of the user.
...
See: [Honcho local setup guide](https://docs.honcho.dev/v3/guides/integrations/hermes#running-honcho-locally-with-hermes)

### Verify

```bash
doppel honcho status    # shows resolved config, connection test, peer info
```
...
3. **AI peer card** -- the identity card for this Doppel profile's AI peer
...
Honcho models conversations as interactions between **peers**. Doppel creates two peers per session:

- **User peer** (`peerName`): represents the human. Honcho builds a user representation from observed messages.
- **AI peer** (`aiPeer`): represents this Doppel instance. Each profile gets its own AI peer so agents develop independent views.
...
| `per-session` | New Honcho session each Doppel run |
...
Each Doppel profile gets its own Honcho AI peer while sharing the same workspace (user context). This means:
...
# creates host block doppel.coder, AI peer "coder", inherits config from default
```

What `--clone` does for Honcho:
1. Creates a `doppel.coder` host block in `honcho.json`
...
```bash
doppel honcho sync    # creates host blocks for all profiles that don't have one yet
```
...
Guidelines for Doppel when Honcho memory is active.
...
Run `doppel honcho setup`. Ensure `memory.provider: honcho` is in `~/.doppel/config.yaml`.

### Memory not persisting across sessions
Check `doppel honcho status` -- verify `saveMessages: true` and `writeFrequency` isn't `session` (which only writes on exit).

### Profile not getting its own peer
Use `--clone` when creating: `doppel profile create <name> --clone`. For existing profiles: `doppel honcho sync`.
...
| Command | Description |
|---------|-------------|
| `doppel honcho setup` | Interactive setup wizard (cloud/local, identity, observation, recall, sessions) |
| `doppel honcho status` | Show resolved config, connection test, peer info for active profile |
| `doppel honcho enable` | Enable Honcho for the active profile (creates host block if needed) |
| `doppel honcho disable` | Disable Honcho for the active profile |
| `doppel honcho peer` | Show or update peer names (`--user <name>`, `--ai <name>`, `--reasoning <level>`) |
...
| `doppel honcho tokens` | Show or set token budgets (`--context <N>`, `--dialectic <N>`) |
| `doppel honcho sessions` | List known directory-to-session-name mappings |
| `doppel honcho map <name>` | Map current working directory to a Honcho session name |
| `doppel honcho identity` | Seed AI peer identity or show both peer representations |
| `doppel honcho sync` | Create host blocks for all Doppel profiles that don't have one yet |
| `doppel honcho migrate` | Step-by-step migration guide from OpenClaw native memory to Doppel + Honcho |
| `doppel memory setup` | Generic memory provider picker (selecting "honcho" runs the same wizard) |
| `doppel memory status` | Show active memory provider and config |
| `doppel memory off` | Disable external memory provider |
