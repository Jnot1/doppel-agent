...
| | |
|---|---|
| Source | Optional — install with `doppel skills install official/blockchain/hyperliquid` |
...
The script reads `~/.doppel/.env` for two optional defaults:
...
optional when `HYPERLIQUID_USER_ADDRESS` is set in `~/.doppel/.env`.
...
```bash
python3 ~/.doppel/skills/blockchain/hyperliquid/scripts/hyperliquid_client.py dexs

python3 ~/.doppel/skills/blockchain/hyperliquid/scripts/hyperliquid_client.py \
