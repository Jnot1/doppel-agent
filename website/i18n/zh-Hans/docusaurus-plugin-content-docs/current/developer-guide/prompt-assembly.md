...
1. agent 身份 — 优先使用当前 home 根目录中的 `SOUL.md`（由 `DOPPEL_HOME` / `HERMES_HOME` 或原生根目录解析），否则回退到 `prompt_builder.py` 中的 `DEFAULT_AGENT_IDENTITY`
2. 工具感知行为指导
3. Honcho 静态块（激活时）
4. 可选系统消息
5. 冻结的 MEMORY 快照
...
`SOUL.md` 在新安装中位于 `~/.doppel/SOUL.md`，既有 `~/.hermes/SOUL.md` 根目录也会继续生效。它作为 agent 的身份标识——系统 prompt 的第一个部分。`prompt_builder.py` 中的加载逻辑如下：
...
| 优先级 | 文件 | 搜索范围 | 说明 |
|--------|------|----------|------|
| 1 | `.hermes.md`、`HERMES.md` | 从 CWD 向上至 git 根目录 | Doppel 原生项目配置（保留 legacy 文件名） |
