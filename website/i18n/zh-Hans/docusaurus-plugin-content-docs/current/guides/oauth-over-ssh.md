...
description: "当 Doppel 运行在远程机器、容器或跳板机后面时，如何完成基于浏览器的 OAuth（xAI、Spotify、MCP 服务器）"
---

# SSH / 远程主机上的 OAuth

部分 Doppel 提供商——包括 **xAI Grok OAuth**、**Spotify** 以及**远程 MCP 服务器**——使用*回环重定向（loopback redirect）* OAuth 流程。认证服务器会将浏览器重定向到 `http://127.0.0.1:<port>/callback`，由 `doppel auth ...` 命令启动的一个小型 HTTP 监听器来获取授权码。

当 Doppel 和浏览器在同一台机器上时，这一切运行正常。一旦两者不在同一台机器上就会出问题：你笔记本上的浏览器试图访问**你笔记本**上的 `127.0.0.1`，但监听器绑定的是**远程服务器**上的 `127.0.0.1`。
...
Doppel 对两种路径使用**相同的 PKCE verifier、state 和 nonce**，因此上游 OAuth 流程在字节层面完全一致——`--manual-paste` 纯粹是回调跳转的传输方式变更，不会降低安全性。

## 哪些提供商需要此操作

| 提供商 | 回环端口 | 需要隧道？ |
|----------|---------------|----------------|
| `xai-oauth`（Grok SuperGrok） | `56121` | 是，当 Doppel 在远程时 |
| Spotify | `43827` | 是，当 Doppel 在远程时 |
| MCP 服务器（`auth: oauth`） | 每个服务器自动选择 | 是，当 Doppel 在远程时 |
...
隧道是底层 SSH 连接的属性。如果你在 mosh 会话中的 `tmux` 里运行 Doppel，mosh 的漫游不会携带 `-L` 转发。**单独**开一个普通 SSH 会话**仅用于** `-L` 隧道——这个连接必须在整个认证流程期间保持存活。你的交互式 mosh/tmux 会话可以继续正常运行 Doppel。
...
你笔记本上已有某个程序占用了该端口。可能是上一个隧道没有正常关闭，或者本地也有一个 Doppel 在监听。找到并终止占用进程：
...
Token 写入运行 `doppel auth add ...` 的 Linux 用户目录下。如果你的网关 / systemd 服务以不同用户运行（例如全新 Doppel-first 部署中的专用 `doppel` 用户，或旧安装中的历史 `hermes` 用户），请以**该**用户身份进行认证，使 token 写入其 `~/.doppel/auth.json`；较旧安装仍会使用兼容的 `~/.hermes/auth.json`。全新 Doppel-first 部署可使用 `sudo -u doppel -i`，如果保留了历史默认值则使用 `sudo -u hermes -i`。
...
- [原生 MCP 客户端（OAuth 章节）](../user-guide/features/mcp.md#oauth-authenticated-http-servers)
