...
# 在 Doppel Agent 中使用 SOUL.md

`SOUL.md` 是你的 Doppel Agent 实例的**主要身份标识**。它是系统提示词（system prompt）中的第一项内容——定义了 Agent 是谁、如何表达，以及应避免什么。

如果你希望每次与 Doppel Agent 交谈时都感受到一致的助手风格，或者想用自己的角色完全替换内置默认人设，这就是你需要编辑的文件。
...
- 个性
- 沟通风格
- Doppel Agent 应有多直接或多温和
- Doppel Agent 在风格上应避免什么
- Doppel Agent 如何应对不确定性、分歧和模糊情况

简而言之：
- `SOUL.md` 关注的是 Doppel Agent 是谁，以及 Doppel Agent 如何表达
...
如果 `SOUL.md` 尚不存在，Doppel Agent 会自动为你生成一个初始文件。

这意味着大多数用户一开始就有一个可以立即阅读和编辑的真实文件。

注意：
- 如果你已有 `SOUL.md`，Doppel Agent 不会覆盖它
- 如果文件存在但为空，Doppel Agent 不会从中向提示词添加任何内容
...
Doppel Agent 本身已经尽力做到有帮助且清晰。`SOUL.md` 应当赋予真实的个性和风格，而不是重申显而易见的默认行为。
...
### Doppel Agent 忽略了我 SOUL.md 中的部分内容
