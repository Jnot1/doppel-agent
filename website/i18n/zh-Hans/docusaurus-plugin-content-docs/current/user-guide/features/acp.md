...
sidebar_position: 11
title: "ACP 编辑器集成"
description: "在 VS Code、Zed 和 JetBrains 等兼容 ACP 的编辑器中使用 Doppel Agent"
---

# ACP 编辑器集成

Doppel Agent 可作为 ACP 服务器运行，让兼容 ACP 的编辑器通过 stdio 与 Doppel Agent 通信并渲染：
...
当你希望 Doppel Agent 表现得像编辑器原生的编码 agent，而非独立 CLI 或消息机器人时，ACP 是合适的选择。
...
以下任意命令均可以 ACP 模式启动 Doppel Agent：
...
Doppel Agent 将日志输出到 stderr，以保留 stdout 用于 ACP JSON-RPC 流量。
...
```bash
doppel acp --setup-browser           # 交互式（下载约 400 MB 前会提示确认）
doppel acp --setup-browser --yes     # 非交互式接受下载
```

这是独立命令。Zed registry 的终端认证流程在模型选择后也会将浏览器引导作为后续问题提供，因此大多数用户无需直接运行 `--setup-browser`。

具体操作：

- 若缺少 Node.js 22 LTS，将其安装到 `~/.doppel/node/`
- 将 `npm install -g agent-browser @askjo/camofox-browser` 安装到该前缀（无需 sudo — `npm` 的 `--prefix` 指向用户可写的 Doppel 管理 Node）
...
Doppel Agent 当前 ACP Registry 元数据的源文件位于：
...
ACP 模式使用与 CLI 相同的 Doppel Agent 配置：
...
现有的旧版安装仍可能把这些文件保留在 `~/.hermes/` 下。

Provider 解析使用 Doppel Agent 的正常运行时解析器，因此 ACP 继承当前配置的 provider 和凭据。Doppel Agent 还为首次运行的 registry 客户端提供终端认证方法（`--setup`）；这将打开 Doppel Agent 的交互式模型/provider 设置。
...
底层 `AIAgent` 仍使用 Doppel Agent 的正常持久化/日志路径，但 ACP 的 `list/load/resume/fork` 仅限于当前运行的 ACP 服务器进程。

## 工作目录行为

ACP 会话将编辑器的 cwd 绑定到 Doppel Agent 任务 ID，使文件和终端工具相对于编辑器工作区运行，而非服务器进程的 cwd。
...
| `allow_always` | 始终允许 | 所有未来会话 | 是（写入 Doppel Agent 永久允许列表） |
| `deny` | 拒绝 | 本次工具调用 | 否 |
...
ACP 桥接将这些选项映射到 Doppel Agent 的内部审批语义——`allow_always` 与 CLI 相同地写入永久允许列表条目，而 `allow_session` 仅影响当前 ACP 会话的进程内审批缓存。
...
ACP 模式使用 Doppel Agent 现有的 provider 设置。通过以下方式配置凭据：

```bash
doppel model
```

或编辑 `~/.doppel/.env`。Registry 客户端也可以触发 Doppel Agent 的终端认证流程，该流程运行相同的交互式 provider/模型设置。

### Zed registry 启动器找不到 uv

从官方 uv 安装文档安装 `uv`，然后从 Zed 重试 Doppel Agent 线程。
