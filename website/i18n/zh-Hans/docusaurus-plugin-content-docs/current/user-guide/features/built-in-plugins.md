...
sidebar_label: "内置插件"
title: "内置插件"
description: "随 Doppel Agent 附带并通过生命周期 hook 自动运行的插件——disk-cleanup 等"
---
...
`plugins/memory/` 和 `plugins/context_engine/` 被刻意排除在内置扫描之外。这两个目录使用各自的发现路径，因为内存提供者和上下文引擎是通过 `doppel memory setup` / 配置中的 `context.engine` 进行单选配置的提供者。

## 内置插件默认不启用

内置插件随附时处于禁用状态。发现机制会找到它们（它们会出现在 `doppel plugins list` 和交互式 `doppel plugins` UI 中），但在你明确启用之前不会加载：
...
这与用户安装的插件使用的机制相同。内置插件永远不会自动启用——无论是全新安装，还是现有用户升级到更新版本的 Doppel Agent，都需要你明确选择启用。
...
仓库在 `plugins/` 下附带了以下内置插件。所有插件均需手动启用——通过 `doppel plugins enable <name>` 启用。
...
**再次禁用：** `doppel plugins disable disk-cleanup`。

### observability/langfuse

将 Doppel 的轮次、LLM 调用和工具调用追踪到 [Langfuse](https://langfuse.com)——一个开源 LLM 可观测性平台。每轮一个 span，每次 API 调用一个 generation，每次工具调用一个 tool observation。用量总计、各类型 token 数量和成本估算来自 Doppel Agent 的标准 `agent.usage_pricing` 数据，因此 Langfuse 仪表盘看到的分类（input / output / `cache_read_input_tokens` / `cache_creation_input_tokens` / `reasoning_tokens`）与 `doppel logs` 中显示的一致。
...
或在交互式 `doppel plugins` UI 中勾选复选框。然后将凭据写入 `~/.doppel/.env`：

```bash
DOPPEL_LANGFUSE_PUBLIC_KEY=pk-lf-...
DOPPEL_LANGFUSE_SECRET_KEY=sk-lf-...
DOPPEL_LANGFUSE_BASE_URL=https://cloud.langfuse.com   # 或你的自托管 URL
```
...
| Hook | 行为 |
|---|---|
| `pre_api_request` / `pre_llm_call` | 打开（或复用）每轮的根 span "Doppel turn"。为本次 API 调用启动一个 `generation` 子 observation，将最近的消息序列化为输入。 |
...
| `post_tool_call` | 关闭 tool observation，附加经过清理的 `result`。`read_file` 的内容会被摘要化（头部 + 尾部 + 省略行数），以使大文件读取保持在 `DOPPEL_LANGFUSE_MAX_CHARS` 以内。 |

会话分组基于 Doppel 会话 ID（或子 agent 的任务 ID），通过 `langfuse.propagate_attributes` 实现，因此单次 `doppel chat` 会话中的所有内容都归属于同一个 Langfuse session。

**验证：**

```bash
doppel plugins list                # observability/langfuse 应显示 "enabled"
doppel chat -q "hello"             # 在 Langfuse UI 中检查是否有 "Doppel turn" trace
```
...
| 变量 | 默认值 | 用途 |
|---|---|---|
| `DOPPEL_LANGFUSE_ENV` | — | trace 上的环境标签（`production`、`staging` 等） |
| `DOPPEL_LANGFUSE_RELEASE` | — | 发布/版本标签 |
| `DOPPEL_LANGFUSE_SAMPLE_RATE` | `1.0` | 传递给 SDK 的采样率（0.0–1.0） |
...
**禁用：** `doppel plugins disable observability/langfuse`。插件模块仍会被发现，但在你重新启用之前不会运行任何模块代码。
...
| 文件 | 内容 |
|---|---|
| `state.json` | 解锁历史：你获得了哪些徽章以及获得时间。在 Doppel Agent 更新间保持稳定。 |
