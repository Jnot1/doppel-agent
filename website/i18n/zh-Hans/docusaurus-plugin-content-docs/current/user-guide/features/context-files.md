...
Doppel Agent 会自动发现并加载上下文文件，以塑造其行为方式。部分文件属于项目本地文件，从工作目录中发现。`SOUL.md` 现在对当前 Doppel Agent 实例全局生效，并且仅从解析后的 home 根目录加载，优先使用 `DOPPEL_HOME`，同时保留 legacy `HERMES_HOME` 支持。
...
| **SOUL.md** | 当前 Doppel Agent 实例的全局个性与语气定制 | 仅解析后的 home 根目录（`DOPPEL_HOME` / legacy `HERMES_HOME`） |
...
- `~/.hermes/SOUL.md`（既有 legacy 安装会原地保留）
- 或 `$DOPPEL_HOME/SOUL.md` / legacy `$HERMES_HOME/SOUL.md`（若使用自定义主目录运行 Doppel Agent）

重要说明：

- 若 `SOUL.md` 尚不存在，Doppel Agent 会自动生成一个默认文件
- Doppel Agent 仅从解析后的 home 根目录加载 `SOUL.md`（先读 `DOPPEL_HOME`，legacy `HERMES_HOME` 仍可使用）
...
Doppel Agent 兼容 Cursor IDE 的 `.cursorrules` 文件和 `.cursor/rules/*.mdc` 规则模块。若这些文件存在于项目根目录，且未找到更高优先级的上下文文件（`.hermes.md`、`AGENTS.md` 或 `CLAUDE.md`），则将其作为项目上下文加载。
