...
[Honcho](https://github.com/plastic-labs/honcho) 是一个 AI 原生记忆后端，在 Doppel Agent 内置记忆系统之上增加了辩证推理（dialectic reasoning）和深度用户建模能力。它不是简单的键值存储，而是通过对对话事后推理，持续维护一个关于用户的动态模型——涵盖其偏好、沟通风格、目标与行为模式。
...
**多 agent 画像**：当多个 Doppel Agent 实例与同一用户交互时（例如编程助手和个人助手），Honcho 为每个 peer 维护独立画像。每个 peer 只能看到自己的观察和结论，防止上下文交叉污染。

## 设置

```bash
doppel memory setup    # 从 provider 列表中选择 "honcho"
```
...
旧安装仍可能将相同配置保存在 `~/.hermes/config.yaml` 和 `~/.hermes/.env` 中。
...
Honcho 在 `~/.honcho/config.json`（全局）或 `$DOPPEL_HOME/honcho.json`（profile 本地；旧版 `$HERMES_HOME/honcho.json` 仍然可用）中配置。设置向导会自动处理。

### 使用认证的自托管 Honcho

当你将 Doppel Agent 指向自托管 Honcho 服务器时，`doppel honcho setup`（以及 `doppel memory setup`）会在基础 URL 之后提示输入**本地 JWT / bearer token**。粘贴一个由服务器 `AUTH_JWT_SECRET`（Honcho compose 环境变量）签名的 JWT，即可启用认证访问；如果服务器运行时设置了 `AUTH_USE_AUTH=false`，则可留空。该本地 token 会存储在主机块下（`honcho.json` 中的 `hosts.<host>.apiKey`），与云端 `apiKey` 分开，因此你之后可以把 `Cloud or local?` 提示切回 `cloud`，而不会丢失任一凭据。
...
- `per-session` — 每次 `doppel` 运行获得一个新会话。干净启动，通过 tools 访问记忆。推荐新用户使用。
...
通过 [Honcho 控制台](https://app.honcho.dev) 设置的服务端开关优先于本地默认值——Doppel Agent 会在会话初始化时同步回本地。
...
`doppel honcho` 子命令**仅在 Honcho 为当前活跃 memory provider 时注册**（`config.yaml` 中 `memory.provider: honcho`）。全新安装时，可直接运行 `doppel memory setup honcho` 配置 Honcho（或运行 `doppel memory setup` 再从列表中选择它）；随后下次调用时 `doppel honcho` 子命令就会出现。

```bash
doppel memory setup honcho    # 直接配置 Honcho（激活前也可运行）
doppel honcho status          # 连接状态、配置及关键设置
doppel honcho setup           # 重定向到 `doppel memory setup`
doppel honcho strategy        # 查看或设置会话策略（per-session/per-directory/per-repo/global）
doppel honcho peer            # 查看或更新 peer 名称及辩证推理级别
doppel honcho mode            # 查看或设置 recall 模式（hybrid/context/tools）
doppel honcho tokens          # 查看或设置上下文和辩证的 token 预算
doppel honcho identity        # 初始化或查看 AI peer 的 Honcho 身份
doppel honcho sync            # 将 Honcho 配置同步到所有现有 profile
doppel honcho peers           # 查看所有 profile 中的 peer 身份
doppel honcho sessions        # 列出已知的 Honcho 会话映射
doppel honcho map             # 将当前目录映射到 Honcho 会话名称
doppel honcho enable          # 为当前 profile 启用 Honcho
doppel honcho disable         # 为当前 profile 禁用 Honcho
doppel honcho migrate         # 从 openclaw-honcho 迁移的分步指南
```
...
无需重新登录或重新设置。运行 `doppel memory setup` 并选择"honcho"——向导会自动检测你的现有配置。
