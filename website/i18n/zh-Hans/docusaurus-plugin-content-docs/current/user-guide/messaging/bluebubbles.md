...
通过 [BlueBubbles](https://bluebubbles.app/) 将 Doppel Agent 连接至 Apple iMessage——这是一款免费、开源的 macOS 服务端，可将 iMessage 桥接至任意设备。
...
- Doppel Agent 与 BlueBubbles 服务端之间的网络连通性
...
或直接在你的 agent-home `.env` 文件中设置环境变量（全新安装默认为 `~/.doppel/.env`；旧版 `~/.hermes/.env` 仍兼容）：
...
**DM 配对（推荐）：**
当有人向你的 iMessage 发送消息时，Doppel Agent 会自动向其发送配对码。使用以下命令批准：
```bash
doppel pairing approve bluebubbles <CODE>
```
使用 `doppel pairing list` 查看待处理的配对码和已授权用户。

**预授权特定用户**（在你的 agent-home `.env` 文件中）：
```bash
BLUEBUBBLES_ALLOWED_USERS=user@icloud.com,+15551234567
```

**开放访问**（在你的 agent-home `.env` 文件中）：
```bash
...
Doppel Agent 将连接至你的 BlueBubbles 服务端，注册 webhook，并开始监听 iMessage 消息。
...
- **出站：** Doppel Agent 通过 BlueBubbles REST API 发送消息。
...
自动将消息标记为已读由你的 agent-home `config.yaml` 中 `platforms.bluebubbles.extra` 下的 `send_read_receipts` 键控制（全新安装默认为 `~/.doppel/config.yaml`；旧版 `~/.hermes/config.yaml` 仍兼容；默认值：`true`）。该选项没有对应的环境变量。
...
### 聊天寻址
你可以通过邮箱或手机号寻址聊天——Doppel Agent 会自动将其解析为 BlueBubbles 聊天 GUID，无需使用原始 GUID 格式。
...
- 查看 `doppel logs gateway` 中的 webhook 错误（或使用 `doppel logs -f` 实时跟踪）
