...
sidebar_position: 1
title: "消息网关"
description: "通过 Telegram、Discord、Slack、WhatsApp、Signal、SMS、Email、Home Assistant、Mattermost、Matrix、DingTalk、Yuanbao、Microsoft Teams、LINE、Webhooks 或任何兼容 OpenAI 的前端与 Doppel Agent 对话 — 架构与配置概览"
---

# 消息网关

通过 Telegram、Discord、Slack、WhatsApp、Signal、SMS、Email、Home Assistant、Mattermost、Matrix、DingTalk、Feishu/Lark、WeCom、Weixin、BlueBubbles（iMessage）、QQ、Yuanbao、Microsoft Teams、LINE、ntfy 或浏览器与 Doppel Agent 对话。网关是一个单一后台进程，连接所有已配置的平台，管理会话，运行 cron 任务，并传递语音消息。

完整的语音功能集——包括 CLI 麦克风模式、消息中的语音回复以及 Discord 语音频道对话——请参阅 [Voice Mode](/user-guide/features/voice-mode) 和 [在 Doppel Agent 中使用语音模式](/guides/use-voice-mode-with-doppel-agent)。
...
```bash
doppel gateway setup        # 交互式配置所有消息平台
```
...
```bash
doppel gateway              # 在前台运行
doppel gateway setup        # 交互式配置消息平台
doppel gateway install      # 安装为用户服务（Linux）/ launchd 服务（macOS）
sudo doppel gateway install --system   # 仅 Linux：安装开机启动的系统服务
doppel gateway start        # 启动默认服务
doppel gateway stop         # 停止默认服务
doppel gateway status       # 检查默认服务状态
doppel gateway status --system         # 仅 Linux：显式检查系统服务
```
...
在 `~/.doppel/gateway.json` 中配置各平台的覆盖设置（旧版 `~/.hermes/gateway.json` 仍兼容）：
...
# 其他配对命令：
doppel pairing list          # 查看待审核和已批准的用户
doppel pairing revoke telegram 123456789  # 撤销访问权限
```
...
第一次在任意平台向繁忙的 agent 发送消息时，Doppel 会在繁忙确认中附加一行提示，说明该配置项（`"💡 First-time tip — …"`）。该提示每次安装只触发一次——由 `onboarding.seen.busy_input_prompt` 下的标志锁定。删除该键可再次看到提示。

如果你觉得繁忙确认消息过多——尤其是使用语音输入或快速连续发送消息时——可设置 `display.busy_ack_enabled: false`。你的输入仍会正常排队/引导/中断，只是聊天回复被静默。

## 工具进度通知

在你的 agent-home `config.yaml` 中控制显示多少工具活动信息（全新安装默认为 `~/.doppel/config.yaml`；旧版 `~/.hermes/config.yaml` 仍兼容）：
...
当运行后台会话的 agent 使用 `terminal(background=true)` 启动长时间运行的进程（服务器、构建等）时，网关可以向你的聊天推送状态更新。通过你的 agent-home `config.yaml` 中的 `display.background_process_notifications` 控制（全新安装默认为 `~/.doppel/config.yaml`；旧版 `~/.hermes/config.yaml` 仍兼容）：
...
doppel gateway start                 # 启动服务
doppel gateway stop                  # 停止服务
doppel gateway status                # 检查状态
journalctl --user -u doppel-gateway -f  # 查看日志
...
除非你确实有此需要，否则避免同时安装用户和系统网关单元。Doppel 检测到两者同时存在时会发出警告，因为 start/stop/status 行为会变得不明确。

:::info 多个安装
如果你在同一台机器上运行多个 Doppel 安装，每个安装都有自己的 systemd 服务名称。全新安装优先使用 `DOPPEL_HOME` 并默认映射到 `~/.doppel`，对应 `doppel-gateway`；命名配置文件使用 `doppel-gateway-coder` 之类的后缀。旧版 `HERMES_HOME` 安装和 `hermes-gateway*` 单元仍会被检测，因此升级流程保持正常，`doppel gateway` 会自动针对当前活动 home 的正确服务。
:::

### macOS（launchd）

```bash
doppel gateway install               # 安装为 launchd agent
doppel gateway start                 # 启动服务
doppel gateway stop                  # 停止服务
doppel gateway status                # 检查状态
tail -f ~/.doppel/logs/gateway.log   # 查看日志（全新安装）
```

全新安装生成的 plist 文件位于 `~/Library/LaunchAgents/ai.doppel.gateway.plist`。旧版 `ai.hermes.gateway*.plist` 文件仍会在存在时被检测并迁移。它包含三个环境变量：
...
- **DOPPEL_HOME** — 全新安装优先使用的 home 环境变量。旧版 launchd 流程仍可能导出 `HERMES_HOME`，网关在迁移期间同时兼容这两者。

:::tip 安装后 PATH 变更
launchd plist 是静态的——如果你在配置网关后安装了新工具（例如通过 nvm 安装新版 Node.js，或通过 Homebrew 安装 ffmpeg），请重新运行 `doppel gateway install` 以捕获更新后的 PATH。网关会检测到过时的 plist 并自动重新加载。
:::

:::info 多个安装
与 Linux systemd 服务类似，每个 agent-home 目录都有自己的 launchd 标签。全新安装使用 `ai.doppel.gateway`（以及 `ai.doppel.gateway-coder` 之类的配置文件变体）。升级系统上可能仍存在旧版 `ai.hermes.gateway*` 标签，网关会自动处理。
:::
...
1. **网关日志**（全新安装为 `~/.doppel/logs/gateway.log`，旧版为 `~/.hermes/logs/gateway.log`，或 systemd / launchd 单元日志）。搜索平台名称以及 `circuit breaker`、`paused` 或 `disabled`。触发事件包含失败次数和最后一个错误。
