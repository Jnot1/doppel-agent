...
sidebar_position: 17
title: "LINE"
description: "将 Doppel Agent 设置为 LINE Messaging API 机器人"
---

# LINE 配置

通过官方 LINE Messaging API 将 Doppel Agent 作为 [LINE](https://line.me/) 机器人运行。适配器以捆绑平台插件的形式存放于 `plugins/platforms/line/` — 无需修改核心代码，像其他平台一样启用即可。

LINE 是日本、台湾和泰国的主流即时通讯应用。如果你的用户在这些地区，这就是他们与你沟通的方式。

> 运行 `doppel gateway setup` 并选择 **LINE**，即可跟随引导完成配置。
...
在 agent-home `.env` 文件中添加（全新安装通常是 `~/.doppel/.env`；旧安装的 `~/.hermes/.env` 仍可继续使用）：
...
然后在 agent-home `config.yaml` 中添加（全新安装通常是 `~/.doppel/config.yaml`；旧安装的 `~/.hermes/config.yaml` 仍可继续使用）：
...
**机器人在群组中收不到消息。** 检查 `LINE_ALLOWED_GROUPS` 是否包含对应的 `C...` 群组 ID。如需查找群组 ID，发送一条测试消息后在全新安装的 `~/.doppel/logs/gateway.log` 中搜索 `LINE: rejecting unauthorized source`（旧安装的 `~/.hermes/logs/gateway.log` 仍可继续使用）— 被拒绝的 source 字典中包含相关 ID。
...
**"already in use by another profile"。** 同一个频道访问 token 已被另一个运行中的 Doppel profile 占用。请停止另一个 gateway，或使用独立的频道。
