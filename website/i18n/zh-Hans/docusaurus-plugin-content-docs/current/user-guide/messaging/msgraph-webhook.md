...
description: "在 Doppel Agent 中接收 Microsoft Graph 变更通知（会议、日历、聊天等）"
---

# Microsoft Graph Webhook 监听器

`msgraph_webhook` gateway 平台是一个入站事件监听器。它是 Doppel Agent 接收来自 Microsoft Graph 的**变更通知**的方式——"一个 Teams 会议已结束"、"此聊天中收到了一条新消息"、"此日历事件已更新"。与 `teams` 平台（用户向其发送消息的聊天机器人）不同——此平台是 M365 告知 Doppel Agent 某事已发生，而非来自用户的消息。

目前主要的消费者是 Teams 会议摘要流水线：Graph 在会议产生转录文本时发出通知，流水线获取该内容，Doppel Agent 将摘要发回 Teams。其他 Graph 资源（`/chats/.../messages`、`/users/.../events`）使用同一监听器——流水线消费者通过各自的 PR 接入。
...
- 一个强共享密钥，用作 `clientState` 的值。使用 `openssl rand -hex 32` 生成，并以 `MSGRAPH_WEBHOOK_CLIENT_STATE` 写入你的 agent-home `.env` 文件（全新安装默认为 `~/.doppel/.env`；旧版 `~/.hermes/.env` 仍兼容）。

## 快速开始

最小化 agent-home `config.yaml`（全新安装默认为 `~/.doppel/config.yaml`；旧版 `~/.hermes/config.yaml` 仍兼容）：
...
或通过你的 agent-home `.env` 文件中的环境变量（启动时自动合并）：
...
注意：绑定地址从 `config.yaml` 中的 `extra.host` 读取（见上方示例）；没有 `MSGRAPH_WEBHOOK_HOST` 环境变量覆盖项。

启动 gateway：`doppel gateway run`。监听器暴露以下端点：
...
| 设置 | 默认值 | 说明 |
|------|--------|------|
| `host` | `0.0.0.0` | HTTP 监听器的绑定地址。非回环绑定需要 `allowed_source_cidrs`；回环（`127.0.0.1` / `::1`）最适合 dev tunnel / 本地反向代理。 |
| `port` | `8646` | 绑定端口。 |
...
| `allowed_source_cidrs` | `[]` | 非回环绑定时必填。仅当监听器绑定到回环并由本地 tunnel / 反向代理转发时才应留空。 |
...
若在未设置 `allowed_source_cidrs` 的情况下绑定到 `0.0.0.0`、`::` 或局域网 IP，监听器会在启动时拒绝运行。若你在同一台机器上使用 dev tunnel 或反向代理，请将 Doppel Agent 绑定到 `127.0.0.1` 或 `::1`，并在该场景下保持白名单为空。无效的 CIDR 字符串会记录警告并被忽略。**请每季度审查 Microsoft IP 列表**——它会变更。
...
| 每条通知均返回 403 | `clientState` 不匹配（伪造，或订阅时使用了不同的值）。使用 `doppel teams-pipeline subscribe --client-state "$MSGRAPH_WEBHOOK_CLIENT_STATE" ...` 重新创建订阅（随流水线运行时 PR 一同发布）。 |
| 监听器拒绝在 `0.0.0.0` 上启动 | 将 `allowed_source_cidrs` 设为 Microsoft 当前 webhook 出口地址范围，或将 Doppel Agent 绑定到 `127.0.0.1` / `::1` 并放在 tunnel 或反向代理后面。 |
| 监听器已启动，但 `curl http://localhost:8646/health` 挂起 | 端口绑定冲突。检查 `ss -tlnp \| grep 8646`，如有需要更改 `port:`。 |
| 来自 Microsoft 的真实 Graph 请求返回 403 | 源 IP 白名单范围过窄。将范围扩展至包含当前 Microsoft 出口地址；若仍在验证 tunnel 路径，可先将 Doppel Agent 绑定到回环，由 tunnel 负责公网暴露。 |
...
- [Microsoft Teams 机器人设置](/user-guide/messaging/teams) — 允许用户在 Teams 中与 Doppel Agent 聊天的另一平台
