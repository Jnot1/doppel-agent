...
通过**官方 QQ Bot API（v2）**将 Doppel Agent 接入 QQ——支持私聊（C2C）、群组 @-提及、频道及直接消息，并具备语音转写功能。
...
在你的 agent-home `.env` 文件中设置所需环境变量（全新安装默认为 `~/.doppel/.env`；旧版 `~/.hermes/.env` 仍兼容）：
...
如需精细控制，可在你的 agent-home `config.yaml` 中添加平台设置（全新安装默认为 `~/.doppel/config.yaml`；旧版 `~/.hermes/config.yaml` 仍兼容）：
