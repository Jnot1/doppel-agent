...
sidebar_position: 5
title: "WhatsApp"
description: "通过内置 Baileys 桥接将 Doppel Agent 设置为 WhatsApp 机器人"
---

# WhatsApp 配置

Doppel Agent 通过基于 **Baileys** 的内置桥接连接到 WhatsApp。其工作原理是模拟 WhatsApp Web 会话——**而非**通过官方 WhatsApp Business API。无需 Meta 开发者账号或 Business 认证。

> 运行 `doppel gateway setup` 并选择 **WhatsApp**，即可跟随引导完成配置。
...
发生这种情况时，Doppel Agent 会更新桥接依赖。如果机器人在 WhatsApp 更新后停止工作，
请拉取最新版 Doppel Agent 并重新配对。
:::
...
2. 用新号码注册 WhatsApp
3. 运行 `doppel whatsapp` 并从该 WhatsApp 账号扫描二维码

---

## 第三步：配置 Doppel Agent

在 agent-home `.env` 文件中添加以下内容（全新安装通常是 `~/.doppel/.env`；旧安装的 `~/.hermes/.env` 仍可继续使用）：
...
在 agent-home `config.yaml` 中添加可选行为设置（全新安装通常是 `~/.doppel/config.yaml`；旧安装的 `~/.hermes/config.yaml` 仍可继续使用）：
...
Baileys 桥接会在全新安装时将会话保存在 `~/.doppel/platforms/whatsapp/session`（旧安装的 `~/.hermes/platforms/whatsapp/session` 仍可继续使用）。这意味着：
...
Doppel Agent 支持 WhatsApp 上的语音功能：
...
- Agent 响应默认以"⚕ **Doppel Agent**"为前缀。可在 `config.yaml` 中自定义或禁用：
...
| **二维码过期** | 二维码约每 20 秒刷新一次。如果超时，重新运行 `doppel whatsapp`。 |
| **会话未持久化** | 检查 WhatsApp 会话目录是否存在且可写（全新安装通常是 `~/.doppel/platforms/whatsapp/session`；旧安装的 `~/.hermes/platforms/whatsapp/session` 仍可继续使用）。如在容器中运行，请将其挂载为持久卷。 |
| **意外退出登录** | WhatsApp 会在长时间不活跃后取消关联设备。保持手机开机并连接网络，如有需要使用 `doppel whatsapp` 重新配对。 |
| **桥接崩溃或重连循环** | 重启 gateway，更新 Doppel Agent，如会话因 WhatsApp 协议变更而失效则重新配对。 |
| **WhatsApp 更新后机器人停止工作** | 更新 Doppel Agent 以获取最新桥接版本，然后重新配对。 |
| **macOS："Node.js not installed"但终端中 node 可用** | launchd 服务不继承你的 shell PATH。运行 `doppel gateway install` 将当前 PATH 重新快照到 plist 中，然后运行 `doppel gateway start`。详见 [Gateway 服务文档](./index.md#macos-launchd)。 |
...
| **机器人向陌生人回复配对码** | 如需对未授权私信静默处理，请在 agent-home `config.yaml` 中设置 `whatsapp.unauthorized_dm_behavior: ignore`。 |
...
- WhatsApp 会话目录包含完整会话凭证——请像保护密码一样保护它（全新安装通常是 `~/.doppel/platforms/whatsapp/session`；旧安装的 `~/.hermes/platforms/whatsapp/session` 仍可继续使用）
- 设置文件权限：`chmod 700 ~/.doppel/platforms/whatsapp/session`（若你仍在使用旧安装路径，则对 `~/.hermes/platforms/whatsapp/session` 执行同样操作）
