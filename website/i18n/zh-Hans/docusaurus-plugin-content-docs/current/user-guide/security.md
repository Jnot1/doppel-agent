...
Doppel Agent 采用纵深防御安全模型。本页涵盖所有安全边界——从命令审批到容器隔离，再到消息平台上的用户授权。
...
在执行任何命令之前，Doppel 会将其与一份精心维护的危险模式列表进行比对。若匹配，用户必须明确批准。

### 审批模式

审批系统支持三种模式，通过 agent-home `config.yaml` 中的 `approvals.mode` 配置（新安装默认位于 `~/.doppel/config.yaml`；旧版 `~/.hermes/config.yaml` 仍然有效）：

```yaml
approvals:
  mode: manual                    # manual | smart | off
  timeout: 60                     # 等待用户响应的秒数（默认：60）
  cron_mode: deny                 # deny | approve — cron 任务遇到危险命令时如何处理
  mcp_reload_confirm: true        # /reload-mcp 在使 MCP 工具缓存失效前先确认
  destructive_slash_confirm: true # /clear、/new、/reset、/undo 在丢弃状态前先确认
```
...
| `timeout` | `60` | Doppel 等待审批回复后超时的秒数。 |
| `cron_mode` | `deny` | [cron 任务](./features/cron.md) 在无头运行时触发危险命令提示后的行为。`deny` 会阻止命令（Agent 必须寻找其他路径）；`approve` 会在 cron 上下文中自动批准所有内容。 |
| `mcp_reload_confirm` | `true` | 为真时，`/reload-mcp` 会在重建 MCP 工具集前先确认。重建会使提供商提示词缓存失效（工具 schema 位于系统提示中），因此下一条消息需要重新发送完整输入令牌。用户点击 **Always Approve** 后会把该键改为 `false`。 |
| `destructive_slash_confirm` | `true` | 为真时，破坏性会话斜杠命令（`/clear`、`/new`、`/reset`、`/undo`）会在丢弃会话状态前先确认。该三选一对话框（Approve Once / Always Approve / Cancel）会通过 Telegram、Discord 和 Slack 的原生是/否按钮路由，其他平台回退到文本交互。用户点击 **Always Approve** 后会把该键改为 `false`。TUI 使用自己的模态层（如需关闭，设置 `HERMES_TUI_NO_CONFIRM=1`）。 |
...
1. **CLI 标志**：使用 `doppel --yolo` 或 `doppel chat --yolo` 启动会话
...
当 YOLO 激活时，Doppel 会显示两个持久的视觉提醒，以确保用户不会忘记审批提示已被绕过：
...
某些命令极具破坏性——不可逆的文件系统清除、fork 炸弹、直接写入块设备——无论以下任何情况，Doppel 都**拒绝**执行：
...
在 agent-home `config.yaml` 中配置超时（新安装默认位于 `~/.doppel/config.yaml`；旧版 `~/.hermes/config.yaml` 仍然有效）：
...
通过"always"批准的命令会保存到 agent-home `config.yaml`（新安装默认位于 `~/.doppel/config.yaml`；旧版 `~/.hermes/config.yaml` 仍然有效）：
...
:::tip
使用 `doppel config edit` 查看或删除永久允许列表中的模式。
:::

## 用户授权（Gateway）

运行消息 gateway 时，Doppel 通过分层授权系统控制谁可以与机器人交互。
...
在 agent-home `.env` 文件中以逗号分隔的值设置允许的用户 ID（新安装默认位于 `~/.doppel/.env`；旧版 `~/.hermes/.env` 仍然有效）：
...
为实现更灵活的授权，Doppel 提供了基于验证码的配对系统。无需预先提供用户 ID，未知用户会收到一次性配对码，由机器人所有者通过 CLI 批准。

**工作原理：**

1. 未知用户向机器人发送 DM
2. 机器人回复一个 8 位配对码
3. 机器人所有者在 CLI 上运行 `doppel pairing approve <platform> <code>`
4. 该用户在该平台上获得永久批准

在 agent-home `config.yaml` 中控制未授权私信的处理方式（新安装默认位于 `~/.doppel/config.yaml`；旧版 `~/.hermes/config.yaml` 仍然有效）：
...
**存储：** 配对数据在新安装中存储于 `~/.doppel/pairing/`（旧版 `~/.hermes/pairing/` 仍然有效），按平台分为独立的 JSON 文件：
...
使用 `docker` 终端后端时，Doppel 会对每个容器应用严格的安全加固。
...
容器资源可在 agent-home `config.yaml` 中配置（新安装默认位于 `~/.doppel/config.yaml`；旧版 `~/.hermes/config.yaml` 仍然有效）：
...
加载后，Doppel 会检查这些文件是否存在于活跃 profile 的 `HERMES_HOME` 中，并将其注册为挂载：
...
路径相对于 `~/.hermes/`。文件在容器内挂载到 `/root/.hermes/`。此列表由 `tools/credential_files.py`（`terminal.credential_files`）读取；它位于 `terminal:` 配置块下，但由凭据文件模块而非核心终端后端加载，因此不属于内置的 `DEFAULT_CONFIG` 快照。
...
| **terminal**（本地） | 阻止明确的 Doppel 基础设施变量（提供商密钥、gateway token、工具 API 密钥） | ✅ 透传变量绕过黑名单 |
...
- Doppel 基础设施密钥（提供商 API 密钥、gateway token）不应添加到 `env_passthrough`——它们有专用机制
...
Doppel Agent 集成了 [tirith](https://github.com/sheeki03/tirith)，用于在执行前进行内容级命令扫描。Tirith 能检测单纯模式匹配所遗漏的威胁：
...
Tirith 为 Linux（x86_64 / aarch64）和 macOS（x86_64 / arm64）提供预构建二进制文件。在没有预构建二进制文件的平台（Windows 等）上，tirith 会被静默跳过——模式匹配防护仍然运行，CLI 不会显示"不可用"横幅。若要在 Windows 上使用 tirith，请在 WSL 下运行 Doppel Agent。
...
4. **安全存储密钥** — 将 API 密钥保存在 agent-home `.env` 文件中（新安装默认位于 `~/.doppel/.env`；旧版 `~/.hermes/.env` 仍然有效），并设置合适的文件权限
...
9. **监控日志** — 在新安装中检查 `~/.doppel/logs/`（旧版 `~/.hermes/logs/` 仍然有效）中的未授权访问尝试
10. **保持更新** — 定期运行 `doppel update` 以获取安全补丁
...
为获得最高安全性，请在独立的机器或虚拟机上运行 gateway。在 `config.yaml` 中设置 `terminal.backend: ssh`，然后通过 agent-home `.env` 文件中的环境变量提供主机详情（新安装默认位于 `~/.doppel/.env`；旧版 `~/.hermes/.env` 仍然有效）：
...
Doppel Agent 内置了一个公告扫描器，用于标记活跃 venv 中与已知受损版本目录匹配的 Python 包（例如 2026 年 5 月的 `mistralai 2.4.6` 供应链投毒事件）。实现位于 `hermes_cli/security_advisories.py`。

运行方式：

- **CLI 启动横幅。** 若有任何公告匹配，会打印一行警告，并指向 `doppel doctor` 获取完整修复方案。
- **`doppel doctor`。** 显示所有活跃公告的版本详情和 2-4 步修复说明。
...
3. 若安装失败或用户已禁用懒加载安装，调用会抛出 `FeatureUnavailable`，附带实际的 pip stderr 和指向 `doppel tools` 的提示。
...
禁用后，需要可选依赖的后端会提示用户手动运行安装（`pip install …`）或通过 `doppel tools` 选择其他后端。
