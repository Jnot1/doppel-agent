---
title: "Doppel Agent Skill 编写——在仓库中编写 SKILL"
sidebar_label: "Doppel Agent Skill 编写"
description: "在仓库中编写 SKILL.md"
---

{/* This page is auto-generated from the skill's SKILL.md by website/scripts/generate-skill-docs.py. Edit the source SKILL.md, not this page. */}

# Doppel Agent Skill 编写
...
| 标签 | `skills`, `authoring`, `doppel-agent`, `hermes-agent`, `conventions`, `skill-md` |
...
:::info
以下是 Doppel 在触发此 skill 时加载的完整 skill 定义。这是 agent 在 skill 激活时所看到的指令内容。
:::

# 编写 Doppel Agent Skills（仓库内）

## 概述

SKILL.md 可以存放在两个位置：

1. **用户本地：** `~/.doppel/skills/<maybe-category>/<name>/SKILL.md` 用于全新安装（旧版 `~/.hermes/skills/<maybe-category>/<name>/SKILL.md` 仍可继续使用）——个人使用，不共享。通过 `skill_manage(action='create')` 创建。
2. **仓库内（本 skill 讨论此情况）：** `/path/to/doppel-agent/skills/<category>/<name>/SKILL.md` — 已提交，随包一起发布。使用 `write_file` + `git add`。`skill_manage(action='create')` **不**针对此目录树。

## 使用时机

- 用户要求你"在此分支 / 仓库 / 提交中"添加一个 skill
- 你正在提交一个应随 Doppel Agent 一起发布的可复用工作流
- 你正在编辑 `/path/to/doppel-agent/skills/` 下的现有 skill（小改动用 `patch`，重写用 `write_file`；`skill_manage` 对仓库内 skill 的 `patch` 仍有效，但 `create` 无效）
...
`metadata.hermes.related_skills` 在加载时会合并两个目录树（仓库内 `skills/` 和 `~/.doppel/skills/`，兼容 `~/.hermes/skills/`）。你**可以**从仓库内 skill 引用用户本地 skill，但对于全新克隆仓库的其他用户，该引用无法解析。仓库内 skill 优先只引用仓库内 skill。如果某个频繁被引用的 skill 仅存在于 `~/.doppel/skills/`，请考虑将其提升到仓库中。
...
1. **对仓库内 skill 使用 `skill_manage(action='create')`。** 它会写入 `~/.doppel/skills/`，而非仓库目录树。仓库内创建请使用 `write_file`。
...
- [ ] 文件位于 `skills/<category>/<name>/SKILL.md`（不在 `~/.doppel/skills/` 或旧版 `~/.hermes/skills/` 中）
